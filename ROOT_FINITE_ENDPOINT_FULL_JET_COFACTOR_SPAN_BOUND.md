...
or all five tangent covectors lie in one coordinate-pair plane.            (11)
```

This condition is independent of the two-port factorization and survives
dense root--root tangent graphs.  The next exact task is to combine (11) with
the first-jet quotient frames and the complementary-hafnian identities on
the two deletion classes; neither branch is claimed empty here.
