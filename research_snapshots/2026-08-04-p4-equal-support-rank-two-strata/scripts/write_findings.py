#!/usr/bin/env python3
"""Emit the findings.md deliverable for agent3_out (Task A / Task B summary).
The content is the exact statement list; every claim is replayed by the
step*.py scripts in this directory."""
import pathlib

OUT = pathlib.Path(__file__).resolve().parent.parent / "findings.md"

CONTENT = r"""# Findings: equal-support in-out stratum (Task A) and rank-two-dominated strata (Task B)

Exact computations over Q (sympy rationals + Singular factorization /
primary decomposition).  Scripts in `scripts/` (step1..step26, each
self-contained); conventions follow
...
print("wrote", OUT, len(CONTENT), "chars")
