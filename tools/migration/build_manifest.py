...
# Confidence controls proposal labeling only.  It never supplies review
# authority or operational approval.
HIGH_CONFIDENCE_PROPOSAL_LEVELS = {"high"}
...
def status_for(e: dict, is_pilot: bool, already_moved: set[str]) -> str:
    """Assign proposal status; never infer review or execution approval."""
    if e["old_path"] in already_moved:
        return "moved"
    if is_pilot:
        return "pilot"
    if e.get("confidence") in HIGH_CONFIDENCE_PROPOSAL_LEVELS:
    ...
    manifest = {
        ...
        "status_model": {
            ...
            "proposed_high_confidence": "classifier says high "
                                        "confidence; this is not review "
                                        "authority or operational approval "
                                        "and requires authorized review "
                                        "plus an exact frozen batch",
            "review_required": "medium/low-confidence proposal; requires "
                               "authorized ownership review before it may "
                               "be batched",
        },
        "approval_model": {
            "rule": "classification confidence is never review authority "
                    "or executable on its own; execute_moves.py runs only "
                    "exact frozen batches with an actual reviewer, base "
                    "SHA, member mapping, and mapping hash",
            "standing_delegation": "repository-owner standing delegation "
                    "dated 2026-08-08 authorizes Codex to act as actual "
                    "reviewer for routine, non-ambiguous, evidence-backed "
                    "exact layout mappings; approved_by must identify "
                    "Codex and the delegation basis",
            "owner_gates": "scientific status or scope, genuinely "
                    "ambiguous proof-boundary ownership, and architectural "
                    "choices requiring owner preference remain owner-gated",
