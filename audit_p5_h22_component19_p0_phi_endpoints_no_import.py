#!/usr/bin/env python3
"""No-import verifier for component 19 at p=0 and phi=+/-1."""
...
REPORT = ROOT / "P5_H22_COMPONENT19_P0_PHI_ENDPOINTS_NO_IMPORT_VERIFICATION.md"
...
q, lam = sp.symbols("q lambda", nonzero=True)
...
for epsilon in (1, -1):
    ...
    assert_equal(raw_support[(1, 1, 1, 1)], 4 * (q - epsilon))
    ...
    assert_equal(
        sp.Matrix(((epsilon, -1), (-1, q))).det(), epsilon * q - 1
    )
    infinity_l = C1 + C2
    assert_equal(infinite["23"][(0, 0, 0, 0)], -2 * infinity_l)
    assert_equal(infinite["01"][(0, 0, 0, 1)], -2 * (epsilon * C1 - C2))
    ...
    r = q - epsilon
    v_x = sp.Matrix((0, -1 / r, epsilon / r, 0, 1, 0, 0, 0))
    ...
    L = (epsilon * q + 1) * X + 2 * q * Z
    generator_01 = sp.factor(-64 * X * (q**2 - 1) * L * G / r**2)
    values_01 = four_minors(maps[("01", 3)])
    ...
    for key, matrix in maps.items():
        if key not in (("01", 3), ("23", 2)):
            assert four_minors(matrix) == {sp.Integer(0)}

    family_a = {X: 0, Y: 0}
    family_b = {Y: 0, Z: -(epsilon * q + 1) * X / (2 * q)}
    ...
    assert_equal(witness_a, -64 * Z**4 * q / (q - epsilon) ** 2)
    assert_equal(witness_b, 4 * X**4 * (q - epsilon) ** 2 / q**3)
    assert stack_a.rank() == 4
    assert stack_b.rank() == 4

    for matrix in maps.values():
        values = four_minors(matrix)
        assert all(
            sp.factor(value.subs(family_a)) == 0 for value in values
        )
        assert all(
            sp.factor(value.subs(family_b)) == 0 for value in values
        )
    ...
    endpoint_results[str(epsilon)] = {
        ...
        "individual_rank_safe_locus": "Y=0 and X*L=0",
        "survivor_A": "X=Y=0, Z*t!=0",
        "survivor_B": "Y=0, Z=-(e*q+1)*X/(2*q), X*t!=0",
...
payload = {
    ...
    "scope": "component 19 ordinary p=0,phi=+/-1 over Q(q), q*(q^2-1)!=0",
    "inputs": {COMPONENT.name: sha256(COMPONENT)},
    "method": (
        "fresh regular-basis permanents, direct finite/infinity incidence, "
    ...
    "candidate_claim": (
        "exactly two individual-rank survivor families remain for each phi "
        "endpoint on q*(q^2-1)!=0, and both have nonzero mode-one stacks"
    ),
    "endpoints": endpoint_results,
    "phi_endpoint_construction_artifacts_read_or_imported": False,
    "limitations": (
        "ordinary phi endpoints on q*(q^2-1)!=0 only; no q=0,+/-1, "
        "projectivized, valuative, closure, arbitrary-order, or global claim"
    ),
}
