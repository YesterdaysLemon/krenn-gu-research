...
flowchart TD
  ...
  GLS77 -->|Family-A r=2,3 / pure-zero triangle integrability| GL
...
| `GLS77` | remaining Family-A / source-integrability obligations | `GL` | The complete Family-A `r=1` key is empty.  Treat its `r=2,3` keys through their distinct row-plane configurations; do not assume the one-silent rank argument or five kernel differences transfer.  Retain every pure/zero-triangle, five-deficient, three-/four-deficient, unique-nonrigid, attachment, anchor, arbitrary-root, and local-to-global obligation. |
