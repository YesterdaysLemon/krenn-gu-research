...
flowchart BT
  ...
  N8R2C["n=8 maximum r=2; root block rank at least2<br/>COMMON-COORDINATE SOURCE EXCLUDED"]
  ...
  N8R2C -->|"other incidences and root-rank boundaries open"| GL
...
| `N8R2C` | At n=8 and maximum torus-root cardinality two, the common-coordinate physical root-incidence class is excluded whenever the chosen root block has rank at least two. The full source forces two ports per colour; all same-pair channel ranks, zero individual legs, coordinate-kernel rank-two boundaries, and dependent cross channels are retained. Labelled four-hafnians, partner stresses, the complete dependent-channel graph cover, four-cycle exclusion, and a six-cycle contradiction prove a **complete exclusion of this stated child**. Other incidence types, rank-at-most-one root blocks, general r=2, and the high-surplus parent remain open. | [Two-root common-coordinate exclusion](../claims/finite/n08/TWO_ROOT_COMMON_COORDINATE_EXCLUSION_THEOREM.md), [independent review](audits/TWO_ROOT_COMMON_COORDINATE_EXCLUSION_REVIEW_2026-09-04.md) |
...
| `M1` | full-source finite specialization | `N8R2C` | At n=8 and maximum r=2, retain common-coordinate physical root incidences and a root edge of rank at least two. The complete source labels three pure four-cofactors; nonlinear compatibility excludes this entire child, including rank-one channels, nonzero dependent channels, and four-cycle stress circulations. |
| `N8R2C` | boundary obligation | `GL` | Maximum-root saturation does not force common-coordinate physical incidences or a root edge of rank at least two. Other r=2 branches, higher root orders, and arbitrary n are not excluded by this theorem. |
