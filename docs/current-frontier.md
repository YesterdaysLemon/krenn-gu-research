...
flowchart TD
  ...
  S2AH -->|fully transverse correction-line / Type-I completion| S2AI
  S2T -. three physical component types remain .-> S2
  S2AI -. remaining rank-five boundaries / rank <=4 .-> S2
  S1 -->|boundary| S3
...
| `S2AH` | exact Type-I completion | `S2AI` | A coordinate-monomial block leaves two kernel-colour planes.  One has incompatible zero/target coefficients; the other sends every unrestricted-block correction to `T_1` against a rank-one `T_2` square, where full factor transversality upgrades the common-zero atlas. |
| `S2AI` | remaining common-three-space obligation | `S2` | The Type-II-only no-coordinate-monomial boundary-pencil tangent atlas in support-two `(2,2)`, the support-one transverse kernel, support-two `(3,3)` and `(3,2)` profiles, three Hilbert--Burch coordinate atlases, joint rank at most four, other S2T component types, other S2Q strata, and every higher order remain open. |
