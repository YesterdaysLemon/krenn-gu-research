...
flowchart BT
  ...
  N8R2C["n=8 maximum r=2; all root-block ranks<br/>COMMON-COORDINATE SOURCE EXCLUDED"]
  N8R2K["n=8 maximum r=2; invertible root edge<br/>ZERO OR COORDINATE-AXIS JOINT KERNEL REQUIRED"]
  N8R2S["n=8 paired invertible root stars<br/>COMMON PLANE + ZERO ONE-SHORE SOURCE REQUIRED"]
  N8R2E["n=8 max-r=2 invertible-edge graph<br/>ADJACENT DEGREE-FOUR VERTICES EXCLUDED"]
  ...
  N8R1["n=8, d=3 maximum root one<br/>ALL COMPLETE MATRIX-UNIT SOURCES EXCLUDED"]
  ...
  GLS61["Cross-product partial uncontraction<br/>PROVED arbitrary-r hierarchy<br/>r=3 all-injective branch EMPTY"]
  GLS62["Exactly-one-deficient row quotient<br/>PROVED r=3 branch EMPTY<br/>all-rigid deficient floor at least 2"]
  GLS63["Mixed-kernel partial uncontraction<br/>PROVED two-deficient localization<br/>binary same-kernel residual OPEN"]
  GLS64["Two-deficient matching integrability<br/>PROVED deficient kernel edge ZERO<br/>eta=0 residual OPEN"]
  GLS65["Eta-zero separated permanent source<br/>PROVED exact local ranks 2+2<br/>two-rank-three residual OPEN"]
  GLS66["Eta-zero scalar axis / common hyperplane<br/>PROVED 2233 EMPTY<br/>exactly-two-deficient branch EMPTY"]
  GLS67["Three-deficient dual P3 localization<br/>PROVED common-support orbits EMPTY<br/>432 profiles / 8 orbits OPEN"]
  GLS68["Four-deficient pair-class census<br/>PROVED 4,794 profiles / 50 keys OPEN<br/>direct n=6 bridge INVALID"]
  GLS69["Five-/six-deficient open-set tower<br/>PROVED support / face / span localization<br/>six residual 99,180 / 86 keys OPEN"]
  GLS70["Six-deficient binary-triangle parents<br/>PROVED two-family taxonomy / deck kernel<br/>S_c^2 T_c^4 EMPTY / residual 99,135 / 85"]
  GLS71["Single-binary strict-parent descent<br/>PROVED Family B r<=2 / Family A r=0 EMPTY<br/>residual 98,355 / 81"]
  GLS72["One-T_0 single-binary full-parent refinement<br/>PROVED Family A r=1 localized / transverse control<br/>residual unchanged 98,355 / 81"]
  GLS73["One-T_0 off-port-core full-source test<br/>PROVED arbitrary incident-edge completion EMPTY<br/>Family A r=1 still OPEN"]
  GLS74["All-T_0 Family-B full-parent boundary<br/>PROVED central-mixed-support nonendpoint EMPTY<br/>root-axis / endpoints OPEN"]
  GLS75["All-T_0 Family-B one-sided Koszul<br/>PROVED complete outside nonendpoint EMPTY<br/>P/Q-common endpoints OPEN"]
  ...
  GLD96["Equal-leaf H4 Q6 R31-free resultant<br/>PROVED localized F88 reduction"]
  ...
  N8PS["Protected two-K4 full source<br/>PROVED entire 96-parameter scaffold EXCLUDED"]
  PSMR["Protected scaffold minority cycles<br/>PROVED all-order resource formula; localization barrier"]
  PSFD["Protected scaffold fixed-depth source tests<br/>PROVED insufficient at every fixed minority depth"]
  PSCS["Protected common-star source coupling<br/>PROVED one-binary-resource exclusion"]
  PSRA["Common-star source-to-resource alignment<br/>REFUTED by an exact full-U4 control"]
  PSBS["Common-star binary resource supply<br/>REFUTED with all binary components noncomplete"]
  PSCL["Full binary induced-cycle localization<br/>PROVED; all-length face-only closure REFUTED"]
  PSCT["Two-sided binary cycle closure<br/>REFUTED with all binary singleton and double cuts"]
  PSCF["Full common-star physical source<br/>PROVED entire family EXCLUDED at every k at least 2"]
  PSCE["Full-source extension barriers<br/>PROVED AB and cut no-go; frozen crossing core"]
  PSEL["Protected-component contraction / Ward module<br/>PROVED identities and canonical-update barriers"]
  PSQ1["Paired-minority source / rooted circuits<br/>PROVED identities; SES and same-word descent blocked"]
  PSPR["Fixed paired-minority support parents<br/>REFUTED for every fixed cutoff; weighted WP1 OPEN"]
  PSCG["Protected crossing degree at most one<br/>PROVED macro normal form and full-source exclusion"]
  PSMD["Protected minimum crossing density<br/>PROVED full sources need more than 6k crossings"]
  MDW1F["Minimum-density WP1 at k=2 and k=3<br/>PROVED exhaustive finite exclusions"]
  PRDC["Paired resources on three protected matchings<br/>PROVED elementary mixed-word cycle obstruction"]
  PSDT["Degree-two source transport<br/>PROVED C8 repair, q1 escape and reciprocal local no-go"]
  RSTC["Resource-simple transition closure<br/>PROVED conditional mixed-cycle exclusion"]
  ...
  WB2 -->|stronger recursive support refinement at n=10| RZP10
  RZP10 -->|finite all-diagonal exclusion only; all-order occurrence open| GL
  P25 -->|conditional cut only; weighted source-isolation occurrence open| GL
  S3C -->|explicit two-slot annihilator specialization| P17
  P17 -->|complete template orbits do not cover Boolean parent| GL
  G0 -->|exact shared source slices| ST20
  ST20 -->|guard occurrence and parent coverage open| GL
  G0 -->|conditional protected scaffold at n=8| N8PS
  N8PS -->|larger orders retain exterior resources| PSMR
  G0 -->|conditional protected scaffold source identities| PSMR
  PSMR -->|full shared resource coupling remains open| GL
  PSMR -->|bounded-depth parent mechanism refuted| PSFD
  PSFD -->|complete pair allocation specialization| PSCG
  PSQ1 -->|crossing degree at most one plus singleton macro supply| PSCG
  PSCG -->|arbitrary arrays and low-q matching parent remain OPEN| GL
  PSQ1 -->|singleton supply plus macro and two-pair equations at equality| PSMD
  PSCG -->|complete pairing consumer after overlap exclusion| PSMD
  PSMD -->|denser protected sources and arbitrary-witness supply OPEN| GL
  PSMD -->|macro equality normal form and exhaustive low-q cover| MDW1F
  MDW1F -->|WP1 at higher order and denser supports OPEN| GL
  PRDC -->|elementary alternative and denser paired blocks| PSCG
  PRDC -->|import-free complete-pairing consumer| PSMD
  PRDC -->|degree-two source normalization and arbitrary-witness supply OPEN| GL
  G0 -->|conditional protected color-regular source rows| PSDT
  RSTC -->|paired blocks satisfy two-exit closure| PRDC
  PSDT -. global resource and closure supply OPEN .-> RSTC
  PSDT -->|coupled backgrounds and general degree-two distributions OPEN| GL
  RSTC -->|source-driven block or existential cycle supply OPEN| GL
  G0 -->|conditional protected color-regular all-split rows| ASSR
  ASSR -->|mixed transversal satisfying path criterion OPEN| GL
  ASSR -->|exact audit of one-switch supplier mechanism| ASOS
  ASOS -->|bidirectional K4 straddling or multi-switch closure OPEN| GL
  G0 -->|conditional common-star full source identity| PSCS
  PSCS -->|stronger tripartite supply tested with all U4 sources| PSRA
  PSRA -->|weaker binary supply tested with exact tensor control| PSBS
  PSBS -->|consumer survives but local supply is refuted| PSCS
  PSCS -->|complete binary targets localize every proper induced cycle| PSCL
  PSCL -->|disjoint resource shores force unique-port Hamilton cycle| PSCS
  PSCL -->|opposite face and binary low-order cuts still allow the defect| PSCT
  PSCT -->|weaker FB subsystem open; full-source consumer bypassed by PSCF| GL
  PSCL -->|internal-cycle FB subsystem open; not needed for PSCF| GL
  PSCS -->|weaker FB and even-k CSQ4 subsystems remain open| GL
  G0 -->|conditional common-star support and all physical AB-word targets| PSCF
  PSCF -->|arbitrary protected fillings and arbitrary-witness supply remain open| GL
  PSCF -->|arbitrary filling extension tested against exact controls| PSCE
  PSCE -->|full split-leaf cancellation and SFULL remain open| GL
  PSCE -->|full-source elimination parent tested with actual exterior core| PSEL
  PSEL -->|legal source-preserving elimination or uniform syzygy OPEN| GL
  PSMR -->|paired-minority paths and injective shared resources| PSQ1
  G0 -->|conditional protected-source support supplies all P1 premises| PSQ1
  PSQ1 -->|all-word weighted cross-color closure coupling OPEN| GL
  PSQ1 -->|support-only parent tested at every fixed pair depth| PSPR
  ...
  M1 -->|case r >= 2| M2
  M1 -->|case r = 1| U1
  M1 -->|"full source; n=8 common-coordinate specialization"| N8R2C
  N8R2C -->|"common-coordinate supply and other orders open"| GL
  M1 -->|"full source; n=8 invertible root specialization"| N8R2K
  N8R2C -->|"derived common-column orientation exception"| N8R2K
  N8R2K -->|"zero/axis joint kernels and lower root ranks open"| GL
  M1 -->|"full source; adjacent degree-four invertible centres"| N8R2S
  N8R2S -->|"arbitrary outside blocks and component supply open"| GL
  N8R2S -->|"complete 3/2/1 cover with physical AA/BB units"| N8R2E
  M1 -->|"two-invertible paths supply nonzero matrix units"| N8R2E
  N8R2E -->|"other invertible components and lower-rank sources open"| GL
  ...
  U1 -->|"complete n=8 scaffold and source certificate cover"| N8R1
  N8R1 -->|"every hypothetical ternary n=8 witness has r>=2"| M2
  ...
  GLS58 -->|complete cross-product partial uncontraction| GLS61
  GLS61 -->|exactly one deficient / row and active-line quotients| GLS62
  GLS62 -->|mixed-kernel parent attempt / exactly two deficient| GLS63
  GLS63 -->|complementary-edge integrability identity| GLS64
  GLS64 -->|eta-zero separated P4 extraction / local rank drop| GLS65
  GLS65 -->|scalar axis / common hyperplane annihilator| GLS66
  GLS66 -->|three-deficient pair classes / dual P3 extraction| GLS67
  GLS67 -->|four-deficient census / probe-dependent four-port test| GLS68
  GLS67 -->|eight three-deficient residual orbits / attachment open| GL
  GLS68 -->|four-deficient same-source pullback integrability| GL
  GLS68 -->|five-/six-deficient support tower / three-open span| GLS69
  GLS69 -->|binary-triangle first-parent / source-integrability test| GLS70
  GLS70 -->|single-binary strict-parent descent / attachment activity| GLS71
  GLS71 -->|one-T_0 activity and full-deck refinement| GLS72
  GLS72 -->|transverse coefficient / full-source completion test| GLS73
  GLS73 -->|coupled-correction boundary / complete kernel differences| GLS77
  GLS71 -->|all-T_0 full-parent Koszul descent| GLS74
  GLS74 -->|one-sided Koszul / central root-axis completion| GLS75
  ...
  GLD95 -->|R31-free E31/H2/g0 offset reduction into F88| GLD96
  GLD96 -->|p=2 symbolic-a six-minor closure without R31/E31/g0 localization| GLD97
  ...
  GLD93 -->|H4 Q6/e boundaries / pulled-back Fitting / other branches open| GLD94
  GLD94 -->|H4 Q6 boundary / pulled-back Fitting / other branches open| GL
  ...
  GLD95 -->|E31/Delta exceptional factors after GLD96/GLD99/GLD100, full H4 Q6 outside F88 / pulled-back Fitting / other branches open| GL
  GLD96 -->|E31/Delta exceptional loci after GLD100 / H2 degree-drop handled by GLD99 / p=2 fibre handled by GLD97 / Fitting / other branches open| GL
...
| `RZP10` | Every all-diagonal complex witness yields the stronger recursive zero-pattern model (RZP), which shares all principal-hafnian supports and forbids singleton cancellation in every Laplace expansion.  Thirteen matching-normalized CNFs exhaust six nonshared colour-0/1 matching-pair types and seven colour-2 types in the shared-pair case.  Exact binary DRAT replay and an independent no-import reconstruction of all CNF bytes, proof identities, and 945 matching transporters exclude every branch.  **Proved finite computer-assisted all-diagonal exclusion at `n=10`**; this does not prove AP' UNSAT at `n=10`, address bichromatic entries, or supply an all-order occurrence theorem | [Ten-vertex all-diagonal recursive-hafnian exclusion](../claims/finite/n10/TEN_VERTEX_ALL_DIAGONAL_RECURSIVE_HAFNIAN_EXCLUSION_THEOREM.md), [evidence manifest](strategy/recursive-cancellation-consistency-evidence-2026-09-12.json) |
| `P25` | At `n=8`, eighteen named zero physical entries and seven named nonzero entries force a coefficient-one monomial into the ideal of four mixed full hafnians by a division-free integer identity; two entries required by the older source-core projection are arbitrary.  **Proved conditional physical obstruction over every field.**  The cumulative recursive/killer/ratio parent model remains SAT after every allowed image of this 25-literal cut and the three older cuts is forbidden, with the same checked 134-entry physical projection.  Thus the support-only `P8-25-occurrence` implication is false in that Boolean abstraction; this is not a weighted witness or an all-order result | [25-literal physical-hafnian identity](../claims/finite/n08/EIGHT_VERTEX_25_LITERAL_PHYSICAL_HAFNIAN_IDENTITY.md), [review follow-up](audits/CANCELLATION_PHYSICAL_PRO6_REVIEW_FOLLOWUP_2026-09-13.md) |
| `P17` | **Proved physical-source exclusion over every field:** a five-shore with two covering edges supplies a four-equation degree-six identity using fifteen zero and two nonzero entries. It excludes the preserved P25-surviving 134-entry support. Disjoint/shared-centre and equal/distinct alternate-colour types are exactly replayed independently. Their complete orbits still admit a different checked Boolean support; that survivor is excluded by ST20. The full declared degree-five module on the original support has no monomial certificate. This is an explicit specialization of the existing majority principle, not unrestricted n=8 exclusion. | [Two-edge shore theorem](../claims/finite/n08/EIGHT_VERTEX_TWO_EDGE_SURPLUS_SHORE_EXCLUSION.md), [degree-five limitation](../claims/finite/n08/EIGHT_VERTEX_DEGREE_FIVE_SOURCE_MODULE_LIMITATION.md) |
| `ST20` | **Proved conditional source transfer for every even n>=4 over every field:** two full colour slices share one actual bulk hafnian; opposite alternating contractions eliminate it without dividing by any cofactor or determinant. The written 4(n-4)+2 zero and two nonzero premises give an obstruction; at n=8 this is an independently replayed twenty-literal, eight-equation certificate excluding the P17-orbit survivor. All four ternary colour-equality types are checked. The combined twelve-orbit Boolean parent still has a fully clause-replayed 129-entry support; its pure-attached degree-six rows have no monomial certificate or lower-term coupling. No occurrence theorem, exhaustive parent cover, or all-order resolution is asserted. | [Shared-hafnian transfer theorem](../claims/arbitrary-order/TWO_SLICE_SHARED_HAFNIAN_TRANSFER_OBSTRUCTION.md), [resolution attempt](strategy/source-module-resolution-attempt-2026-09-14.md) |
| `N8PS` | **Proved finite exclusion over C:** two fixed unit K4 colour scaffolds with all 96 hollow crossing entries arbitrary cannot realize the full ternary target. All 6,561 source equations yield one necessary CNF; a portable 1,521-clause core and 108 exact RUP steps refute it. Independent word-first reconstruction and independent propagation reproduce and check the proof. Three exact Laurent controls refute the tested proper subsystems. This is neither unrestricted n=8 exclusion nor a larger-order theorem. | [Complete protected-scaffold exclusion](../claims/finite/n08/EIGHT_VERTEX_PROTECTED_SCAFFOLD_EXCLUSION.md) |
| `PSMR` | **Proved all-order protected-scaffold identities over C:** independent minorities give an exact permanent and an equivalent forbidden directed-cycle condition. General boundary words require an injection-labelled hafnian expansion with each exterior matching edge usable once. An exact n=12 control satisfies the entire localized subsystem but changes a mixed coefficient from 1 to 0 through an exterior component; a separate n=8 Laurent control also satisfies every component-constant target. Literal two-component localization and sufficiency of those proper subsystems are refuted. Full resource coupling remains open. | [Minority cycles and resource boundary](../claims/arbitrary-order/PROTECTED_SCAFFOLD_MINORITY_CYCLE_AND_RESOURCE_BOUNDARY.md), [independent review](audits/PROTECTED_SCAFFOLD_SOURCE_REVIEW_2026-09-14.md) |
| `PSFD` | **Proved constructive no-go over C:** for every fixed minority depth r there is a finite protected scaffold with entries 0,+1,-1 satisfying every full source equation whose word differs from some constant at at most r vertices, while an explicit mixed word has amplitude 1 instead of 0. A labelled component graph of girth greater than r forces all relevant matching terms through exact cancelling pair gadgets; a finite reduced-word permutation cover supplies arbitrary depth and preserves the explicit failure. The n=248 base already works through depth five. This refutes bounded-depth source exclusion uniformly in order, not Krenn-Gu or a depth growing with order. All component-constant targets together with depth-four mixed equations remain an open sharper parent. | [Fixed minority-depth no-go](../claims/arbitrary-order/PROTECTED_SCAFFOLD_FIXED_MINORITY_DEPTH_NO_GO.md), [independent review](audits/PROTECTED_SCAFFOLD_FIXED_DEPTH_REVIEW_2026-09-14.md) |
| `PSCS` | **Proved all-order source obstructions over C:** common-centre gadgets, with one unequal-colour label per component edge and two nonzero factors per gadget, obey an exact shared-neighbour identity retaining both hafnians and four-cycle interference. Singleton and both double-component targets exclude every unique port. The full component target is impossible whenever just one binary state graph is a union of complete bicliques: singleton sums balance the blocks, double rows exclude size one, and a directed binary-resource cycle gives a mixed word with nonzero star-factor product. This excludes the family in which the ports for just one unordered color pair all have size at most two, with the other degrees unrestricted. Earlier complete-tripartite and all-port-at-most-two results remain valid. Both proposed S1+S2+U4 supplies are refuted at PSRA/PSBS; the full binary-source parent FB, arbitrary protected fillings and Q4 remain open. | [Common-star source obstruction](../claims/arbitrary-order/PROTECTED_SCAFFOLD_COMMON_STAR_COMPONENT_TARGET_OBSTRUCTION.md), [two-port review](audits/PROTECTED_SCAFFOLD_COMMON_STAR_TWO_PORT_REVIEW_2026-09-14.md), [binary-resource review](audits/COMMON_STAR_RESOURCE_ALIGNMENT_REVIEW_2026-09-14.md), [parent attempt](strategy/common-star-q4-parent-attempt-2026-09-14.md) |
| `PSRA` | **Proved exact mechanism no-go over Q(sqrt(3)):** S1, both S2 families and every full physical four-minority source do not force the common-star state graph to split into complete tripartite resources. A finite SL(2,F5) realization has 720 protected K4 components, 10080 legal gadgets and 120 connected noncomplete state resources. The exact local identities assemble through unique state-resource membership; an exhaustive crossing-cycle classification and all 8640 coherent component triangles prove full U4. The literal 2880-vertex scalar failure has source 2^(-360), so this is not a CSQ4 or global counterexample. Each binary graph is nevertheless a biclique union; the weaker supply left open by this control is subsequently refuted at PSBS. | [Resource-alignment no-go](../claims/arbitrary-order/PROTECTED_SCAFFOLD_COMMON_STAR_RESOURCE_ALIGNMENT_NO_GO.md), [independent review](audits/COMMON_STAR_RESOURCE_ALIGNMENT_REVIEW_2026-09-14.md), [parent attempt](strategy/common-star-q4-parent-attempt-2026-09-14.md) |
| `PSBS` | **Proved exact mechanism no-go over Q(sqrt(3)):** S1, both S2 and the complete physical U4 system do not force even one binary color-pair graph to be a union of complete bicliques. Overlapping inverse-relative matrix products and a signed tensor construction produce a legal 6000-component common-star array with 312480 gadgets. All 423360 component triangles are coherent; exact factor identities and the audited source bridge prove the full antecedent. Every connected binary support component is noncomplete. The literal 24000-vertex mixed word has coefficient (-463/2592)^600, checked using 600 scalar cores and 6000 protected edges, so this is not a CSQ4 or global counterexample. The PSCS consumer survives; any source-to-structure route must use more than S1+S2+U4 or a different implication. | [Binary-supply no-go](../claims/arbitrary-order/PROTECTED_SCAFFOLD_COMMON_STAR_BINARY_SUPPLY_NO_GO.md), [independent review](audits/COMMON_STAR_BINARY_SUPPLY_REVIEW_2026-09-14.md), [parent handoff](strategy/common-star-q4-parent-attempt-2026-09-14.md) |
| `PSCL` | **Proved full-cut reduction over C:** every proper induced directed cycle for a color pair has both states of all its physical components in one connected binary resource. The complete subset targets fix every proper exterior contraction, while the full-cycle source must vanish. If each resource has disjoint physical shores, the full binary target is exactly a directed Hamilton cycle with products -1; PSCS's three-color S2 excludes its singleton ports, without any completeness assumption. An exact algebraic family on 2r-1 components realizes the surviving one-sided cycle face for every r>=3 but fails an exterior singleton target. Thus face-only closure is refuted at every cycle length; simultaneous exterior-varying targets, opposite orientations and third-color coupling remain open. This does not prove FB, CSQ4 or arbitrary-witness coverage. | [Cycle localization and all-length control](../claims/arbitrary-order/PROTECTED_SCAFFOLD_FULL_BINARY_CYCLE_LOCALIZATION.md), [independent review](audits/COMMON_STAR_BINARY_CYCLE_LOCALIZATION_REVIEW_2026-09-20.md), [resumed parent attempt](strategy/full-binary-parent-attempt-2026-09-20.md) |
| `PSCT` | **Proved exact mechanism no-go over algebraic C:** a nine-component, 27-gadget actual common-star array satisfies both localized induced-three-cycle faces and all 90 binary singleton and same-color double cuts in both orientations. The full binary word `001011111` nevertheless has source (r^2+1)/(12r), proved nonzero by its parameter norm. A literal 36-vertex matching audit and independent triangular reduction validate the construction. Thus adding both faces and this binary low-order subsystem cannot close PSCL. The array has no third-color supply and does not refute the sharper three-color antecedent, FB, CSQ4 or Krenn--Gu. Higher exterior-varying cuts or third-color coupling remain load-bearing. | [Two-sided cycle closure no-go](../claims/arbitrary-order/PROTECTED_SCAFFOLD_TWO_SIDED_BINARY_CYCLE_NO_GO.md), [independent review](audits/COMMON_STAR_TWO_SIDED_CYCLE_REVIEW_2026-09-20.md), [parent attempt](strategy/full-binary-parent-attempt-2026-09-20.md) |
| `PSCF` | **Proved complete common-star full-source exclusion over C:** for every k>=2, independently colored centers and uniform leaf triples have the exact operator source `sum_(S even) a_S b_S^T tensor I_(V outside S)`. Locally orthogonal contractions kill every proper crossing set. For odd k the source is zero but the target is `2-2^k`; component-constant words already suffice. For even k the surviving source is one outer product, while four explicit probes give target determinant `1+2^(k-1)`, which is nonzero. Thus the entire common-star family fails the full physical target, bypassing FB and CSQ4 as proposed intermediates. Even-k CSQ4 and FB are not proved by their weaker antecedents; arbitrary protected fillings and arbitrary-source supply remain open. The k=1 K4 remains the exact sharp control. | [Full-source exclusion](../claims/arbitrary-order/PROTECTED_SCAFFOLD_COMMON_STAR_FULL_SOURCE_EXCLUSION.md), [independent review](audits/COMMON_STAR_FULL_SOURCE_EXCLUSION_REVIEW_2026-09-20.md), [parent extension](strategy/exterior-coupling-parent-resumption-2026-09-20.md) |
| `PSCE` | **Proved extension-route barriers over C:** two six-parameter n=8 protected controls satisfy every AB word and give the rank-two target, yet have explicit unique nonzero split-leaf sources. Thus arbitrary hollow fillings defeat AB-only extension of PSCF. If every intercomponent block is invertible, protected matching connectivity excludes every target-visible cut-null separator; this holds on explicit rational nonwitnesses at every k>=2 and a nonempty open of the accepted k>=5 generic scaffold. Protected diagonal nonnegative degenerations also freeze every component-constant matching term through any finite sequence; such targets require a nonempty frozen crossing core, and all twelve entries of each n=8 control are frozen. These facts do not refute full-source separator supply or prove SFULL. The missing implication must use shared-factor consistency with nonuniform leaf columns. | [Extension barriers](../claims/arbitrary-order/PROTECTED_SCAFFOLD_FULL_SOURCE_EXTENSION_BARRIERS.md), [independent review](audits/PROTECTED_FULL_SOURCE_EXTENSION_REVIEW_2026-09-20.md), [parent attempt](strategy/protected-full-source-parent-attempt-2026-09-20.md) |
| `PSEL` | **Exact all-order contraction and method boundaries over C:** contracting one protected K4 produces `K=3+K_2+K_4` coupled to the actual exterior matching map. A legal quadratic correction gives elimination exactly when `Phi_Q(exp(Delta)-K)=0`. Exact Laurent controls refute canonical quadratic updates and fibrewise gluing; local-word multidegrees obstruct direct repeated-leg polarization. Covariant Ward transport is valid, but its unrestricted pointwise module already tests the desired missing top-response condition. A uniform or fully stratified source syzygy remains open. The k>=3 existential elimination parent is not refuted by k=2 nonwitness controls; k=1 remains a valid source. | [Contraction and Ward boundary](../claims/arbitrary-order/PROTECTED_COMPONENT_ELIMINATION_AND_WARD_BOUNDARY.md), [independent review](audits/PROTECTED_FULL_SOURCE_PARENT_REVIEW_2026-09-22.md), [review handoff](strategy/full-source-parent-review-2026-09-22.md) |
| `PSQ1` | **Exact paired-minority source identities and repair boundaries over C:** q=1 coefficients are shared two-path permanent minors contracted with actual closure seams; q=2 requires subtracting repeated-resource hafnians. A protected-seam factor identity needs a nonzero supplier. Under PSMR, every q=1 matching forms one rooted alternating circuit, so same-word protected-pair-saturated exchange cannot reduce its minority set. An independently enumerated 22-entry k=3 support covers every macro word with three whole DAGs, refuting SES; it has 56 unique q=1 words and is not P1. The later PSPR theorem refutes P1 exclusion and every fixed-cutoff support version. The weighted identities remain valid; full weighted source exclusion and arbitrary-witness scaffold supply remain open. | [Paired-minority source and repair boundary](../claims/arbitrary-order/PROTECTED_PAIRED_MINORITY_SOURCE_AND_REPAIR_BOUNDARY.md), [checkpoint review](audits/PROTECTED_FULL_SOURCE_PARENT_REVIEW_2026-09-22.md), [parent countermodels](../claims/arbitrary-order/PROTECTED_LOW_MINORITY_SUPPORT_COUNTERMODELS.md) |
| `PSPR` | **Proved fixed-cutoff support-parent refutation:** for every fixed r>=1 there is a finite simple one-label common-star support covering all mixed macro words and admitting no uniquely supported mixed word with min_c q_c<=r; all whole D_c are DAGs. The proof bounds active components by 3q and combines a forest projection with a self-contained high-girth macro-rigidity existence proof using 48 matching layers. Supports depend on r; no finite all-r or unrestricted UPM control is asserted. P1 has a separate exact 18-K4 certificate checked through 729 left assignments. Every triangle-free control is incompatible with weighted macro targets by the accepted PSCS identity. Conversely a literal 72-entry rational k=3 array satisfies every weighted macro target with whole DAGs but has a q1 coefficient -1. WP1, retaining both macro and low-q weighted equations, remains open; on common-star arrays it is exactly the existing CSQ4 parent. | [Countermodel theorem and weighted boundary](../claims/arbitrary-order/PROTECTED_LOW_MINORITY_SUPPORT_COUNTERMODELS.md), [independent review](audits/PROTECTED_LOW_MINORITY_PARENT_REVIEW_2026-09-22.md), [review brief](strategy/low-minority-parent-review-2026-09-22.md) |
| `PSCG` | **Proved protected sparse-crossing exclusion over C:** complete protected-edge pairings with arbitrary nonzero weights have a state-C4 / simple triangle-free cubic-graph correspondence. The imported three-perfect-matchings classification forces a mixed unique-matching word; PRDC now gives an elementary alternative with no imported matching theorem. More generally, at most one crossing per vertex/color state plus singleton macro support forces this complete pairing: the two backgrounds consume the two protected edges in distinct C4s. Actual singleton zero equations force gadget products -1. Thus every protected array with state crossing degree at most one fails the full source. No component bipartition is required. In the product -1 case auxiliary matchings biject with all nonzero coefficients, but no paired-depth bound is proved; WP1 is reduced to the balanced C4 case and remains open. Arbitrary protected arrays need not satisfy the degree bound. | [Sparse-crossing and cubic matching exclusion](../claims/arbitrary-order/PROTECTED_PAIR_GADGET_CUBIC_MATCHING_EXCLUSION.md), [independent review](audits/PROTECTED_PAIR_GADGET_CUBIC_REVIEW_2026-09-22.md), [review brief](strategy/pair-gadget-cubic-review-2026-09-22.md) |
| `PSMD` | **Proved strict protected crossing-density bound over C:** for every k>=2, macro equations force at least 6k supported crossing scalar entries. At equality they form 3k protected-pair gadgets of product -1. Macro rigidity forces each color-pair permutation to be one cycle and forbids literal triangles by an explicit mixed-transversal arc word. Two-exception macros localize every overlapping resource to one foreign component. Its misaligned orientation has a unique q1 word; its aligned orientation has a unique word with depth vector (2,2k-1,2k-2). Thus WP2 forces all allocations split at equality, and PSCG excludes the resulting complete pairing from the full source. PRDC supplies an elementary alternative to its imported matching-classification step. Every full protected source must therefore have more than 6k crossings. WP1 and WP2 are not themselves excluded at general order; the final cubic word has no depth bound. | [Minimum-density theorem](../claims/arbitrary-order/PROTECTED_MINIMUM_CROSSING_DENSITY_EXCLUSION.md), [independent review](audits/PROTECTED_MINIMUM_CROSSING_DENSITY_REVIEW_2026-09-22.md), [review brief](strategy/minimum-crossing-density-review-2026-09-22.md) |
| `MDW1F` | **Proved finite WP1 exclusion at minimum crossing density:** on protected unit-K4 complex arrays, WP1 (all macro targets and mixed zeros with min q<=1) forces more than 6k crossings for k=2 and k=3. PSMD supplies the macro-only equality gadget normal form. The k=2 cover has all 262,144 supports; the k=3 topology, gauge and analytic gates leave 32,768 supports. Every residual support has a low-q mixed word with a unique matching for arbitrary nonzero weights. Independent checkers reconstruct the complete covers from pinned certificates. General k>=4 WP1, denser supports and arbitrary-witness supply remain open. | [Finite proofs and certificates](../claims/arbitrary-order/minimum-density-wp1/README.md), [independent audit](../claims/arbitrary-order/minimum-density-wp1/AUDIT.md) |
| `PRDC` | **Proved paired-resource exclusion over C:** take any three nonzero-weight protected perfect matchings on the same finite physical set and partition all colored resources into different-color pairs. Crossings occur only inside these blocks, without physical selfloops, and each block crossing support contains a perfect matching. Resources may overlap physically and blocks may have two, three or four crossings. A directed cycle in the mate-to-foreign-state graph changes a pure word so every active block selects exactly one edge. Proper cycles are mixed; spanning cycles have half of each foreign color by resource counts. This gives a mixed unique matching with no external theorem, K4 scaffold, unit weight or cancellation assumption. It provides an elementary PSCG/PSMD consumer but no general degree-two normal form or minority-depth bound. | [Directed-cycle proof](../claims/arbitrary-order/PAIRED_RESOURCE_DIRECTED_CYCLE_EXCLUSION.md), [independent review](audits/PAIRED_RESOURCE_CYCLE_REVIEW_2026-09-22.md), [review brief](strategy/paired-resource-cycle-review-2026-09-22.md) |
| `PSDT` | **Proved color-regular source transport over C:** singleton cuts retain all two- and four-crossing terms. A C8 singleton forces the opposite foreground layer into two C4 resources of product -1, including spanning targets. The forced relations repair the whole 81-word local tensor. A reciprocal k=2 control satisfies all 486 one-component/background specifications on both shores (477 distinct words) but fails 195 full-source rows. A new q1 row forces at least one split resource to escape a one-component C8 target; escape does not automatically iterate. Macro controls refute deletion of four-crossing sectors or macro-invisible source-live entries. A separate actual k=3 control satisfies the complete 81-row tensor while both foreground layers retain two orphan targets; local split/C8 classification is therefore not exhaustive. A k=4 control passes all 243 fixed-component/background specifications with orphans. If both orphan targets lie in one component, a coupled row forces the complementary third-color edge to route into a distinct protected resource with product -1. This is a necessary repair relation, not an exclusion; separated targets and global closure remain open. The full color-regular and degree-two parents remain open, including partial-resource and (2,0) distributions. | [Source transport](../claims/arbitrary-order/DEGREE_TWO_SOURCE_TRANSPORT_AND_LOCAL_BOUNDARY.md), [independent review](audits/DEGREE_TWO_SOURCE_TRANSPORT_REVIEW_2026-09-22.md), [parent brief](strategy/degree-two-source-parent-review-2026-09-22.md) |
| `RSTC` | **Proved conditional resource-simple block exclusion over C:** three nonzero-weight protected matchings, at most one resource per color in each block, only within-block crossings, endpoint supply and two-exit closure. Directed cycles select one edge per active block; reverse-transition symmetry on three colors supplies a mixed cycle. This extends PRDC to triple blocks including degree-two K3,3 blocks. The exact cyclewise condition only asks adjacency at double-hit resources, but a hollow k=3 degree-one fixture has no good cycle in any base color, despite 317 unique mixed words elsewhere. Thus endpoint supply cannot provide even the weaker existential cycle route. A color-regular prism that locally zeros all three binary faces has full local factor -2; another block may still zero the global row, so global isolation remains open. No arbitrary-source block or closure supplier is proved. | [Block theorem and exact route control](../claims/arbitrary-order/RESOURCE_SIMPLE_TRANSITION_CLOSED_EXCLUSION.md), [independent review](audits/RESOURCE_SIMPLE_TRANSITION_REVIEW_2026-09-22.md), [parent brief](strategy/degree-two-source-parent-review-2026-09-22.md) |
...
| `N8R2C` | At n=8 and maximum torus-root cardinality two, the common-coordinate physical root-incidence class is **completely excluded, without a root-block rank hypothesis**. Rank-at-least-two, both nonmonomial rank-one factor types, and the zero block are covered, retaining zero individual legs and all dependent-channel boundaries. The zero-block proof keeps the initially unobserved hafnian, closes an exhaustive orientation cover, and re-roots the final pure case into the accepted high-rank theorem before an independently reconstructed and checked Boolean certificate. The certificate has 6,860 binary branches and 6,861 conflict leaves. Other incidence types, general r=2, larger orders, and the high-surplus parent remain open. | [Complete common-coordinate exclusion](../claims/finite/n08/TWO_ROOT_COMMON_COORDINATE_COMPLETION_THEOREM.md), [integration review](audits/TWO_ROOT_COMMON_COORDINATE_COMPLETION_REVIEW_2026-09-04.md), [certificate review](audits/TWO_ROOT_ZERO_SOURCE_CERTIFICATE_REVIEW_2026-09-04.md) |
| `N8R2K` | At n=8 and maximum torus-root cardinality two, every invertible physical edge must have an outsider whose two root incidences have joint kernel zero or a coordinate axis. The full source excludes the complementary case where every joint kernel contains a vector with exactly one zero coordinate: it supplies one genuine binary common-column anchor per colour pair and closes the actual mixed 2D/1D cofactor system. This is pointwise analytic proof, using N8R2C only after deriving its full incidence premise in the orientation exception. Zero/axis kernels, lower-rank root blocks, and the global parent remain open. | [Invertible-root joint-kernel boundary](../claims/finite/n08/TWO_ROOT_JOINT_KERNEL_BOUNDARY_THEOREM.md), [independent review](audits/TWO_ROOT_JOINT_KERNEL_BOUNDARY_REVIEW_2026-09-04.md), [parent attempt](strategy/astra-root-supply-parent-2026-09-04.md) |
| `N8R2S` | In the eight-vertex full source, an invertible root edge with three invertible A legs, three invertible B legs, and their six complementary diagonal unit legs forces at least one triple of inactive image planes to coincide. On every coincident shore, the outside hafnian vanishes with the opposite shore fully open and the corrected physical cofactor matrix equation holds. The mixed-normal identity excludes all nonzero-inactive-determinant cases for every kappa; the common-plane argument retains every normal support and both simultaneous coincidences. Outside blocks are arbitrary. At max-r=2, adjacent degree-four invertible-edge vertices supply this architecture. For arbitrary outside blocks the cofactor system and architecture supply remain open; N8R2E closes the specialization with nonzero physical AA/BB units: **PROVED SOURCE REDUCTION**, not architecture or global exclusion. | [Diagonal-root-leg common-plane reduction](../claims/finite/n08/TWO_ROOT_DIAGONAL_LEG_COMMON_PLANE_REDUCTION_THEOREM.md), [independent review](audits/TWO_ROOT_DIAGONAL_LEG_COMMON_PLANE_REDUCTION_REVIEW_2026-09-04.md), [parent attempt](strategy/astra-source-residual-parent-2026-09-04.md) |
| `N8R2E` | The entire eight-vertex diagonal-root-leg source configuration is excluded when its six full AA/BB blocks are nonzero matrix units. The opposite planes and arbitrary AB blocks are retained. A single uniform common-vector/torus-annihilator contradiction joins the exhaustive normal-support cover 3/2/1; the final two-coordinate branch uses the missing colour mixed-normal equation with its AB-normal rows intact. In an n=8/max-r=2 witness, invertible two-edge paths supply the units, so adjacent degree-four vertices in the invertible-edge graph are impossible. Other components, lower-rank graphs, and the global conjecture remain open: **PROVED PARENT EXCLUSION**, at the stated finite scope. | [Complete diagonal-leg source exclusion](../claims/finite/n08/TWO_ROOT_DIAGONAL_LEG_SOURCE_EXCLUSION_THEOREM.md), [independent integration review](audits/TWO_ROOT_DIAGONAL_LEG_SOURCE_EXCLUSION_REVIEW_2026-09-04.md), [parent attempt](strategy/astra-common-plane-parent-2026-09-04.md) |
...
| `U1` | Complete nonzero one-matrix-unit blocks and forbidden-word cancellation: **proved normal form; arbitrary-order exclusion open**. N8R1 closes the entire finite n=8 specialization. | [Maximal-root one branch](../claims/arbitrary-order/MAXIMAL_TORUS_ROOT_SATURATION_AND_COORDINATE_ABSORPTION_THEOREM.md#3-the-maximum-one-monomial-branch) |
| `N8R1` | Every complete nonzero ternary matrix-unit source on eight vertices is excluded for arbitrary complex nonzero weights and endpoint labels. Three selected pure matchings supply a complete cover of 1,884 ordered scaffold pairs in eighteen vertex-stabilizer orbits. Necessary matching-word collision clauses plus 39 exact odd-integer-relation cuts give eighteen final CNFs, all supplied with independently accepted DRAT proofs. Independent reconstruction checks the entire cover and every clause; portable generation and compressed proof bytes are pinned. Thus every hypothetical ternary n=8 witness has maximum root cardinality at least two. Other root orders and arbitrary n remain open: **PROVED COMPLETE n=8/r=1 EXCLUSION**, not global resolution. | [Eight-vertex matrix-unit exclusion](../claims/finite/n08/EIGHT_VERTEX_MATRIX_UNIT_EXCLUSION_THEOREM.md), [certificate package](../claims/finite/n08/r1-source-certificate/README.md), [integration review](audits/EIGHT_VERTEX_MATRIX_UNIT_INTEGRATION_REVIEW_2026-09-04.md) |
...
| `GLD94` | On the complete normalized H4 chart, the retained `e=0` boundary is parameterized exactly by `q=t` and `p=(t^2+2t-2)/(2t^2-2t-1)`, with the denominator quadratic coprime to the numerator.  After retaining the collision values for GLD87/GLD89, the old and alternate six-pivots either force the explicit `a,b` family or both vanish.  The simultaneous branch is covered by two auxiliary six-pivots whose exact resultant forces a residual curve that restricts to a nonzero product on `e=0`; the forced branch has three all-block kernel vectors and a direct nonzero block minor, so every compatible centre is singular.  The unsaturated leaf family is nonempty (sample `(13/11,3,8/11,13/9,0)` with leaf determinant `24/11`) but is outside `D(Omega)`.  This closes `B intersect V(I_7(A)) intersect D(Omega) intersect V(e)` after the named GLD87/GLD89 cases, not the GLD83 Fitting ideal, `Q6` or other boundaries/charts/ranks/components, source branches, or global conjecture. | [H4 e=0 determinant safety](../claims/arbitrary-order/FOUR_ROOT_TORUS_STAR_EQUAL_LEAF_H4_E_BOUNDARY_DETERMINANT_SAFETY_THEOREM.md), [hostile review](audits/FOUR_ROOT_TORUS_STAR_EQUAL_LEAF_H4_E_BOUNDARY_DETERMINANT_SAFETY_REVIEW_2026-08-27.md) |
 | `GLD95` | On the written GLD88 rational family `F88` over `D(Delta)`, the exact resultant/content decomposition of the GLD92 six-minors exhausts every finite common-minor component.  Generic `H0,H1,H3,H4,H5,H6` branches have unit six-minors; the true `H2=LC_q(Q6)` fibre has direct gcd `d0`; and all five retained p-content fibres have exact unit six-minors.  Every one of those five content components has old `P6=0`, so the new witnesses explicitly extend across that old pivot boundary.  Thus `V(Q6,F28,F31)` is empty on `D(Delta)` inside `F88`, and the H4 Q6 finite residual is closed only on this written family.  Arbitrary H4 Q6 points outside `F88`, the GLD83 Fitting pullback, other charts/components/source branches, and global resolution remain open. | [H4 Q6 finite common-minor exclusion](../claims/arbitrary-order/FOUR_ROOT_TORUS_STAR_EQUAL_LEAF_H4_Q6_FINITE_COMMON_MINOR_EXCLUSION_THEOREM.md), [hostile review](audits/FOUR_ROOT_TORUS_STAR_EQUAL_LEAF_H4_Q6_FINITE_COMMON_MINOR_EXCLUSION_REVIEW_2026-08-27.md) |
| `GLD96` | In the normalized H4 offset chart on `V(Q6) intersect D(E31 H2 g0 Delta)`, four exact raw bordered seven-minors force the GLD88 offsets `B=C=0`; GLD95 then excludes the written F88 endpoint on `D(Omega)`.  The polynomial adjugate identity uses no inverse of `R31`, so `R31=0` is included wherever the remaining gates are nonzero.  GLD99 handles the normalized `H2=Q6=0` degree-drop branch, and GLD100 removes the normalized `g0` gate on the remaining `H2`-open; the combined normalized route therefore retains `E31=0` and `Delta=0` before its physical and wider-chart boundaries. | [H4 Q6 R31-free generic resultant localization](../claims/arbitrary-order/FOUR_ROOT_TORUS_STAR_EQUAL_LEAF_H4_Q6_R31_GENERIC_RESULTANT_EXCLUSION_THEOREM.md), [gate-removal review](audits/FOUR_ROOT_TORUS_STAR_EQUAL_LEAF_H4_Q6_R31_GATE_REMOVAL_REVIEW_2026-08-29.md) |
...
| `GLS61` | Return to the complete zero-anchor `GLS8` tensor before fully contracting the auxiliary labels.  For every open set `S`, cross-product contraction retains structurally exactly the companions indexed by `binom(S,2)` and gives one compatible same-source tensor hierarchy.  One-open equations force every target colour to have at least two identically zero cross coordinates.  At `r=3`, six injective nonaxis labels would give an exact `2+2+2` partition; a two-open equation then forces one physical companion to be pure `(c,c)`, contradicting each injective shore orientation.  For any nonempty pure-axis set `P`, quotient every pure slot by its active full-row line.  The `S=P` equation forces three disjoint nonaxis zero sets, hence `|P|<=3`; a singleton zero and the `S=P union {u}` equation then have a surviving pure target but a completely killed source side.  Thus the entire all-injective `r=3` branch is empty, and every all-six-rigid witness has a deficient joint map.  The arbitrary-root hierarchy is proved, but the branch exclusion is root-order-three only.  All deficient-map branches, the unique-nonrigid branch, responses, selectors, synchronization/activity, anchors, arbitrary-root attachment, node closure, and global resolution remain open. | [Cross-product partial uncontraction and all-injective exclusion](../claims/arbitrary-order/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_CROSS_PRODUCT_PARTIAL_UNCONTRACTION_AND_INJECTIVE_NONAXIS_EXCLUSION_THEOREM.md), [hostile review](audits/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_CROSS_PRODUCT_PARTIAL_UNCONTRACTION_AND_INJECTIVE_NONAXIS_EXCLUSION_REVIEW_2026-08-28.md) |
| `GLS62` | On the all-six-rigid zero-anchor `r=3` branch, assume exactly one joint probe map `J_n` is deficient.  In the `GLS61` same-source hierarchy, quotient the open `n`-slot by `row J_n` and every injective pure-axis slot by its active full-row line.  Every deficient-kernel-supported colour then needs at least two nonaxis zero labels; equality two would force the corresponding nonaxis pair companion to be pure `(c,c)`, contradicting the exact GLS61 orientation obstruction.  Hence it needs at least three.  Disjointness forces `rank J_n=2` with a pure-coordinate kernel and at most two pure-axis labels.  The remaining zero-, one-, and two-axis cases are excluded by, respectively, the exact `3+1+1` singleton companion obstruction and active-line quotient equations with a surviving target.  Thus the exactly-one-deficient cell is empty and every all-six-rigid witness has at least two deficient maps.  Their common one-/two-/higher-kernel deck compatibility, response, selector, synchronization/activity, anchors, arbitrary-root attachment, node closure, and global resolution remain open. | [Exactly-one-deficient row-quotient exclusion](../claims/arbitrary-order/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_EXACTLY_ONE_DEFICIENT_ROW_QUOTIENT_EXCLUSION_THEOREM.md), [hostile review](audits/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_EXACTLY_ONE_DEFICIENT_ROW_QUOTIENT_EXCLUSION_REVIEW_2026-08-28.md) |
| `GLS63` | For any all-rigid deficient set, independently contract selected deficient labels at generic vectors of their whole joint kernels and selected injective nonaxis labels at their cross products, leaving every pure axis open.  This gives one complete same-source partial-uncontraction hierarchy.  A colour visible on every deficient kernel needs at least three disjoint nonaxis zero labels; a singleton zero visible on all but one deficient kernel forces that remaining map to be a rank-one coordinate readout.  For exactly two deficient labels, the support/rank census and a two-dimensional pure-companion no-go exclude every profile except two rank-two maps with the same pure-coordinate kernel, no pure axes, three or four nonaxis zeros on that colour, no complementary-colour zeros, and a nonzero binary-diagonal deficient companion.  Exact mixed-orientation triangle and four-port controls realize the higher-open pure-colour cancellation with shared physical source/deck data, refuting termwise separation and leaving a function-field restriction-separation lemma open.  Three-or-more-deficient profiles, attachment gates, node closure, and global resolution remain open. | [Mixed-kernel hierarchy and two-deficient binary localization](../claims/arbitrary-order/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_MIXED_KERNEL_PARTIAL_UNCONTRACTION_AND_TWO_DEFICIENT_BINARY_LOCALIZATION_THEOREM.md), [hostile review](audits/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_MIXED_KERNEL_PARTIAL_UNCONTRACTION_AND_TWO_DEFICIENT_BINARY_LOCALIZATION_REVIEW_2026-08-28.md) |
| `GLS64` | In the GLS63 exactly-two-deficient residual, evaluate every raw port edge at its cross products.  Each pair-contracted same-source equation kills the corresponding effective scalar `delta_ij=eta w_ij+A_iB_j+B_iA_j`, and each one-deficient-kernel equation kills the physical four-vertex cofactor `C_u^A` and its `B` mate.  The denominator-free complementary-edge identity `sum w_ij delta_complement = 2 eta H_U + sum B_u C_u^A`, together with the nonzero binary deck `H_U`, forces `eta=W_nm(e_c,e_c)=0`.  Thus both the three-zero and four-zero residuals are empty on `D(eta)` and survive only on the exact deficient-kernel-edge-zero divisor.  Scalar controls show the identity alone cannot remove that divisor.  The full source-realizable permanent-type tensor there, three-or-more-deficient profiles, attachment gates, and global resolution remain open. | [Two-deficient matching-integrability localization](../claims/arbitrary-order/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_TWO_DEFICIENT_MATCHING_INTEGRABILITY_AND_KERNEL_EDGE_ZERO_LOCALIZATION_THEOREM.md), [hostile review](audits/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_TWO_DEFICIENT_MATCHING_INTEGRABILITY_AND_KERNEL_EDGE_ZERO_LOCALIZATION_REVIEW_2026-08-28.md) |
| `GLS65` | On the GLS64 zero-edge divisor, the two-kernel source is exactly one separated restriction of `P_4`: the six pair choices and the two orders in each companion/fixed-row block give all twenty-four `P,Q,A,B` assignments once.  The nonzero pure target and the exact `P_4` rank-drop theorem force a fixed generic rank-two pair.  Every rank-two port is necessarily `c`-oriented and has both fixed rows on its `c`-line.  A complete sixteen-word orientation census excludes local profile `2222`; a perfect-pairing mixed-triple argument and a denominator-free separated anchor-rigidity lemma exclude `2223`, including all zero-coefficient strata.  Hence the only eta-zero local profile is **exactly `2233`**, with two silent rank-two ports and two rank-three ports.  A mixed `YXXX` pure-`P_4` fibre and its exact three-port flat show why pointwise purity is insufficient.  The `2233` same-/opposite-orientation hyperplane boundary, nonzero raw matching deck, three-plus-deficient profiles, attachment gates, and global resolution remain open. | [Eta-zero permanent-source local-rank localization](../claims/arbitrary-order/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_ETA_ZERO_PERMANENT_SOURCE_AND_TWO_TWO_LOCAL_RANK_LOCALIZATION_THEOREM.md), [hostile review](audits/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_ETA_ZERO_PERMANENT_SOURCE_AND_TWO_TWO_LOCAL_RANK_LOCALIZATION_REVIEW_2026-08-28.md) |
| `GLS66` | In the GLS65 `2233` residual, the full GLS64 cofactors and nonzero raw matching scalar force the two rank-three ports onto one active deficient-side axis, kill the silent--silent edge, and synchronize all four silent--rank-three edges.  The two rank-three source rowspaces are hyperplanes omitting the inactive fixed-row coordinate.  If they were distinct, their pair image would contain `A span{P,Q,B}`, whose perfect complement pairing detects every possible silent off-row product; hence purity makes them equal.  Since at least one rank-three port is `c`-oriented, the common normal is `tau P+B` up to probe exchange.  For `tau!=0`, the exact pair-image annihilator forces a rank-two all-`c` slice where the nonzero pure target requires rank one; for `tau=0`, the pair image is totally isotropic and the target vanishes.  Thus the complete zero-anchor root-order-three all-six-rigid **exactly-two-deficient branch is empty**.  Three-plus-deficient profiles, the unique-nonrigid branch, attachment/synchronization gates, nonzero anchor, arbitrary-root transport, and global resolution remain open. | [Eta-zero scalar-axis/common-hyperplane exclusion](../claims/arbitrary-order/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_ETA_ZERO_TWO_TWO_SCALAR_AXIS_AND_COMMON_HYPERPLANE_EXCLUSION_THEOREM.md), [hostile review](audits/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_ETA_ZERO_TWO_TWO_SCALAR_AXIS_AND_COMMON_HYPERPLANE_EXCLUSION_REVIEW_2026-08-28.md) |
| `GLS67` | For any two open deficient labels, contract every other deficient kernel and every injective nonaxis cross product, then quotient every pure-axis active line.  The sole source is one actual pair companion times its common physical deck, and every surviving target colour has one exact deficient support class.  For exactly three deficient maps, the GLS63 incidence/singleton rules and this pair-class theorem give `61,965 -> 2,367 -> 516 -> 453` labelled profiles in ten initial orbits, all with `P=empty, |U|=3`.  Cross-contracting `U` gives one exact deficient-side `P_3`; its pair scalar is the same one-port deck evaluated on the third kernel.  Contracting all three deficient kernels instead gives a second exact `P_3` on `U`.  A common deficient colour makes that target nonzero and pure, but probe-independent deck quotients exclude all injective-nonaxis orientation words.  Thus common-support orbits 1 and 6 are empty and the residual is **432 labelled profiles in eight orbits**.  On every residual the kernel-side `P_3` is zero and forces its three complementary one-port decks to vanish.  Binary, pure, and zero deficient-side endpoints remain algebraically nonempty, so the eight residuals require a shared-cofactor/higher-open integrability argument.  Four-plus-deficient profiles, unique-nonrigid attachment, every downstream gate, and global resolution remain open. | [Three-deficient pair-class and dual-P3 localization](../claims/arbitrary-order/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_THREE_DEFICIENT_PAIR_CLASS_AND_P3_ORBIT_LOCALIZATION_THEOREM.md), [hostile review](audits/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_THREE_DEFICIENT_PAIR_CLASS_AND_P3_ORBIT_LOCALIZATION_REVIEW_2026-08-28.md) |
| `GLS68` | With exactly four deficient labels, the GLS63 incidence/singleton rules and all six universal GLS67 pair classes give the exact normalized census `137,781 -> 20,778 -> 4,794`, with fifty canonical support/rank/zero-count keys and no survivor having two pure-axis labels.  Exactly fifty-four profiles in two keys have `P=empty` and no nonaxis cross-product zero, hence a fully ternary four-port target.  They are not accepted six-vertex inputs: each of the two cross-product contraction vectors has probe bidegree `(1,1)`, each complementary deck has `(2,2)`, and the same-source pullback has `(3,3)` rather than being multilinear in two open roots.  Fixing the probes instead removes those roots and leaves a four-port equation.  Thus the direct finite-`n=6` bridge is invalid and all **4,794 profiles in fifty keys remain open**; `4,740` of them have binary or monocolour targets.  A common-physical-deck factorization, polarization, or coefficientwise integrability theorem is required. | [Four-deficient pair-class and probe-dependent four-port boundary](../claims/arbitrary-order/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_FOUR_DEFICIENT_PAIR_CLASS_AND_PROBE_DEPENDENT_FOUR_PORT_BOUNDARY_THEOREM.md), [hostile review](audits/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_FOUR_DEFICIENT_PAIR_CLASS_AND_PROBE_DEPENDENT_FOUR_PORT_BOUNDARY_REVIEW_2026-08-28.md) |
| `GLS69` | For five or six deficient maps, put `D_a=N-M_a` and `L_a=D_a disjoint-union E_a`.  Every arbitrary open-set member has formal colour-term support exactly when `L_a subseteq T`; face evaluations commute and remain restrictions of the same physical matching decks.  The five-deficient census is `59,049 -> 18,270 -> 2,640` (`P=1`, twelve keys) and `236,196 -> 79,095 -> 24,435` (`U=1`, eighty-nine keys); 270 `U=1` profiles have a five-open colour, including 150 with no size-two minimum.  The six-deficient census is `531,441 -> 276,750 -> 99,855 -> 99,180`, with `90 -> 86` keys: the exact three-open source-span condition removes 675 profiles in four type-profile orbits.  Ternary three-open targets are excluded, but binary, pure, and zero faces remain; the sole binary pair-class key is `S_c^2 T_c^4`, and the `Q(omega)` binary triangle is only a fixed-fibre control.  Shared-deck restriction separation and five-/six-deficient branch exclusion remain open. | [Five-/six-deficient open-set tower and overlap boundary](../claims/arbitrary-order/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_FIVE_SIX_DEFICIENT_OPEN_SET_SUPPORT_TOWER_AND_OVERLAP_INTEGRABILITY_BOUNDARY_THEOREM.md), [hostile review](audits/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_FIVE_SIX_DEFICIENT_OPEN_SET_SUPPORT_TOWER_AND_OVERLAP_INTEGRABILITY_BOUNDARY_REVIEW_2026-08-28.md) |
| `GLS70` | The `3,360` six-deficient profiles with one binary triangle form exactly eight keys in two families: `pair+pair`, where the triangle is the first common parent of two pure pair faces, and `exact-triple+exact-triple`, where the triangle is a minimal binary leaf with three strict four-open parents.  The other `45` binary profiles are the one `S_c^2 T_c^4` key.  Its pair equation forces a crossed separated endpoint normalization.  The complementary four-port deck's complete proper-face map is not separating: its ambient kernel has dimension sixteen and contains an explicit physical hafnian direction.  Nevertheless the different pure-`c` four-open source equation forces a nonzero outside `z_(0,c)` probe coefficient; in that port's binary triangle, coefficient separation kills both non-pair decks and leaves one pair tensor times one deck unable to supply two target colours.  Thus `S_c^2 T_c^4` is empty and the six-deficient residual is **`99,135 / 85`**.  The two single-triangle families and every pure/zero-triangle profile remain open. | [Binary-triangle parent taxonomy and proper-face deck kernel](../claims/arbitrary-order/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_SIX_DEFICIENT_BINARY_TRIANGLE_PARENT_TAXONOMY_AND_PROPER_FACE_DECK_KERNEL_THEOREM.md), [hostile review](audits/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_SIX_DEFICIENT_BINARY_TRIANGLE_PARENT_TAXONOMY_AND_PROPER_FACE_DECK_KERNEL_REVIEW_2026-08-28.md) |
| `GLS71` | For Family B `S_0^3R_0^(3-r)T_0^r` with `r<=2`, quotient one outside `R_0` strict parent.  Its two target coordinates give simultaneous pure `P_3` equations with common probe shores.  An endpoint-complete six-edge lemma forces incompatible common forms for the two independent target colours, so all three keys are empty.  For Family A at `r=0`, the two pure pair faces force a crossed normalization.  Three active outside selectors contradict the pure attachments.  With one silent port, the full `P_0Q_0` coefficient isolates a nonzero pure physical four-deck; either nonzero crossed activity product supplies two further legal selector decks, and an exhaustive `alpha!=0`/`alpha=0` projection kills every supplier of the pure coefficient.  Thus the complete Family-A `r=0` key is empty.  A restricted edge-array control proves that the two-E attachment subsystem alone is compatible.  The six-deficient residual is **`98,355 / 81`**; Family A `r=1,2,3`, Family B `r=3`, and all pure/zero-triangle profiles remain open. | [Single-binary strict-parent edge chart and attachment activity](../claims/arbitrary-order/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_SIX_DEFICIENT_SINGLE_BINARY_STRICT_PARENT_EDGE_CHART_AND_ATTACHMENT_ACTIVITY_THEOREM.md), [hostile review](audits/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_SIX_DEFICIENT_SINGLE_BINARY_STRICT_PARENT_EDGE_CHART_AND_ATTACHMENT_ACTIVITY_REVIEW_2026-08-28.md) |
| `GLS72` | In Family A `S_0R_2R_1R_0^2T_0`, a whole-row activity lemma handles the two-dimensional `T_0` row plane without pretending its `P_0,Q_0` coefficients are pure: each nonzero row covector still exposes its unique strict-parent selectors, and the one-silent `P_0Q_0` coefficient factors the full pure deck.  Three active outside selectors are impossible and a one-silent source cannot be silent at an `R_0` port.  Put the unique silent `T_0` port at `5`.  The two `E` selectors, pure attachments, mixed `F/G` selectors, and full pure deck exclude every `alpha!=0` branch and the `alpha=0,ab!=0` branch by exact rank-two determinant, tangent-space, and characteristic-zero sign arguments.  Every survivor is therefore on `alpha=a=b=0`, where the target-leading restrictions of `W_15,W_25` vanish on `K_5`.  An exact common physical edge array realizes all four selectors, both pure attachments, the zero triangle deck, and the nonzero pure full deck there; its supplier `W_25(e_(2,0),e_(5,0))` is transverse to `K_5`.  Thus the Family-A `r=1` key is **localized but still OPEN**, and the residual remains **`98,355 / 81`**. | [One-T0 activity localization and transverse full-deck sharpness](../claims/arbitrary-order/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_SIX_DEFICIENT_ONE_T0_SINGLE_BINARY_ACTIVITY_LOCALIZATION_AND_TRANSVERSE_FULL_DECK_SHARPNESS_THEOREM.md), [hostile review](audits/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_SIX_DEFICIENT_ONE_T0_SINGLE_BINARY_ACTIVITY_LOCALIZATION_AND_TRANSVERSE_FULL_DECK_SHARPNESS_REVIEW_2026-08-28.md) |
| `GLS73` | Keep the ten physical edges avoiding the silent `T_0` port fixed to the GLS72 `alpha=a=b=0` common-edge control, but allow all five incident edges and every unused compatible probe coefficient to vary arbitrarily.  In the complete six-open source identity, the `P_0Q_0` all-zero word has only source pair `{3,4}` and forces `c_34 xi=mu_0`, where `xi=W_25(e_(2,0),e_(5,0))`.  A mixed `P_0Q_2` word has zero target; its direct `{0,3}` channel is `-A xi`, while all port-4 endpoint channels have the wrong local factor and the sole `{3,5}` repair channel has zero complementary deck.  Since `A c_34 mu_0!=0`, the two rows contradict.  Thus this exact off-port core has **no full source completion by any incident-edge repair**.  Family A `r=1` remains OPEN because a source may change an off-port core edge and open another coupled deck channel; no profile is removed and the residual remains **`98,355 / 81`**. | [Off-port-core full-source nonextension and coupled-correction boundary](../claims/arbitrary-order/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_SIX_DEFICIENT_ONE_T0_OFF_PORT_CORE_FULL_SOURCE_NONEXTENSION_AND_COUPLED_CORRECTION_BOUNDARY_THEOREM.md), [hostile review](audits/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_SIX_DEFICIENT_ONE_T0_OFF_PORT_CORE_FULL_SOURCE_NONEXTENSION_AND_COUPLED_CORRECTION_BOUNDARY_REVIEW_2026-08-28.md) |
| `GLS74` | In the last Family-B single-binary key `S_0^3T_0^3`, take a nonendpoint outside pure-`P_3` chart and assume that both central probe shores have a non-root coefficient at every triangle label.  The two legal mixed-root strict-parent rows put each triple of full pair decks in its one-dimensional alternating Koszul kernel.  The complete `P_0Q_0` row cancels those nuisance sums and its three-one-port outside map is injective, so all full corrections vanish.  Restriction to the common outside kernel lines then gives one coupled physical-edge system.  Splitting the three central all-zero edge coefficients by support size `3,2,1,0` either kills every outside kernel edge or kills the sole spoke that could use its unique survivor.  Thus the **central-mixed-support nonendpoint chart is EMPTY**.  Central root-axis degeneracies lack one Koszul row and P/Q-common outside endpoints have a genuine parent-map kernel; both remain OPEN.  No profile is removed and the residual remains **`98,355 / 81`**. | [All-T0 Family-B nonendpoint full-parent boundary](../claims/arbitrary-order/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_SIX_DEFICIENT_ALL_T0_FAMILY_B_NONENDPOINT_FULL_PARENT_TRANSVERSE_INJECTIVITY_AND_ENDPOINT_ACTIVITY_BOUNDARY_THEOREM.md), [hostile review](audits/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_SIX_DEFICIENT_ALL_T0_FAMILY_B_NONENDPOINT_FULL_PARENT_TRANSVERSE_INJECTIVITY_AND_ENDPOINT_ACTIVITY_BOUNDARY_REVIEW_2026-08-28.md) |
| `GLS75` | In the same Family-B `S_0^3T_0^3` key, both shores at one central label cannot be root-axis-only: the two nonzero `P_aQ_a` rows would force one common four-port deck onto two independent pure-colour lines.  Thus every central label supplies at least one Koszul row.  One row coupled to the complete `P_0Q_0` coefficient already forces each pair deck to vanish on its two outside kernel lines and every one-port correction to vanish on its kernel line; the exact `31/36` block retains only one alternating parameter and four harmless full-row coboundaries.  The GLS74 support-`3,2,0` cases transfer.  In support one, the nonendpoint pure-`P_3` normal form makes every active central row transverse to both outside shore lines, so the single identities kill the last two kernel-row families and then every outside kernel edge.  Hence the **complete outside nonendpoint chart is EMPTY**.  Only P/Q-common outside endpoints remain OPEN; the typed key and residual **`98,355 / 81`** are unchanged. | [One-sided Koszul and complete nonendpoint exclusion](../claims/arbitrary-order/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_SIX_DEFICIENT_ALL_T0_FAMILY_B_ONE_SIDED_KOSZUL_AND_COMPLETE_NONENDPOINT_EXCLUSION_THEOREM.md), [hostile review](audits/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_SIX_DEFICIENT_ALL_T0_FAMILY_B_ONE_SIDED_KOSZUL_AND_COMPLETE_NONENDPOINT_EXCLUSION_REVIEW_2026-08-28.md) |
...
| `P17` | boundary | `GL` | All four two-edge template orbits fail to cover the necessary Boolean parent; their different 134-entry survivor is physically excluded by ST20. This is no exhaustive weighted cover. |
| `G0` | conditional source obstruction | `ST20` | Under the explicitly stated physical zero/nonzero premises, two full colour slices share one actual induced hafnian. Opposite alternating contractions cancel it over every field and even order; the premises are not supplied automatically. |
| `ST20` | boundary | `GL` | A fully clause-replayed 129-entry Boolean support avoids all twelve tested orbits and has no monomial in the complete pure-attached degree-six space. Weighted guard occurrence, unrestricted n=8 exclusion and all-order coverage remain open. |
| `G0` | conditional specialization | `N8PS` | Under the exact fixed two-K4 scaffold hypotheses, every physical source induces the regenerated full-source CNF. Its independently replayed RUP refutation excludes the entire 96-parameter family. |
| `N8PS` | boundary | `PSMR` | Larger protected scaffolds have actual exterior matching-edge resources in two-component slices. The n=8 certificate cannot be applied to a literal restriction without eliminating those contributions. |
| `G0` | conditional source identity | `PSMR` | Every protected scaffold has the written exposed-partner permanent and injective exterior-resource hafnian formulas. Only the hypothetical full witness supplies all target equalities. |
| `PSMR` | boundary | `GL` | The complete localized minority subsystem, even with component-constant targets, admits exact proper-subsystem controls. Arbitrary-source coverage and full shared-resource cancellation remain open. |
| `PSMR` | refuted parent mechanism | `PSFD` | Retaining all full source equations through any fixed minority depth still cannot uniformly exclude the protected scaffold. Exact pair gadgets on finite graphs of sufficiently large girth supply proper-subsystem controls, including actual exterior resource coupling. |
| `G0` | conditional source identity | `PSCS` | Under the common-star physical support hypotheses, one exact double-component identity retains common-neighbour collisions and unequal center/leaf matchings. A full target supplies singleton and double rows plus the additional binary word that excludes the entire at-most-two-port family. |
| `PSCS` | refuted parent mechanism | `PSRA` | The proposed S1+S2+U4 supply of complete tripartite resources is refuted with exact physical weights and an explicit remaining global failure. No support-to-weights converse is used. |
| `PSRA` | refuted parent mechanism | `PSBS` | The weaker S1+bothS2+U4 binary-decomposition supply, not refuted by the first control, is refuted by the exact overlap-and-tensor construction with every binary component noncomplete. |
| `PSBS` | retained conditional consumer | `PSCS` | The binary-resource contradiction remains valid when its structural hypothesis is actually supplied. The local source antecedent cannot supply it; a new route must use additional global component equations or a different implication. |
| `PSCS` | full-source reduction | `PSCL` | All binary subset targets and exact matching-row deletion force each proper induced directed cycle into a single resource, retaining both permanent factors and every exterior term. |
| `PSCL` | stronger conditional consumer | `PSCS` | Disjoint physical shores of binary resources suffice without completeness: full cuts force a directed Hamilton cycle with singleton ports, which the existing three-color S1/S2 theorem excludes. |
| `PSCL` | refuted parent mechanism | `PSCT` | Both opposite induced-cycle faces plus every binary singleton and same-color double cut are simultaneously realized by exact shared factors. The remaining three-component failure proves that this subsystem does not supply the full binary target. |
| `PSCT` | boundary | `GL` | The exact control omits third-color S1 and distinct-color S2 and fails a specified three-component binary cut. Exact row/column occupation identities expose the remaining mixed permanent terms; the [paired exterior-coupling lemma](strategy/full-binary-exterior-coupling-obligation.md) linking them to the full three-color supply is open. |
| `PSCL` | boundary | `GL` | Every color pair of an FB array must contain an internal shortest cycle with the derived proper-subset contractions and zero full-cycle source. Exact controls realize that face at every length but fail exterior targets; the simultaneous full-source compatibility problem remains open. |
| `PSCS` | boundary | `GL` | The direct full-binary-source parent FB and even-k CSQ4 remain open as weaker subsystem questions after the RCS/BS supply failures. PSCF now excludes the full physical common-star target directly, so those subsystem routes are not load-bearing for that full-source consumer. The original two-pair allocation, arbitrary hollow protected fillings, general Q4 and arbitrary-source coverage remain open. |
| `G0` | conditional full-source exclusion | `PSCF` | Under exactly the common-star physical support, arbitrary center/uniform-leaf AB words have a sum of actual hafnian outer products with identity factors on unused components. Locally orthogonal contractions remove every proper crossing set; parity or the two-by-two rank contradiction excludes every k>=2. Odd k already fails the component-constant target. |
| `PSCF` | boundary | `GL` | No arbitrary-witness or arbitrary-protected-filling reduction to this support is supplied. General hollow blocks can add center-leaf crossings or crossings from a leaf in another color, destroying the forced protected leaf pair and the rank-one surviving source. Those actual additional terms remain to be controlled. |
| `PSCF` | refuted extension mechanisms | `PSCE` | All AB equations are insufficient for arbitrary protected fillings, as exact Laurent controls supply the rank-two correction while failing full split-leaf columns. Protected scaffold structure alone cannot supply a visible cut, and nonnegative protected diagonal scaling freezes all component-constant cancellation terms. |
| `PSCE` | boundary | `GL` | The full-source parent SFULL remains open: prove that every arbitrary hollow protected filling satisfying all AB targets has some nonzero nonuniform leaf column. Exterior matching cancellation prevents importing a local n=8 detector. No arbitrary-witness-to-scaffold supply is known. |
| `PSCE` | parent-attempt refinement | `PSEL` | Literal K4 contraction retains all local rows, repeated-leg sectors, and the same exterior core. Legal source-preserving elimination is exactly the relative-kernel condition; the canonical update and weakened-source substitutes fail on exact controls. |
| `PSEL` | boundary | `GL` | No legal correction is supplied from a full source. Bare unrestricted Ward-module membership already contains the needed top-response equation; a uniform or fully stratified actual-source syzygy, or another proved elimination operation, remains open. |
| `PSMR` | exact source refinement | `PSQ1` | Pair-depth one gives the written one/two-path seam formula and rooted-circuit theorem; pair-depth two retains injective resources and its explicit repeated-resource correction. These are identities on the actual common filling, not freely chosen cofactors. |
| `G0` | conditional support implication | `PSQ1` | A full protected source has a crossing term at every mixed macro word and no mixed word with exactly one nonzero term. It therefore supplies P1 and every fixed paired-depth restriction. These remain necessary conditions; PSPR shows that excluding any fixed such support condition cannot prove the weighted parent. |
| `PSQ1` | boundary | `GL` | Macro coverage plus all PSMR cycles does not force SES, even at k=3 with three whole-port DAGs. Same-word saturated exchange cannot reduce q=1 minority sets; stationary closure squares can cancel at actual Laurent weights. A cross-word/color supplier or closure-consistency theorem is required. No finite search outcome supplies it. |
| `PSQ1` | refuted parent mechanism | `PSPR` | A non-macro common-star term of pair depth q exposes a component cycle on at most 3q vertices. For every fixed cutoff, a simple high-girth graph with only three constant gadget-avoiding colorings gives macro completeness without any low-q singleton. P1 is explicitly realized by an 18-component bipartite certificate. |
| `PSPR` | boundary | `GL` | Fixed-cutoff support exclusion is false even with all whole port graphs acyclic. These controls cannot carry weighted macro cancellation; a separate rational weighted-macro control fails a one-pair row. The surviving WP1 parent must couple actual macro and low-q equations on one filling. Unrestricted all-word UPM, SFULL, and arbitrary-witness supply remain open. |
| `PSFD` | construction-family specialization | `PSCG` | The PSFD construction pairs every protected colored edge with one of a different color in another K4. The cubic state-gadget bridge excludes every complete pairing, with arbitrary nonzero crossing weights, from the full source. No minority-depth bound follows. |
| `PSQ1` | conditional normal form | `PSCG` | Under state crossing degree at most one, the six ordered singleton macro assignments per component force two disjoint C4s through each component/color pair of protected edges. This exhausts the support and actual zero coefficients force crossing products -1. The cubic theorem then excludes the full source, while WP1 retains the low-q matching obligation. |
| `PSCG` | boundary | `GL` | For gadget products -1, nonzero physical words biject with auxiliary cubic-graph matchings. WP1 asks whether all extra matchings can avoid both component-constant words and min-q<=1 words. The imported matching-count theorem supplies no such metric control. General protected arrays need not decompose into these gadgets; SFULL and arbitrary-witness scaffold supply remain open. |
| `PSQ1` | conditional normal form | `PSMD` | Singleton cut parity gives at least 6k crossings. At equality all incidences are two-entry protected-pair bijections. Actual macro equations remove triangles and localize overlaps; a one-pair or two-pair unique word removes every overlap under WP2, producing complete protected-edge pairing. No stronger WP1 antecedent is silently substituted. |
| `PSCG` | full-source exclusion consumer | `PSMD` | The equality-case WP2 normal form uses every protected colored edge exactly once, satisfying the cubic theorem's full hypotheses. Its mixed unique-matching word excludes the full source at equality without a bound on that word's paired depth. |
| `PSMD` | boundary | `GL` | The crossing-density theorem does not supply a degeneration or reduction of denser protected sources to equality, nor a reduction of arbitrary conjecture witnesses to the protected scaffold. WP1 remains open at general order, and WP2 is only reduced to complete pairing. |
| `PSMD` | proved finite-cover bridge | `MDW1F` | The macro-only equality normal form is valid under WP1. At k=2 hollowness supplies the full parameterization; at k=3 the physical topology/gauge reduction and local low-q gates prove exhaustiveness of the independently checked residual. Unique matching witnesses exclude every residual support at arbitrary nonzero complex weights. |
| `MDW1F` | boundary | `GL` | The weaker WP1 antecedent is excluded at equality only for k=2 and k=3. The all-order full-source density theorem has no low-q word bound; it cannot silently supply higher-order WP1 or an arbitrary-witness normal form. |
| `PRDC` | elementary alternative and family extension | `PSCG` | Complete two-crossing resource pairings satisfy the direct-cycle hypotheses. The same proof permits three/four crossings per block, arbitrary nonzero protected weights and physical resource overlap. PSCG retains its independent cubic correspondence; its exclusion no longer requires the imported classification. |
| `PRDC` | full-source exclusion consumer | `PSMD` | The equality WP2 normal form yields a complete protected pairing, so PRDC supplies its mixed unique-matching word without an external theorem. All earlier density and overlap arguments remain necessary; no low-depth bound follows. |
| `PRDC` | boundary | `GL` | Full-source supply of paired resources remains open even under crossing degree at most two. Degree two permits endpoints to use different foreign resources and permits four-crossing singleton cancellation. No arbitrary-witness-to-scaffold or resource-block reduction is proved. |
| `G0` | conditional source transport | `PSDT` | Protected color-regular full-source rows force C8-to-split transport and q1 escape. An actual reciprocal all-background local control requires a further cross-component/global implication. No arbitrary-witness scaffold or color-regular supply is implied. |
| `RSTC` | family extension and elementary consumer | `PRDC` | Paired blocks satisfy two-exit closure because distinct foreign targets are the protected foreign pair. Resource-simple triple blocks with that closure have the same unique-edge cycle balance; odd-color exceptional-transition symmetry supplies mixedness. |
| `PSDT` | unresolved supplier | `RSTC` | C8 transport, q1 escape and the new same-component orphan repair do not establish a resource partition or two-exit closure. The exact orphan coefficient h(1+g) forces one complementary paired resource with g=-1. Separated orphan targets and global consistency remain open; no same-component branch exclusion or automatic escape iteration is asserted. |
| `PSDT` | boundary | `GL` | The color-regular C8 implication does not exclude its repaired boundary tensor, all color-regular arrays, or general degree-two supports with (2,0) crossing-color distributions. Macro-only pair normalization and deletion of macro-invisible entries are refuted by exact controls. |
| `RSTC` | boundary | `GL` | Repeated-color resource cycles and arbitrary degree-two arrays need not satisfy block or closure hypotheses. An exact protected-hollow triple fixture refutes every constant-background good-cycle supplier from endpoint supply alone; it has many other unique mixed words and is not a source. |
| `G0` | conditional all-split reduction | `ASSR` | Under explicit protected/color-regular/all-split hypotheses, binary and switched physical words force one resource cycle with every edge signature -1. Hamilton parity removes the global gauge obstruction. The exact path criterion retains the actual physical K4 incidence. |
| `ASSR` | boundary | `GL` | The canonical signed ladder has no continuous weight freedom, but an all-order mixed transversal satisfying the necessary and sufficient path conditions is not supplied. The C8, incomplete split and non-color-regular degree-two branches are outside this reduction. |
| `ASSR` | refutation of a supplier mechanism | `ASOS` | The full source supplies both oriented one-switch root conditions. An exact k=5 control passes every forward predecessor condition while retaining hollow C30 and all binary Hamilton gates, so forward-only root minimality cannot provide a contradiction. |
| `ASOS` | boundary | `GL` | The bidirectional K4 straddling proposition is open. Proving it would close only the explicitly all-split color-regular protected family; source-to-all-split supply and arbitrary-witness extraction are separate obligations. |
...
| `M1` | full-source finite specialization | `N8R2C` | At n=8 and maximum r=2, retain the common-coordinate physical-incidence hypothesis. Every root rank is excluded: exact source/cofactor arguments handle nonzero blocks and most zero-block orientations; the remaining pure orientation has a proved necessary-support bridge and independently accepted exact decision-tree certificate. |
| `N8R2C` | boundary obligation | `GL` | Maximum-root saturation does not force common-coordinate physical incidences at a usable pair. Other r=2 incidence types, higher root orders, and arbitrary n are not excluded by this theorem. No root-rank residual remains inside the common-coordinate child. |
| `M1` + `N8R2C` | full-source finite implication | `N8R2K` | At n=8/max-r=2 with an invertible physical root edge, one joint-kernel vector with exactly one zero coordinate at every outsider would force paired labels, one physical common-column anchor per pair, and a contradictory physical cofactor system. The orientation exception derives the complete N8R2C incidence premise before invoking it. |
| `N8R2K` | necessary boundary | `GL` | Every invertible physical edge must have an outsider whose joint kernel is zero or a coordinate axis. This does not prove an invertible edge exists or exclude either residual kernel type, lower-rank root edges, general r=2, or arbitrary n. |
| `M1` | conditional physical-source implication | `N8R2S` | At n=8/max-r=2, the invertible-edge graph is triangle-free and a degree-four vertex has three complementary diagonal unit edges. Two adjacent degree-four vertices therefore supply the root architecture of the six-diagonal-leg source theorem. Its analytic implication requires no maximum-root or outside-support hypothesis once that architecture is given. |
| `N8R2S` | source reduction / boundary obligation | `GL` | At least one triple of inactive image planes must coincide; on every such shore the outside hafnian vanishes with the opposite shore fully open and the corrected physical cofactor identity holds. Arbitrary outside blocks on this boundary, arbitrary invertible components, and the no-invertible-edge branch remain open. N8R2E closes the specialization with physical AA/BB units, including the max-r=2 adjacent-degree-four application. |
| `M1` + `N8R2S` | exhaustive physical-source specialization | `N8R2E` | In the paired-root architecture, require all six full AA/BB blocks to be nonzero matrix units. N8R2S supplies a common plane. Its normal has support three, two, or one; each case forces the retained BB cofactor to vanish and contradicts one uniform three-target lemma. Two invertible edges in a max-r=2 triangle supply the required third-edge unit, closing the adjacent-degree-four application. |
| `N8R2E` | boundary obligation | `GL` | The n=8/max-r=2 invertible-edge graph has no adjacent degree-four vertices. This does not assert that arbitrary witnesses supply those vertices, exclude other invertible-edge components or all-rank-at-most-two graphs, settle other root orders, or prove the global conjecture. |
...
| `U1` | complete finite source/certificate cover | `N8R1` | At n=8, selected pure matchings supply eighteen exhaustive scaffold orbits while all 28 nonzero unit blocks remain present. Collision clauses and 39 exact complete-fibre odd relations are necessary for every complex weighted source; eighteen supplied, independently checked final UNSAT proofs exclude all label assignments. |
| `N8R1` | finite source-branch supply | `M2` | Every hypothetical ternary n=8 witness has maximum torus-root cardinality at least two. This removes only the n=8/r=1 child; it supplies no invertible-edge assumption and leaves other root orders and arbitrary n unresolved. |
...
| `GLD93` | H4 Q6/e boundaries, Fitting, and remaining component/source cover | `GLD94` | Close the retained `e=0` coefficient boundary by a complete parameterization, determinant-safe simultaneous-pivot cover, and exact common-kernel analysis before pursuing `Q6`, the Fitting pullback, and remaining component/source coverage. |
...
| `GLD95` | H4 Q6 outside the written F88 family, Fitting, and remaining component/source cover | `GLD96` | Use the exact four raw bordered seven-minors and the E31/H2/g0 localization to force the written F88 offsets.  The global adjugate identity removes `R31` as a gate; retain the remaining exceptional factors for the GLD96/GLD99/GLD100 continuation. |
| `GLD96` | complete normalized p=2 H4/Q6 fibre, including R31=0 | `GLD97` | Retain symbolic `a` and arbitrary offsets `B,C`; combine `Q2`, the four bordered minors, and two direct seven-minors to prove `(Q2,T0,T1,T2,T3,D0,D2)=(Q2/5,B,C)` without inverting R31/E31/g0. |
| `GLD96` | normalized H2=Q6=0 degree-drop branch on D(Delta) | `GLD99` | Recompute both Q(i) branches with the six selected seven-minors and the uniform symbolic-a `158 x 144` exact membership system; pin branch-specific payload/certificate hashes and force `B=C=0`. |
| `GLD96` | normalized g0 branch on `D(E31*H2*Delta)` after the E31-open B=0 step | `GLD100` | Recompute the four primitive gamma coefficients and the three pair resultants, prove the exact eight-factor necessary radical cover, and close every admissible quadratic/quartic fibre by exact quotient-field direct-minor certificates, forcing `C=0`. |
| `GLD99` | normalized H2=Q6=0 handoff on D(Delta) | `GLD100` | Supply the complementary H2=0 piece of the normalized route; together with GLD100's H2-open result, this removes both H2 and g0 from the remaining normalized gate set. |
| `GLD96` | E31/Delta exceptional strata after the normalized GLD100 continuation, arbitrary H4 Q6 outside F88, Fitting, and remaining component/source cover | `GL` | Analyze the retained `E31=0` and `Delta=0` loci, compute the GLD83 pullback with full raw response incidence on `C_F` rank drops, and cover remaining gauges, components, sources, profiles, roots and orders. |
...
| `GLD93` | completed e=0 boundary handoff | `GLD94` | GLD94 parameterizes the complete affine e=0 boundary, retains all exceptional values, closes the simultaneous-pivot branch by two auxiliary charts and exact resultants, and proves the forced unsaturated family has singular centre. |
...
| `GLD95` | remaining H4 Q6 outside F88 and global Fitting/component/source-cover obligation | `GLD96` | Close the generic offset residual on `D(E31*H2*g0*Delta)` by an exact four-minor resultant witness; the polynomial bordered-determinant identity includes `R31=0` and retains the other exceptional strata for the later GLD99/GLD100 synthesis. |
| `GLD96` | completed p=2 full-offset parent-attempt handoff | `GLD97` | GLD97 closes the complete normalized p=2 Q6 fibre by adjoining `Q2` to six raw seven-minors and proving `(Q2,T0,T1,T2,T3,D0,D2)=(Q2/5,B,C)`, including R31=0 without E31/g0 localization. |
| `GLD96` | normalized H2=Q6=0 degree-drop branch on D(Delta) | `GLD99` | GLD99 recomputes both Q(i) branches independently, checks all six total-degree-three minor payloads (including genuine `C^2` in `D0,D2`), and proves exact uniform-a memberships forcing `B=C=0`; GLD95 remains the separate endpoint. |
...
| `GLS56` | rigid coupling / alternate-receiver successor | `GL` | On the all-six-rigid branch, `GLS57` handles the all-rank-one cell, `GLS58` gives the exhaustive rank-profile reduction, `GLS61` excludes the complete all-injective cell, `GLS62` excludes the exactly-one-deficient cell, and `GLS63`--`GLS66` exclude the complete exactly-two-deficient cell through mixed-kernel localization, scalar integrability, separated permanent extraction, and the common-hyperplane annihilator.  `GLS67` localizes exactly three deficient maps to 432 profiles in eight orbits, excludes their common-support cells, and forces the opposite kernel-side one-port decks to vanish.  `GLS68` localizes four deficient maps to 4,794 normalized profiles in fifty keys and proves that their direct cross-deck six-vertex shortcut is invalid because the would-be edges depend on the open probes.  `GLS69` gives the exact five-/six-deficient open-set support tower, localizes the five-deficient branches to `2,640/12` and `24,435/89`, and corrects the six-deficient residual to `99,180/86` after a rank-one three-open span exclusion.  `GLS70` classifies all binary triangles and excludes the sole `S_c^2 T_c^4` pair-class key by descending from the pure four-open parent, leaving `99,135/85`.  `GLS71` descends through both single-binary strict-parent families, excludes Family B `r<=2` and the complete Family-A `r=0` key, and leaves `98,355/81`.  `GLS72` localizes Family A `r=1` to one silent-`T_0`, `alpha=a=b=0` transverse cell and exhibits a common-edge control there.  `GLS73` proves that this exact off-port core has no full source completion even after arbitrary replacement of all five silent-port incident edges, forcing any repair to be coupled to the core.  `GLS74` returns to Family B `r=3` and excludes its central-mixed-support nonendpoint full-parent chart by alternating pair-deck camouflage and common-edge kernel integrability.  `GLS75` proves one legal Koszul row per central label is automatic and sufficient, closing every outside nonendpoint.  `GLS76` uses all mixed complete endpoint coefficients, retains the genuine full coboundary kernel, and nevertheless closes every P/Q-common endpoint; the complete Family-B single-binary family is empty and the residual becomes `98,295/80`.  `GLS77` returns to the complete GLS72 Family-A `r=1` source: twenty-four literal full coefficients force a common outer-product matrix to vanish, while five complete colour-row differences cancel every repair channel and contradict both nonzero GHZ targets.  The complete Family-A `r=1` key is empty and the residual becomes `97,215/79`.  `GLS78` treats the two remaining Family-A keys as one serious parent attempt: their exact proper-face blind spot has dimensions four and eight and a nonzero physical direction, all kernel selectors plus attachments have common-edge controls, and every off-kernel `T_0` contraction retains repairs from the other `T_0` ports.  It leaves both keys open and the residual unchanged.  The eight three-deficient orbits, all four-deficient survivors, both five-deficient residuals, the remaining six-deficient residual, and their distinct same-source integrability obligations remain open.  `GLS59` handles the unique-nonrigid source structure: old-probe exchange forces overlapping pure probe stars and closes the natural `GLD3` re-anchor because its star triangle cannot be target-diagonal.  A different legally transported receiver or complete-equation exclusion is still required, with every response, selector, synchronization, nuisance-survival, activity, and anchor gate.  For `r>=4`, neighbour rigidity and arbitrary-root receiver coverage remain separate. |
...
| `GLS58` | complete cross-product partial-uncontraction successor | `GLS61` | Return to the complete same-graph tensor identity before contracting all six cross products, leave each open-label set in turn, and retain every structurally surviving physical companion/deck term. |
| `GLS61` | exactly-one-deficient row-quotient successor | `GLS62` | Keep the unique deficient label and every pure-axis injective label open.  Quotient the former by its joint row space and the latter by their active full-row lines, while preserving every same-source complementary physical deck. |
| `GLS62` | mixed-kernel deficient parent attempt | `GLS63` | Contract arbitrary deficient labels at independent whole-kernel vectors, retain cross-product contractions at injective nonaxis labels, and couple every open-set equation with row-space and active-line quotients on the same physical decks. |
| `GLS63` | complementary-edge integrability successor | `GLS64` | Evaluate the six raw port edges and the deficient--port shores on one common cross-product tuple.  Couple every two-kernel pair equation to the four one-kernel cofactor equations, without dividing by the deficient edge or the nonzero binary deck. |
| `GLS64` | eta-zero permanent extraction | `GLS65` | On `W_nm(e_c,e_c)=0`, retain the factorization `a_i tensor b_j+b_i tensor a_j` across all six complementary pairs.  Identify the resulting twenty-four terms with one separated `P_4` source and apply its exact decomposable rank-drop geometry without choosing fibre-dependent local bases. |
| `GLS65` | two-two scalar-axis/common-hyperplane successor | `GLS66` | Reinsert the full GLS64 pair/cofactor hierarchy and `H!=0` into the exact `2233` separated permanent source.  Synchronize the rank-three active deficient side and source hyperplanes, then compute the common pair-image annihilator without choosing fibre-dependent bases or discarding `tau=0`. |
| `GLS66` | three-deficient pair-class / dual-permanent successor | `GLS67` | Open each deficient pair in turn, retain one physical deck, enumerate the exact three-deficient typed profiles, and compare the deficient-side and nonaxis-side `P_3` contractions over the common probe function field. |
| `GLS67` | four-deficient pair-class / receiver-interface successor | `GLS68` | Apply the universal two-open pair classes and GLS63 incidence rules to all four deficient maps, then test the fully ternary full-cross stratum without treating probe-dependent cross-product decks as fixed graph edges. |
| `GLS67` | residual three-deficient cofactor-integrability successor | `GL` | Couple the eight residual full-cross `P_3` equations to the vanishing kernel-side one-port decks and proper higher-open members through the same deficient--nonaxis edge blocks. |
| `GLS68` | five-/six-deficient support-tower refinement | `GLS69` | Apply the GLS63 incidence rules and GLS67 pair classes to the higher-deficient branches, retain every arbitrary-open face on common physical decks, and impose the corrected three-open source-span condition. |
| `GLS68` | four-deficient same-source integrability obligation | `GL` | Compare the probe-dependent two-port decks as cofactors of one physical matching tensor, polarize the full `(3,3)` identity, or prove a genuine fixed-edge descent.  All `4,794` four-deficient profiles remain open. |
| `GLS69` | binary-triangle first-parent and source-integrability successor | `GLS70` | Classify every binary three-open profile, normalize the sole binary pair-class key, audit the common complementary deck's proper-face kernel, and use a different pure four-open parent before descending to a synchronized triangle. |
| `GLS70` | single-binary strict-parent and activity successor | `GLS71` | For Family B, quotient an outside `R_0` strict parent and compare the two pure target-colour equations on common probe shores.  For Family A `r=0`, combine its crossed pair normalization, all legal activity selectors, the pure attachments, and the full missing-colour four-deck. |
| `GLS71` | one-`T_0` activity/full-deck refinement | `GLS72` | Reuse the crossed pair normalization and exact activity fork on Family A `r=1`.  Separate the silent-`R_0` and silent-`T_0` cells, retain both mixed selectors, and compare the kernel-restricted equations with the actual unrestricted missing-colour deck. |
| `GLS71` | all-`T_0` Family-B full-parent refinement | `GLS74` | On Family B `r=3`, take the outside pure-`P_3` chart before contracting its row-plane kernels.  Use both legal central mixed-root rows, the complete `P_0Q_0` coefficient, and the common physical pair/spoke edges to test the nonendpoint chart. |
| `GLS72` | transverse full-source completion test | `GLS73` | Hold the exact surviving off-port edge core fixed, allow arbitrary changes on every edge incident to the silent port, and inspect complete six-open coefficients that evaluate the transverse `W_25(e_(2,0),e_(5,0))` entry. |
| `GLS73` | complete kernel-difference successor | `GLS77` | Abandon the fixed off-port control.  In the full GLS72 survivor, use literal complete-source rows to constrain the varying off-port core, then subtract two slot-5 colour rows only where their endpoint and off-kernel repair channels cancel identically before restriction. |
| `GLS74` | one-sided Koszul / complete nonendpoint successor | `GLS75` | Prove that double central root-axis support is impossible, couple one remaining Koszul row to the full `P_0Q_0` one-port map, and replace the two-sided exactly-one-central-edge argument by nonendpoint row transversality. |
...
1. **Maximum-root same-pair target attachment.**  `GLS4` closes the
   ...
   **full-swallow** remainder universally starts at nuisance rank four.  This
   ...
   `GLS61` performs that first complete mixed coupling without changing the
   graph or choosing independent local charts.  Before the GLS58 full
   contraction, leave an arbitrary auxiliary set `S` open.  Exactly the
   physical pair companions in `binom(S,2)` remain structurally, producing
   one compatible arbitrary-root hierarchy.  Its one-open equations force
   two identically zero cross coordinates for each target colour.  At
   `r=3`, injective nonaxis labels would then form a labelled `2+2+2`
   partition, but the associated two-open physical companion is forced pure
   diagonal while every possible injective shore orientation has a nonzero
   off-diagonal projection.  The nonaxis branch is empty.  The same
   hierarchy excludes every nonempty pure-axis set.  Quotient each pure slot
   by its own full active probe-row line.  The `S=P` equation forces three
   disjoint nonaxis zero sets and hence at most three axes; a singleton zero
   then makes `S=P union {u}` a nonzero pure target whose complete source side
   is killed by those quotients.  Thus the entire all-injective `r=3` branch
   is empty.  This is genuine progress down the source-integrability tree,
   but every deficient-map continuation, the unique-nonrigid branch,
   promoted attachment gates, nonzero anchor, and higher-root exclusions
   remain open.

   `GLS62` continues down that same-source hierarchy on the
   exactly-one-deficient `r=3` cell.  With the deficient label open, quotient
   its covector slot by the joint probe-row space; quotient every injective
   pure-axis slot by its active full-row line.  The one-nonaxis-open equations
   force at least two zero labels for every coordinate visible on the
   deficient kernel.  Equality two leaves exactly one nonaxis pair companion
   and its actual complementary deck; the nonzero pure target would force
   that companion pure diagonal, contradicting all injective orientations.
   Hence each visible coordinate needs three labels.  Disjointness forces a
   rank-two deficient map with a pure-coordinate kernel.  Zero pure axes give
   an exact `3+1+1` partition and an impossible rank-two/nonaxis pure
   companion; one or two pure axes are killed by active-line quotients while
   a target coefficient survives.  Thus the cell is empty and every
   all-six-rigid witness has at least two deficient maps.  The next obstruction
   is now their common same-source overlap: rigid double contractions are only
   zero, mono, or binary, and neither pairwise kernel choices nor their
   physical decks synchronize automatically.

   `GLS63` makes the required serious parent attempt.  It contracts arbitrary
   deficient labels at independent generic vectors of their whole kernels
   while retaining the cross-product hierarchy at injective nonaxis labels.
   Common kernel-supported colours need three disjoint nonaxis zero labels;
   a singleton zero forces the opposite deficient map to be a rank-one
   coordinate readout.  For exactly two deficient labels, the complete
   support/rank census and a rank-two pure-companion no-go leave only two
   rank-two maps with one common pure-coordinate kernel, no pure axes, no
   complementary-colour zeros, and a nonzero binary diagonal deficient-pair
   companion.  That final family is not excluded.  Mixed `XXY` orientations
   give an exact pure-colour three-port triangle, and a two-`X`/two-`Y`
   six-pair identity gives the four-port analogue; both can use shared
   physical source/deck data.  The next load-bearing lemma is therefore
   function-field restriction-separation coupling every proper-subset deck
   equation to the nonzero binary residual.  Pointwise polarization or
   termwise purity cannot supply it.

   `GLS64` supplies the first exact restriction-separation edge.  On one
   common cross-product tuple, all six pair-contracted equations kill the
   effective edge scalars `delta_ij`, while the four one-kernel equations
   kill the physical cofactors `C_u^A` and `C_u^B`.  The complementary-edge
   identity counts every raw four-port matching twice and gives
   `2 eta H_U=0`.  The binary `S={n,m}` contraction has `H_U!=0`, so the raw
   deficient edge must satisfy `eta=W_nm(e_c,e_c)=0`.  This closes both the
   three-zero and four-zero residuals on `D(eta)`, but not the zero divisor.
   There the contracted tensor has permanent-type port blocks
   `a_i tensor b_j+b_i tensor a_j`; scalar controls retain nonzero `H_U`, so
   a full tensor source-integrability theorem or a legal receiver is still
   required.

   `GLS65` identifies that full tensor exactly.  The six choices of the
   `P,Q` port pair and the two orders in each probe and fixed-row block are
   the twenty-four terms of one separated `P_4` restriction, with nonzero
   target `kappa z_(0,c)z_(1,c)c^tensor4`.  The accepted `P_4` rank-drop
   theorem forces a fixed generic rank-two pair; each rank-two port must be
   `c`-oriented and both of its fixed deficient-edge rows lie on the
   `c`-line.  The complete binary orientation census excludes four
   rank-two ports.  For three, mixed off-row words violate the perfect
   `R_1 x R_3` pairing bound, while a homogeneous word forces the fourth
   shore pure and then loses a transverse source direction by exact anchor
   rigidity.  Thus the eta-zero branch has exactly two rank-two and two
   rank-three local maps.

   `GLS66` closes that load-bearing `2233` boundary by reusing the complete
   scalar hierarchy rather than the pure permanent equation alone.  The
   rank-three ports lie on one active deficient-side axis and their source
   hyperplanes coincide.  One `c`-oriented rank-three port reduces the
   common normal to `tau P+B` up to probe exchange.  Its exact degree-two
   pair-image annihilator is incompatible with a nonzero rank-one GHZ
   slice for both `tau!=0` and `tau=0`.  Hence exactly two deficient maps
   are now excluded; the next all-rigid parent is the complete
   three-plus-deficient mixed-kernel branch.

   `GLS67` attacks that parent from both sides of the same hierarchy.  Every
   two-open deficient contraction has only one physical pair companion and
   forces exact support classes.  At deficient count three this reduces the
   complete typed search to `453` profiles in ten initial orbits, all with
   three nonaxis ports.  Cross-contracting those ports gives one exact
   `P_3` on the deficient modes, while contracting the three deficient
   kernels gives another exact `P_3` on the nonaxis modes.  A common
   deficient colour would make the second target pure; its deck rows cannot
   follow the independent probe variables, so all same and mixed orientation
   words force the source to zero.  This excludes two orbits and leaves
   exactly `432` profiles in eight orbits.  On each residual, the second
   `P_3` is zero and its three physical one-port decks vanish.  The eight
   deficient-side binary/pure/zero endpoints remain algebraically possible;
   their shared deficient--nonaxis cofactors and higher-open equations are
   now the load-bearing integrability wall.  Four-plus-deficient profiles
   remain a separate part of the same parent.

   `GLS68` carries the same parent attempt through exactly four deficient
   labels.  The complete normalized support/rank/zero-count search falls from
   `137,781` profiles to `20,778` after GLS63 and to `4,794` after all GLS67
   pair classes, in fifty canonical keys.  Fifty-four profiles in two keys
   have a fully ternary target after the two nonaxis cross contractions.  A
   hostile receiver audit rejects the obvious finite-`n=6` shortcut: those
   contraction vectors are bilinear in the open probes, so the complementary
   decks vary with the same probes and the exact pullback has bidegree `(3,3)`
   rather than root multilinearity `(1,1)`.  Fixing the probes leaves only
   four open ports; reopening independent probes restores the terms incident
   with the contracted labels.  Thus none of the fifty-four profiles is
   excluded, and the other `4,740` profiles are binary or monocolour anyway.
   A same-source cofactor/factorization theorem, not target shape, is the
   next four-deficient obligation.  Five-/six-deficient profiles now pass to
   the `GLS69` support tower; their shared-deck integrability remains open.

   `GLS69` extends the same-source parent to five and six deficient labels.
   With `D_a=N-M_a` and `L_a=D_a disjoint-union E_a`, every arbitrary
   open-set equation has formal colour-term support exactly on the classes
   `L_a subseteq T`; evaluating an open deficient or nonaxis slot gives a
   commuting face of the same physical matching deck.  The five-deficient
   censuses are `59,049 -> 18,270 -> 2,640` for `P=1` and `236,196 ->
   79,095 -> 24,435` for `U=1`.  In the latter branch, 270 profiles have a
   five-open colour and 150 have no size-two minimum, so the tower cannot be
   reduced to pair and triangle leaves.

   For six deficient labels, the pair-level census is `531,441 -> 276,750
   -> 99,855` in ninety keys.  The actual three-open source-span condition
   removes 675 profiles in four type-profile orbits, giving the corrected
   residual `99,180` profiles in eighty-six keys.  This excludes ternary
   three-open targets and the rank-one obstruction inside four candidate
   binary-pair orbits; binary, pure, and zero three-open targets remain.  The
   sole surviving binary pair-class key is `S_c^2 T_c^4`, whose nonzero pair
   face propagates to four triangles.  The exact `Q(omega)` control realizes
   the binary `P_3 -> Delta_2` shape at a fixed fibre but is not a global
   physical-deck realization.  Therefore `GLS69` proves support extraction,
   face synchronization, and finite necessary-profile localization, while
   shared-deck restriction separation and both higher-deficient branch
   exclusions remain open.

   `GLS70` attacks the complete binary-triangle stratum.  The `3,360`
   one-triangle profiles split into exactly eight keys in a pair-plus-pair
   family and an exact-three-set family, with different first-parent maps.
   The other `45` profiles are the sole `S_c^2 T_c^4` pair-class key.  Its
   pair equation forces crossed endpoint probe forms.  Although the proper
   face tower of its four-port complementary deck has a sixteen-dimensional
   kernel meeting the physical hafnian family, the pure-`c` four-open source
   equation forces one outside `z_(0,c)` coefficient.  The corresponding
   triangle then kills both non-pair decks and leaves one common one-port
   deck unable to carry two independent target coordinates.  This excludes
   the key and leaves `99,135` six-deficient profiles in eighty-five keys.
   The next six-deficient theorem must couple Family A's two pure pair faces
   or Family B's exact-three-set leaf to its three strict four-open parents;
   another quotient of the excluded pair class is not load-bearing.

   `GLS71` performs that strict-parent descent on both single-binary
   families.  For Family B, any outside `R_0` quotient separates the two
   target coordinates into pure `P_3` equations with the same probe shores.
   The six oriented cube-edge charts, including every endpoint, force a
   common `P` or `Q` form, and the two independent target colours conflict.
   This removes the `r=0,1,2` keys.  For Family A at `r=0`, the pure pair
   faces give a crossed normalization.  Three active selectors contradict
   the two pure attachments.  In the remaining one-silent activity pattern,
   the full missing-colour coefficient isolates a pure physical four-deck;
   either nonzero crossed activity term also forces two mixed selector decks.
   An exhaustive central-coordinate projection then makes the pure
   four-deck coefficient vanish.  Thus the complete Family-A `r=0` key is
   empty and the live six-deficient residual is `98,355` profiles in
   eighty-one keys.  The next single-binary obligations are Family A
   `r=1,2,3` and the all-`T_0` Family B `r=3`; the pure/zero-triangle profiles
   and both five-deficient branches remain separate.

   `GLS72` refines the first of those four keys.  With two `R_0` outside
   ports and one `T_0` port, three active selectors are impossible.  In the
   one-silent cell, a silent `R_0` port retains enough independent target
   factors for the `GLS71` full-parent contradiction.  If the `T_0` port is
   silent, exact determinant and tangent-space calculations exclude every
   branch except `alpha=a=b=0`, where both target-leading spokes vanish on
   its kernel.  That last cell is genuinely sharp for the equations used:
   one common physical edge array makes both `E` decks, both mixed `F/G`
   decks, and the zero triangle deck vanish while supplying both pure
   attachments and the required pure unrestricted deck.  The supplier is a
   transverse `W_25(e_(2,0),e_(5,0))` coefficient that no kernel selector
   sees.  Hence Family A `r=1` remains open and the live residual stays
   `98,355` profiles in eighty-one keys.  The next load-bearing step must
   expose that transverse coefficient through another legal same-source
   equation or prove a source-integrability bridge; repeating kernel-only
   selectors cannot close the cell.

   `GLS73` performs the first complete-source test of that coefficient.
   Holding only the ten edges away from the silent port fixed to the GLS72
   control, it permits arbitrary replacement of all five incident edges and
   arbitrary unused compatible probe coefficients.  The full `P_0Q_0`
   all-zero word forces `c_34 xi=mu_0` for
   `xi=W_25(e_(2,0),e_(5,0))`.  A mixed `P_0Q_2` word has zero target and,
   after exhaustive source-pair and matching support separation, equals
   `-A xi`.  Since `A c_34 mu_0!=0`, no incident-edge-only completion exists.
   This is an exact source-integrability obstruction, not a key exclusion:
   a surviving source must change an off-port core edge and open a coupled
   cancelling deck channel.  Family A `r=1` remains open and the live
   residual stays `98,355` profiles in eighty-one keys.

   `GLS74` attacks the last Family-B single-binary key directly at its
   complete parent.  On a nonendpoint outside pure-`P_3` chart, and when
   both probe shores have a non-root coefficient at every central label,
   two legal mixed-root rows leave only a one-dimensional alternating
   camouflage among the three full pair decks.  The complete `P_0Q_0` row
   cancels that camouflage and its three-one-port outside map is injective.
   Restricting the resulting full equations to the three common outside
   kernel lines leaves one physical-edge system; the four support sizes of
   the central all-zero edge triple are all impossible against the binary
   target.  Thus the central-mixed-support nonendpoint chart is empty.  A
   root-axis-only central shore removes one of the two Koszul rows, and a
   P/Q-common outside endpoint gives the parent map a genuine kernel.  Both
   boundaries remain open, the typed key is not removed, and the live
   residual stays `98,355` profiles in eighty-one keys.

   `GLS75` closes the root-axis boundary of that nonendpoint argument.  A
   central label cannot have both shores root-axis-only, because its two
   nonzero diagonal probe coefficients would leave the same complementary
   physical deck as a pure tensor in two independent colours.  Thus every
   central label has one legal Koszul row.  One row plus the complete
   `P_0Q_0` coefficient forces every complementary pair deck and one-port
   correction to vanish on the relevant outside kernel lines; the exact
   block retains only one alternating parameter and four full-row
   coboundaries.  The common-edge support argument then closes support sizes
   three, two, and zero.  At support one, the nonendpoint pure-`P_3` normal
   form makes every active row transverse to both shore lines, so the single
   identities kill the last kernel rows.  Hence every outside nonendpoint is
   empty.  Only the P/Q-common endpoints remain, and the typed key and live
   residual are unchanged.

   `GLS76` closes those endpoints with the mixed complete coefficient family
   rather than the rank-deficient `P_0Q_0` row alone.  At a P-common endpoint,
   write each outside opposite shore as
   `q_u(t)=A_u(t)U_u+B_u(t)V_u`, with only `B_u!=0` required.  Splitting every
   complete `P_0Q_t` row in the central `S_0` plane and comparing the
   two-kernel and one-slot transverse tensor grades forces the exact pair- and
   one-port kernel restrictions used by the common physical system.  This
   includes root-only, zero, and projectively synchronized central shores.
   Full row-plane coboundaries and pure endpoint target-line directions do
   remain, so no endpoint-map injectivity is claimed.  In the support-one
   physical case, both inactive central labels nevertheless kill their
   kernel rows at the selected endpoint.  The two incident outside kernel
   edges and the sole opposite-edge spoke then vanish, contradicting the
   binary target.  Probe exchange closes Q-common endpoints.  Hence the
   complete Family-B single-binary family is empty; `60 / 1` is removed and
   the live six-deficient residual becomes `98,295` profiles in eighty keys.

   `GLS77` then returns to the complete Family-A `r=1` source rather than
   repairing the fixed GLS73 control.  Twenty-four literal zero-target
   coefficients expose the four entries of one common outer-product matrix
   against six coordinates of the silent-port row plane.  If any entry were
   nonzero, the remaining endpoint rows would span only one dimension,
   contradicting the rank-two `T_0` type.  Five differences of complete
   slot-5 colour rows subsequently cancel every endpoint, off-kernel, and
   synchronized repair channel before any restriction.  The resulting five
   scalar equations and the vanished outer-product matrix contradict the two
   nonzero GHZ target coefficients by an exhaustive characteristic-zero
   rank-one/zero-product split.  Hence the complete Family-A `r=1` key is
   empty; `1,080 / 1` is removed and the live six-deficient residual is now
   `97,215` profiles in seventy-nine keys.  Family A `r=2,3` and every other
   stated branch remain open.

   `GLS78` makes the required serious parent attempt on those last two
   single-binary keys.  The intersection of the three single-slot
   proper-face kernels is exactly `J_3 tensor J_4 tensor J_5`, of dimensions
   four for `r=2` and eight for `r=3`; an explicit nonzero physical
   four-deck lies in that blind spot.  Exact common-edge arrays satisfy all
   three kernel selector equations and both nonzero pure attachments in
   each chart.  Even adjoining the `GLS77`-shaped relation `C=rp+sm=0` on a
   one-silent `r=2` cell remains compatible.  A partial `T_0` kernel
   contraction leaves the repairs from every other uncontracted `T_0`
   ...
   proper-face/kernel-difference route is
   ...
   GLD94 now closes the retained H4 coefficient boundary `e=0` on
   `D(Omega)`.  Writing `q=t` and `p=(t^2+2t-2)/(2t^2-2t-1)` is complete
   because the two parameter polynomials are coprime.  Away from the recorded
   H1/H2/H3 and d0/P exceptional values, the old and alternate six-pivots
   either force an explicit one-parameter leaf family or vanish together.
   The latter branch is excluded by two auxiliary pivots and exact residual
   resultants; the former has three all-block kernel vectors and a nonzero
   block minor, so every compatible centre is singular.  The unsaturated leaf
   family is nevertheless nonempty (the exact sample has leaf determinant
   `24/11`), and only its intersection with `D(Omega)` is excluded.  The
   remaining H4 boundary is `Q6=0`; the GLD83 Fitting pullback, other
   charts/components/source branches, and global resolution remain open.

   GLD95 now closes the finite common-minor residual on the written GLD88
   rational family `F88` over `D(Delta)`.  The exact resultant/content
   decomposition exhausts every generic factor and all five retained
   content fibres.  The `H2=2p^2-2p+1` fibre is specialized directly because
   it is `LC_q(Q6)` and the generic division has denominator `H2^47`; its
   true gcd is `d0`, hence it is outside the declared open.  Each remaining
   generic branch has an exact unit six-minor, and every retained content
   component has a unit six-minor even though its old GLD88 `P6` is zero.
   ...
   GLD96 now closes an R31-free generic continuation into that F88 result.
   On `D(E31*H2*g0*Delta)`, the four exact raw bordered seven-minors are affine
   ...
   excludes the resulting F88 incidence on `D(Omega)`.  The bordered
   determinant equals its polynomial adjugate expression without division,
   so `R31=0` is included wherever the remaining gates are nonzero.  GLD99
   and GLD100 subsequently close the normalized H2 and g0 pieces; the
   combined route retains `E31=0` and `Delta=0`, together with arbitrary H4
   Q6 points outside F88, the GLD83 Fitting pullback, and the other
   chart/component/source obligations.

   GLD97 supplies the serious parent-level p=2 test of those retained loci.
   On `p=2`, `Q2=Q6(2,q)=0`, and `D(Delta_2)`, the offsets `B,C` are an
   affine translation of arbitrary original `b,c`, with `a` still symbolic.
   Adjoining `Q2` to the four GLD96 bordered seven-minors and two direct
   seven-minors gives the exact ideal
   `(Q2,T0,T1,T2,T3,D0,D2)=(Q2/5,B,C)` in `Q[B,C,q,a]`.  Hence full syndrome rank at
   most six forces `B=C=0` without an `R31`, `E31`, or `g0` localization;
   the GLD75/GLD86 bridge and GLD95 then exclude the incidence on
   `D(Omega Delta_2)`.  Independent canonical-builder and direct
   sparse-support/Bareiss replays agree exactly.  This closes the complete
   normalized p=2 fibre only.  A generic-p quotient Groebner probe timed out
   without output and is not evidence; arbitrary `p`, the GLD83 Fitting
   pullback, other charts/components/source branches, and global resolution
   remain open.

   GLD99 now closes the full normalized `H2=Q6=0` degree-drop branch on
   `D(Delta)` in the GLD88/F88 offset chart.  Both quadratic `Q(i)` branches
   are recomputed with symbolic `a`; the six selected seven-minors all have
   total `BC` degree `3`, with genuine `C^2` terms in `D0,D2`.  Exact
   `158 x 144` Macaulay systems have rank `140`, and the `B`- and `C`-target
   augmentations retain rank `140` with zero residuals, forcing `B=C=0`.
   Branch-specific support, generator, and certificate hashes are pinned, and
   the independent audit recomputes the minus branch rather than treating
   conjugation as a substitute.  The result uses no `R31`, `E31`, or `g0`
   localization; `d0` is excluded by `Delta`, and GLD95 remains the separate
   endpoint.  The physical conclusion retains `D(Omega)`.  This removes `H2`
   from the normalized residual.  GLD100 now removes `g0` on the complementary
   H2-open, so the combined normalized route retains `E31=0`, `Delta=0`, and
   the physical `Omega=0` boundary, as well as arbitrary H4 Q6 points outside
   F88, the GLD83 Fitting pullback, other charts/components/source branches,
   and global resolution.

   GLD100 records the serious parent-level synthesis required after GLD96 and
   GLD99, not a third sibling refinement.  On `D(E31*H2*Delta)`, GLD96's
   E31-open implication first gives `B=0`.  Recomputing the four primitive
   gamma coefficients from the frozen GLD71/GLD88 sparse source and taking
   the exact pair resultants gives the necessary radical cover
   `p(p-1)(p^2+1)(p^2-2p+2)P H2 A4 C4`.  The `p`, `p-1`, and `P` branches are
   already `Delta`-boundary; GLD99 receives `H2`; exact quotient-field
   gamma screens and direct seven-minor identities close the remaining
   `p^2+1`, `p^2-2p+2`, `A4`, and `C4` fibres, forcing `C=0`.  Thus `g0` is
   removed on the H2-open.  Together with GLD99 and the GLD95 F88 endpoint,
   this yields the combined normalized `D(E31*Delta)` route.  Its physical
   conclusion still uses the scale-fixed `C_8=1` incidence bridge and
   excludes the F88 endpoint only on `D(Omega)`; `E31=0`, `Delta=0`,
...
11. **Support-level weighted Bogdanov.**  `WB2` shows that at `n=6,8` the
...
|---|---|---|
| Forward one-switch saturation, one hollow resource cycle and all binary Hamilton gates force a contradiction by shortest-cycle or predecessor-cycle arguments | False: an exact k=5 control satisfies every listed gate for base A, including its actual predecessor assignments. Reverse switches and a unique-matching mixed word fail, so this is a mechanism control and not a source. | [All-split one-switch obstruction](../claims/arbitrary-order/all-split-one-switch/README.md) |
...
| All full source equations through some fixed minority depth exclude the protected scaffold uniformly in order | False for every fixed depth: an explicit finite graph lift gives a rational protected scaffold satisfying all those exact equations and an explicit remaining mixed coefficient equal to one. The graph depends on depth; no full witness or depth-growing obstruction is asserted. | [Fixed minority-depth no-go](../claims/arbitrary-order/PROTECTED_SCAFFOLD_FIXED_MINORITY_DEPTH_NO_GO.md) |
