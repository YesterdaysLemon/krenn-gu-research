...
flowchart TD
  ...
  GLD70 -->|restricted secant saturation / residual-boundary atlas / triangle compression open| GL
...
| `GLD70` | fixed-space secant certificate and remaining atlas | `GL` | For the fixed torus-star `44`-space, prove that epsilon-nonzero forces a balanced flattening rank jump or certify radical membership using the complete restricted flattening-plus-Strassen ideal.  Then build compatible residual-coordinate-boundary and triangle-centre atlases.  A nonunit saturation is only an algebraic survivor, not a graph witness. |
...
| `GLD70` | torus-star secant saturation and atlas obligation | `GL` | Prove `epsilon` lies in the radical of the restricted third-secant ideal on the fixed `44`-space, preferably using balanced minors if sufficient and otherwise retaining the Strassen equations.  Then extend the same language to residual-coordinate boundaries and the general triangle.  A failed elimination, timeout, modular sample, or nonunit algebraic survivor changes no graph status. |
