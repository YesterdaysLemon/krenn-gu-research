...
flowchart TD
  ...
  GLD84 -->|pulled-back Fitting ideals / centre rank at most 6 / other branches open| GL
...
| `GLD84` | pulled-back Fitting and remaining component/source cover | `GL` | Analyze the pullback of `I_Pl` in the six-variable/two-equation rank-eight charts and the saturated Gaussian rank-seven chart, retain `V(I_7(A))`, then cover other survivor components/gauges, lower ranks, triangles, residual-coordinate and isotropic boundaries, smaller survivor families, off-chart source branches, other roots, and non-star atlases. |
...
| `GLD84` | remaining pulled-back Fitting/component/source-cover obligation | `GL` | Compute or certify the pullback of `I_Pl` first on the named six-variable rank-eight chart and independently on the Gaussian rank-seven chart; retain the rank-at-most-six branch.  Then cover other survivor components/gauges, lower ranks, triangles, residual-coordinate and isotropic boundaries, smaller survivor families, non-star profiles, other root orders, and global resolution. |
