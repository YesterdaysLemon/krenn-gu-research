...
flowchart TD
  ...
  GLS65 -->|two-rank-two / two-rank-three source / three-plus deficient / attachment open| GL
...
| `GLS56` | rigid coupling / alternate-receiver successor | `GL` | On the all-six-rigid branch, `GLS57` handles the all-rank-one cell, `GLS58` gives the exhaustive rank-profile reduction, `GLS61` excludes the complete all-injective cell, `GLS62` excludes the exactly-one-deficient cell, `GLS63` localizes the exactly-two-deficient cell to one binary same-kernel residual, `GLS64` forces that residual onto its deficient-kernel-edge-zero divisor, and `GLS65` reduces the eta-zero permanent source to exact local profile `2233`.  Profiles with three or more deficient maps and the GLS65 two-rank-two/two-rank-three source-integrability residual remain open.  `GLS59` handles the unique-nonrigid source structure: old-probe exchange forces overlapping pure probe stars and closes the natural `GLD3` re-anchor because its star triangle cannot be target-diagonal.  A different legally transported receiver or complete-equation exclusion is still required, with every response, selector, synchronization, nuisance-survival, activity, and anchor gate.  For `r>=4`, neighbour rigidity and arbitrary-root receiver coverage remain separate. |
...
| `GLS65` | two-two local-rank source-integrability successor | `GL` | Exhaust the exact `2233` residual: two silent `c`-oriented rank-two source planes and two rank-three source hyperplanes, with same or opposite silent orientations, separated probe dependence, fixed deficient-edge rows, the full pure target, and the nonzero raw matching deck.  For three or more deficient labels, continue the mixed-kernel hierarchy without assuming pairwise mono/binary restrictions synchronize.  Every response, selector, nuisance-survival, activity, anchor, arbitrary-root attachment, and nonzero-anchor gate remains separate. |
...
1. **Maximum-root same-pair target attachment.**  `GLS4` closes the
   ...
   **full-swallow** remainder universally starts at nuisance rank four.  This
   ...
   rank-three local maps.  The remaining load-bearing problem is the full
   `2233` same-source hyperplane boundary coupled to `H_U!=0`, not another
   fixed-fibre permanent test.
