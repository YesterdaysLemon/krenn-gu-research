...
flowchart TD
  ...
  GLS73 -->|coupled off-port correction / remaining integrability| GL
  ...
  GLS76 -->|remaining Family-A / pure-zero triangle integrability| GL
...
| `GLS73` | coupled off-port source-integrability obligation | `GL` | On Family A `r=1`, change at least one off-port core edge while preserving the GLS72 kernel restrictions, and classify the extra full-deck channels that can cancel the two GLS73 coefficient rows.  Treat Family A `r=2,3`, every pure/zero-triangle six-deficient profile, and both five-deficient residuals separately; GLS76 has independently closed the formerly retained Family-B endpoint key.  Resolve the unique-nonrigid/alternate-receiver branch plus every response, selector, nuisance-survival, activity, anchor, arbitrary-root attachment, and nonzero-anchor gate independently. |
...
1. **Maximum-root same-pair target attachment.**  `GLS4` closes the
   ...
   **full-swallow** remainder universally starts at nuisance rank four.  This
   ...
   the live six-deficient residual is now `98,295` profiles in eighty keys.
   Family A `r=1,2,3` and every other stated branch remain open.
