...
| `S2AH` | remaining common-three-space obligation | `S2` | The support-one transverse kernel, support-two `(3,3)` and `(3,2)` profiles, the singly monomial/tangent `(2,2)` profile, three Hilbert--Burch coordinate atlases, joint rank at most four, other S2T component types, other S2Q strata, and every higher order remain open. |
