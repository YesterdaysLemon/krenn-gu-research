...
flowchart TD
  ...
  GLD86 -->|four divisor residual / pulled-back Fitting / other branches open| GL
...
| `GLD86` | divisor-residual Fitting and remaining component/source cover | `GL` | Analyze the `GLD83` pullback on each of the four named divisors after `D(Omega)`, retaining the full raw incidence on `C_F` rank drops; then cover other survivor components/gauges, lower ranks outside this chart, triangles, residual-coordinate and isotropic boundaries, smaller survivor families, off-chart source branches, other roots, and non-star atlases. |
...
| `GLD84` | completed rank-at-most-six boundary handoff | `GLD86` | The exact syndrome minor and `C_8=1` column replacement force every rank-at-most-six point onto the four named divisors, while leaving each divisor after `D(Omega)`. |
| `GLD86` | remaining divisor/Fitting/component/source-cover obligation | `GL` | Compute or certify the `GLD83` pullback on each named divisor after `D(Omega)`, retaining the full raw response incidence on any `C_F` rank drop; then cover other survivor components/gauges, lower ranks outside this chart, triangles, residual-coordinate and isotropic boundaries, smaller survivor families, non-star profiles, other root orders, and global resolution. |
...
1. **Maximum-root same-pair target attachment.**  `GLS4` closes the
   ...
   **full-swallow** remainder universally starts at nuisance rank four.  This
   ...
   holds after `D(Omega)`.  None of the four divisors is excluded after
   `Omega` saturation; the pulled-back Fitting ideals and divisor-by-divisor
   residual analysis remain open.
