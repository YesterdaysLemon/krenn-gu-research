...
flowchart TD
  ...
  GLS64 -->|eta=0 permanent source / three-plus deficient / attachment open| GL
...
| `GLS64` | remaining zero-edge source-integrability successor | `GL` | On `W_nm(e_c,e_c)=0`, the effective port blocks are the permanent-type cross terms `a_i tensor b_j+b_i tensor a_j`.  Exclude their complete three-zero and four-zero same-source tensor together with the nonzero raw four-port matching deck, or transport it to a legal receiver.  For three or more deficient labels, continue the mixed-kernel hierarchy without assuming pairwise mono/binary restrictions synchronize.  Every response, selector, nuisance-survival, activity, anchor, arbitrary-root attachment, and nonzero-anchor gate remains separate. |
