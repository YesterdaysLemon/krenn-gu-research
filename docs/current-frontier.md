...
| `GLS57` | rank-one rigid receiver successor | `GL` | The old-probe `GLD3` route is pointwise impossible: every pair response is supported only at `(kappa(s),kappa(t))`, so a fixed port has diagonal activity in at most one colour.  Decide complete-nuisance survival and the remaining mixed-deck cancellations, legally contract/splice the six-mode first variation to a weighted `P_6` restriction, or construct a different fully gated receiver.  Higher joint ranks, the unique-nonrigid branch, every response divisor, and arbitrary-root coverage remain separate. |
