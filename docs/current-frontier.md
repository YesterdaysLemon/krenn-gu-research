...
flowchart TD
  ...
  GLD87 -->|H4 residual / pulled-back Fitting / other branches open| GL
...
| `GLD87` | H4 residual/Fitting and remaining component/source cover | `GL` | Analyze the `GLD83` pullback on `H_4 intersect D(Omega)`, retaining the full raw response incidence on `C_F` rank drops; then cover other survivor components/gauges, lower ranks outside this chart, triangles, residual-coordinate and isotropic boundaries, smaller survivor families, off-chart source branches, other roots, and non-star atlases. |
...
| `GLD87` | remaining H4/Fitting/component/source-cover obligation | `GL` | Compute or certify the `GLD83` pullback on `H_4 intersect D(Omega)`, retaining the full raw response incidence on `C_F` rank drops; then cover other survivor components/gauges, lower ranks outside this chart, triangles, residual-coordinate and isotropic boundaries, smaller survivor families, non-star profiles, other root orders, and global resolution. |
...
1. **Maximum-root same-pair target attachment.**  `GLS4` closes the
   ...
   **full-swallow** remainder universally starts at nuisance rank four.  This
   ...
   GLD87 does not compute the pulled-back Fitting ideal, analyze H4, or close
   any other chart/component/source branch.
