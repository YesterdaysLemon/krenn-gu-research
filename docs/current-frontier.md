...
flowchart TD
  ...
  GLD79 -->|proper-image survivor open / source bridge open| GL
...
| `GLD78` | On the fixed-star, scale-fixed equal-leaf `GLD75` germ, Reynolds averaging reduces any actual raw solution in a `GLD77` first proportional-column chart to the eight-dimensional invariant raw block.  The fixed quotient and invariant-basis minors are `8(1+i)/27` and `1008i`.  At all three sign points, exact selected augmented minors give rank jumps `8 -> 9` and remain nonzero on explicitly defined `delta_j(F,a,b)` principal opens.  Hence no affine branch, including any higher-order formal arc with homogenizing coordinate not identically zero, enters through those three opens.  This is an **exact local all-order sign-boundary-chart nonextension**, not a full survivor-open exclusion, a cover outside the sign plane, a survivor-only exceptional polynomial, source integration, or global resolution. | [Invariant principal-open nonextension](../claims/arbitrary-order/FOUR_ROOT_TORUS_STAR_SURVIVOR_RESPONSE_SIGN_BOUNDARY_INVARIANT_PRINCIPAL_OPEN_NONEXTENSION_THEOREM.md), [hostile review](audits/FOUR_ROOT_TORUS_STAR_SURVIVOR_RESPONSE_SIGN_BOUNDARY_INVARIANT_PRINCIPAL_OPEN_NONEXTENSION_REVIEW_2026-08-26.md) |
...
| `GLD79` | survivor-locus response and source-presentation globalization | `GL` | Combine the pointwise affine exclusion `GLD74`, the exhaustive Gaussian boundary fibre `GLD79`, and the all-order entrance exclusions `GLD78` through a precise projective proper-image and curve-selection argument to obtain a survivor-open theorem.  Compute a survivor-only exceptional polynomial if explicitness is required; then cover other survivor components or prove that every relevant source presentation is forced through the excluded effective interface.  Maximum-root/no-fifth-root certification, other roots, non-star atlases, and global coverage remain separate. |
...
| `GLD79` | remaining universal response/source-interface obligation | `GL` | Use projective properness and curve selection to combine `GLD74`, `GLD78`, and the exhaustive Gaussian boundary fibre into a survivor-open exclusion.  An explicit survivor-only exceptional polynomial remains a separate elimination target.  Then cover other survivor components or establish a source bridge forcing every relevant presentation through this interface.  Maximum-root/no-fifth-root certification, non-star and lower-rank profiles, other root orders, and global resolution remain open. |
...
1. **Maximum-root same-pair target attachment.**  `GLS4` closes the
   ...
   **full-swallow** remainder universally starts at nuisance rank four.  This
   ...
   calculation for an all-order theorem.  Its corrected dual-number systems
   retain all `35` raw corrections, four scale-fixed survivor directions, and
   both slope derivatives, giving exact ranks `34`, `36`, and `37` before and
   after affine augmentation at every sign point.  The load-bearing argument
   is stronger: average the actual `79`-coordinate raw vector under the
   interface-preserving leaf `S_3`, reduce to the eight invariant kernel
