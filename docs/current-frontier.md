...
flowchart TD
  ...
  GLD65 -->|swallowed shadows / at-most-two-colour response / other roots open| GL
...
| `GLS18`, `GLD65` | pure-three leading-column failure | `GL` | With all six pair base shadows surviving, three nonzero pure first-root classes at any open root would make `M_U=C(B)` pure and three-colour-full.  GLD65 excludes the forced matching camouflage by a five-vector/four-coordinate contradiction.  Thus every first-root quotient loses at least one pure colour; coupling those losses remains open. |
...
| `GLS17` | remaining leading-absorption and activity obligation | `GL` | Force leading survival for a sufficient even-target family, or contradict the simultaneous partial-root absorptions by complete mixed GHZ coefficients.  At `r=4`, GLD65 excludes the all-six-pair-survival branch whenever one four-port first-root quotient retains all three pure colours.  The residue is at least one swallowed pair base class, all four swallowed four-port desired classes, or a coordinated pure-column loss/zero-to-two-colour `M_U` response at every first-root quotient.  The theorem neither proves the converse of its shadow criterion nor integrates GLS8. |
| `GLS18` | remaining target-coupled failure-locus obligation | `GL` | Exclude the simultaneous geometric radical--Fitting containments for a sufficient target family on the complete witness locus, or exhibit a shared residual point with useful leading shadows and all downstream activity gates.  At `r=4`, absence of the four-port pure-`M` row is exactly four full first-root nuisance shadows; with all pair base shadows surviving, GLD65 additionally forces at least one pure first-root column to vanish for every open root.  Pair failure still forces only the diagonal three-space, not the whole nine-space.  Cross-root coupling of the new losses, foreign transport, response activity, arbitrary-root source coverage, and GLS8 remain separate. |
...
| `GLD65` | remaining product-selector source obligation | `GL` | On the all-six-pair base-survival branch, couple the forced missing pure first-root column across all four open roots, or exclude the resulting zero/one/two-colour `M_U` responses with the complete mixed equations and `GLS18`/`GLS19` Fitting profiles.  One swallowed pair shadow, every non-leading four-port selector, other root orders, the promoted source, anchors, and permanent consequences remain separate. |
...
1. **Maximum-root same-pair target attachment.**  `GLS4` closes the
   ...
   **full-swallow** remainder universally starts at nuisance rank four.  This
   ...
   quotient loses at least one pure colour.  Zero-, one-, and two-colour
   responses, swallowed shadows, non-leading selectors, other root orders,
   and the promoted source remain open.
