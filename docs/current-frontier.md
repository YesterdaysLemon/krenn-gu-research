...
flowchart TD
  ...
  GLD72 -->|nonlinear source / graph integrability open| GL
...
| `GLD72` | source-integrability of the fixed-star GHZ-survivor locus | `GL` | Determine whether the concrete Gaussian survivor, or any point on its rank-drop component, can arise from one legal graph with shared decks, responses, and source attachments.  Exclusion requires a nonlinear integrability obstruction; a lift requires full same-graph data and the actual target interface.  Contracted `N_star` membership alone cannot decide either direction. |
...
| `GLD72` | remaining source-integrability obligation | `GL` | Compare the exact GHZ-survivor locus with the nonlinear image of legal same-graph nuisance coefficients.  First test the displayed Gaussian point against shared-deck, incidence, response, and attachment equations.  A contracted-space separator is impossible; a graph-level exclusion or fully integrable lift would be load-bearing.  Non-star atlases and global coverage remain separate. |
