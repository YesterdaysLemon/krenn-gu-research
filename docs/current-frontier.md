...
flowchart BT
  ...
  PSCS["Protected common-star source coupling<br/>PROVED at-most-two-port exclusion and complete-resource consumer"]
  ...
  PSCS -->|larger-port shared resources and general fillings remain open| GL
...
| `PSCS` | **Proved all-order source obstructions over C:** common-centre gadgets, with one unequal-colour label per component edge and two nonzero factors per gadget, obey an exact shared-neighbour source identity retaining both hafnians and four-cycle interference. Singleton and double-component targets forbid one common resource, force coherent triangles and exclude every unique port. If every ordered port has at most two edges, they force a coherent K_(2,2,2) resource decomposition; an additional binary component word then has a product of nonzero factors 1+q, contradicting the full target. More generally, a directed resource cycle excludes every complete tripartite resource decomposition at arbitrary sizes, giving a reusable consumer for a proved decomposition. An exact 56-vertex control passes all singleton and double rows but fails the additional global word, so the latter cannot be dropped. Larger-port decomposition supply, arbitrary protected fillings and Q4 remain open. | [Common-star source obstruction](../claims/arbitrary-order/PROTECTED_SCAFFOLD_COMMON_STAR_COMPONENT_TARGET_OBSTRUCTION.md), [initial review](audits/PROTECTED_SCAFFOLD_COMMON_STAR_REVIEW_2026-09-14.md), [two-port review](audits/PROTECTED_SCAFFOLD_COMMON_STAR_TWO_PORT_REVIEW_2026-09-14.md), [parent attempt](strategy/common-star-q4-parent-attempt-2026-09-14.md) |
...
| `PSCS` | boundary | `GL` | Every common-star port must have at least two edges, at least one must have three or more, and each state edge needs a coherent third-colour triangle. A complete-resource decomposition is excluded at any size, but its source-derived supply beyond the two-port case is open. The original two-pair allocation, arbitrary hollow protected fillings, Q4 and arbitrary-source coverage remain open. |
