...
| `U7D` | A complete exact eight-vertex `r=1` table simultaneously has all three pure target coefficients, strict positive endpoint balance, a moment-balanced representative over `C`, three proper nonempty nonrigidity sets, and an odd three-fibre binomial cycle with `H=-1`. An exposed mixed coefficient is nonzero, so this is sharpness and **not a witness**; the remaining mixed equations are essential. | [Complete pure-target moment-compatible odd-holonomy sharpness](../claims/arbitrary-order/MATRIX_UNIT_COMPLETE_PURE_TARGET_MOMENT_COMPATIBLE_ODD_HOLONOMY_SHARPNESS_THEOREM.md) |
...
| `U7D` | sharpened boundary obligation | `U7` | Additional mixed target equations beyond the cycle fibres are load-bearing; completion, pure normalization, magnitude balance, proper nonrigidity, and odd-cycle sign do not exclude the holonomy branch. |
...
4. **Active word holonomy and pure-shore cancellation.** Every active
   ...
   Its exposed mixed coefficient is nonzero, so the next binomial-holonomy
   step must use mixed target identities beyond the cycle fibres. A least
...
| Binomiality plus completion, pure normalization, moment balance, and proper nonrigidity exclude an odd active-word cycle | False: a complete eight-vertex matrix-unit table has all three pure coefficients one, strict balance, a moment-balanced representative, three proper nonempty nonrigidity sets, and three complete fibres `(1,-1)` with `H=-1=(-1)^3`. An exposed mixed coefficient proves it is not a witness, so additional mixed target equations remain essential. | [Complete moment-compatible odd-holonomy sharpness](../claims/arbitrary-order/MATRIX_UNIT_COMPLETE_PURE_TARGET_MOMENT_COMPATIBLE_ODD_HOLONOMY_SHARPNESS_THEOREM.md) |
