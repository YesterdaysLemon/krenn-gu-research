...
flowchart TD
  ...
  GLS69 -->|five-/six-deficient shared-deck restriction separation| GL
...
| `GLS69` | five-/six-deficient shared-deck integrability obligation | `GL` | Couple the two-/three-/four-/five-/six-open equations coefficientwise through the common physical decks.  The sharp first six-deficient test is the `S_c^2 T_c^4` binary pair-to-four-triangles branch; support extraction and the finite census do not exclude the survivors.  Resolve the unique-nonrigid/alternate-receiver branch plus every response, selector, nuisance-survival, activity, anchor, arbitrary-root attachment, and nonzero-anchor gate separately. |
