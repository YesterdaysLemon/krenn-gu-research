...
flowchart TD
  ...
  S2AO -. discrete repeated patterns / other Hilbert--Burch boundaries .-> S2
  S1 -->|boundary| S3
...
| `S2AO` | remaining common-three-space obligation | `S2` | The repeated-coordinate `(1,1,1)` chart survives only on discrete factor-line patterns `(s,s,t)` with `s!=t`.  Those patterns, the other `(1,1,1)` charts, the other Hilbert--Burch profiles, and all lower or external branches remain open. |
