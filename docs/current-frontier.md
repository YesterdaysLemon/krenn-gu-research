...
| `U7` | Force a singleton or odd signed dependency from arbitrary active-cycle response data, control an aggregate normalized fibre, prove cross-multiplicity coupling, or close the pure-cofactor/deeper exits. This remains **open**: complete physical support with pure/moment-compatible data does not exclude odd binomial holonomy; the all-binomial complete block is now classified and can consistently impose only the known sign, while no theorem forces a favourable singleton/parity outcome or controls aggregate fibres. Conserved cofactor signs need not be positive. | [Complete-block lattice boundary](../claims/arbitrary-order/MATRIX_UNIT_COMPLETE_SAME_MULTIDEGREE_FIBRE_LATTICE_REDUCTION_AND_BINOMIAL_PARITY_DICHOTOMY_THEOREM.md#4-consequence-for-the-live-u7-edge), [phase-holonomy and pure-cofactor boundary](../claims/arbitrary-order/MATRIX_UNIT_PHASE_HOLONOMY_AND_MINIMAL_PURE_COFACTOR_FLOW_REDUCTION_THEOREM.md#8-combined-proof-topology-consequence), and [fixed-template same-degree exclusion](../claims/arbitrary-order/MATRIX_UNIT_U7D_COMPLETE_SAME_MULTIDEGREE_TARGET_BLOCK_SATURATION_EXCLUSION_THEOREM.md) |
...
| `U7E` | boundary obligation | `U7` | The universal reduction does not force a singleton or odd relation from active response data and does not decide aggregate normalized fibre polynomials. A continuation must close one of those exact residuals, couple another multidegree, or enter the pure/deeper topology. |
...
4. **Active word holonomy and pure-shore cancellation.** Every active
   ...
   exact signed-kernel parity.  What remains is to force one of those
   favourable outcomes from arbitrary response data, control an aggregate
   normalized fibre, prove a cross-multiplicity bridge, or couple to the
   deeper topology. A least pure cancelling residual likewise has a conserved
   cofactor flow which branches or alternates on even cycles.
