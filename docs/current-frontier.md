...
flowchart TD
  ...
  GLS67 -->|eight residual orbits / four-plus deficient / attachment open| GL
...
| `GLS67` | residual cofactor-integrability / higher-deficiency successor | `GL` | Couple the eight residual full-cross `P_3` equations to the vanishing kernel-side one-port decks and proper higher-open members through the same deficient--nonaxis edge blocks.  Separately classify four-plus-deficient minimal-open triangles/four-ports and resolve the unique-nonrigid/alternate-receiver branch plus every response, selector, nuisance-survival, activity, anchor, arbitrary-root attachment, and nonzero-anchor gate. |
