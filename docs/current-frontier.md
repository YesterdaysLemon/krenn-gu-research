...
flowchart TD
  ...
  S2AW -. same-colour / outer coordinate charts / rank <=4 .-> S2
  S1 -->|boundary| S3
...
| `S2AW` | remaining common-three-space obligation | `S2` | The complete `(1,1,1)` profile and complete distinct-colour central `(1,1,2)` coordinate-pair chart are closed.  Same-colour and genuinely outer coordinate charts, other `(1,1,2)` boundaries, `(1,2,2)`, and all lower or external branches remain open. |
