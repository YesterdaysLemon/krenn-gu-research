...
flowchart TD
  ...
  GLD80 -->|explicit base divisor / other survivor components open| GL
  GLD81 -->|delta divisor / off-chart source branches / other roots open| GL
...
| `GLD81` | explicit divisor and remaining source/component cover | `GL` | Eliminate the `s`-saturated projective incidence to exhibit a base polynomial `delta(F)` nonzero at `F_0`, or give a finite exact divisor cover.  Then cover other survivor components/gauges, the divisor itself, rank drops, triangles, residual-coordinate and isotropic boundaries, smaller survivor families, other roots, and non-star atlases. |
...
| `GLD81` | remaining explicit-divisor/component/source-cover obligation | `GL` | Compute an explicit survivor-only exceptional polynomial from the saturated elimination ideal, or a finite exact base-divisor cover.  Then cover the divisor, other survivor components/gauges, rank drops, triangles, residual-coordinate and isotropic boundaries, smaller survivor families, non-star profiles, other root orders, and global resolution. |
...
1. **Maximum-root same-pair target attachment.**  `GLS4` closes the
   ...
   **full-swallow** remainder universally starts at nuisance rank four.  This
   ...
   its induced survivor frame lies in `D(delta)`.  Computing that divisor,
   treating it, covering other survivor components/gauges, and closing the
   triangle, rank-drop, residual-boundary, isotropic, smaller-survivor, and
   other-root branches remain the smallest local-to-global obligations.
