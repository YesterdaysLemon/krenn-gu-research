...
flowchart TD
  ...
  GLD89 -->|H4 L1/L2/e P6 boundaries / pure P6 / pulled-back Fitting / other branches open| GL
...
| `GLD89` | H4 L1/L2/e P6 boundaries, pure P6, Fitting, and remaining component/source cover | `GL` | Analyze `L1/L2/e intersect V(P6)` and the pure `P6` residual while computing the GLD83 pullback with full raw response incidence on `C_F` rank drops; then cover remaining gauges, components, sources, profiles, roots and orders. |
...
| `GLD89` | remaining H4-boundary/Fitting/component/source-cover obligation | `GL` | Analyze `L1/L2/e intersect V(P6)` and the pure `P6` residual, compute the GLD83 Fitting pullback with full raw response incidence on `C_F` rank drops, and then cover other components/gauges, lower ranks, source branches, profiles, roots, orders and global resolution. |
...
1. **Maximum-root same-pair target attachment.**  `GLS4` closes the
   ...
   **full-swallow** remainder universally starts at nuisance rank four.  This
   ...
   excluded on the determinant-safe open.  The L1/L2/e intersections with
   the GLD88 six-pivot boundary, the pure P6 residual, the pulled-back
