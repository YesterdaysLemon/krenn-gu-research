...
flowchart BT
  ...
  PSQ1 -->|all-word cross-color closure coupling and P1 exclusion OPEN| GL
...
| `PSQ1` | **Exact paired-minority source identities and repair boundaries over C:** q=1 coefficients are shared two-path permanent minors contracted with actual closure seams; q=2 requires subtracting repeated-resource hafnians. A protected-seam factor identity needs a genuinely nonzero supplier. Under PSMR, every q=1 matching forms one rooted alternating circuit, so same-word protected-pair-saturated exchange cannot reduce its minority set. An independently enumerated 22-entry k=3 support covers every macro word while all three directed graphs are DAGs, refuting SES from macro plus PSMR; it has 56 unique q=1 words and is not a P1 support. The all-order P1 exclusion, full weighted source exclusion, and arbitrary-witness scaffold supply remain open. | [Paired-minority source and repair boundary](../claims/arbitrary-order/PROTECTED_PAIRED_MINORITY_SOURCE_AND_REPAIR_BOUNDARY.md), [independent review](audits/PROTECTED_FULL_SOURCE_PARENT_REVIEW_2026-09-22.md), [review handoff](strategy/full-source-parent-review-2026-09-22.md) |
...
| `G0` | conditional support implication | `PSQ1` | A full protected source has a crossing term at every mixed macro word and no mixed word with exactly one nonzero term. It therefore supplies P1's q<=1 restriction, whose arbitrary-order exclusion is still open. |
