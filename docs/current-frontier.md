...
flowchart TD
  ...
  S2O["Eight controls -> one binary residual<br/>PROVED reduction; residual OPEN"]
  ...
  S2O -->|binary residual remains open| S2
  S1 -->|boundary| S3
...
| `S2O` | Project every S2M control onto its two nonzero root colours.  All eight become one binary pattern: one transverse pure singleton, zero quiet-colour singleton slices, and one quiet-colour pure empty coefficient.  Thus realization of any control requires one explicit binary image/kernel/permanent system.  Its emptiness and realizability are both open; zero singleton slices alone do not force zero empty coefficient: **proved common residual reduction** | [Normalized control pullback](../claims/arbitrary-order/BALANCED_FULL_SENSOR_COMMON_SHORE_NORMALIZED_PAIR_CONTROL_PULLBACK_REDUCTION.md) |
...
| `S2O` | residual realized-incidence obligation | `S2` | First decide the common binary residual or otherwise intersect the full common-shore equations with GHZ target, normalization, and retained jets; at higher order the remaining recurrences also return. |
...
| `A7` | boundary obligation | `U7` | Force the integral containment, then exclude the aligned `Q/C^2` survivor and all three balanced `Q/Q` restrictions using another complete target equation; or treat the A6 rank-three ideal when no suitable binomial core contains it. |
...
1. **Balanced full-sensor gate failure.** Starting from the exact Cramer
   ...
   the eight controls now pulls back to the same exact binary
   image/kernel/permanent residual.  The nearest bridge is to decide that
   residual, lift any binary survivor through the full ternary equations, or
...
6. **All-bridge localized cancellations and bipartite least cores.** Every
   ...
   The next step is to force the completion and integral containment, kill
   those balanced survivors with another complete target equation, or
   control the uncontained rank-at-least-three ideal.  The complementary
   fibre may be empty, so neither `A7` nor equality `N=beta+1` is a universal
   exclusion.
...
| Three vanished singleton slices force the corresponding empty-companion coefficient to vanish | False even in one-dimensional root spaces: three vectors in the plane `u_1+u_2+u_3=0` can have nonzero `3 x 3` permanent.  The S2O residual remains nontrivial because it simultaneously requires a transverse pure tensor in the shared-factor image. | [Normalized control pullback boundary](../claims/arbitrary-order/BALANCED_FULL_SENSOR_COMMON_SHORE_NORMALIZED_PAIR_CONTROL_PULLBACK_REDUCTION.md#3-the-exact-residual-obligation) |
