...
| `U7` | Force the global target-lattice ideal to be a unit or close a topological exit: constrain the aggregate-defect product using equations outside the cycle, combine the primitive pure-cycle relation with mixed response, couple a sparse pure fan character to a target equation, control a nonzero aggregate pure port, force genuine deeper-blocker data, derive a singleton or odd dependency, force useful cross-multiplicity overlap and kill every quotient sheet, or control a rank-at-least-two ideal. This remains **open**: no theorem forces a favourable core, defect coupling, overlap, rank, unit, mixed fan coupling, aggregate-port collapse, or deeper outcome, and aggregate-cycle equations alone can leave `H` algebraically free. | [Aggregate-cycle boundary](../claims/arbitrary-order/MATRIX_UNIT_AGGREGATE_ACTIVE_CYCLE_DEFECT_FACTORISATION_AND_SPLIT_FIBRE_SHARPNESS_THEOREM.md#6-consequence-for-the-live-u7-edge), [pure port/fan boundary](../claims/arbitrary-order/MATRIX_UNIT_MINIMAL_PURE_COFACTOR_PORT_AGGREGATE_AND_CONFORMAL_FAN_REDUCTION_THEOREM.md#7-relation-to-the-live-proof-forest), and [global cross-multiplicity boundary](../claims/arbitrary-order/MATRIX_UNIT_CROSS_MULTIPLICITY_GLOBAL_TARGET_LATTICE_AND_HOLONOMY_DICHOTOMY_THEOREM.md#7-consequence-for-the-live-u7-edge) |
...
| `U7J` | boundary obligation | `U7` | Aggregate-cycle equations alone do not constrain holonomy: a successful continuation must relate their gauge-invariant defects to other target multidegrees, pure relations, or deeper data strongly enough to force a unit or a fixed product. |
...
4. **Active word holonomy and pure-shore cancellation.** Every active
   ...
   or force the global ideal to be a unit.
