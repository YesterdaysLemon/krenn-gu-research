...
| `U7` | Force a singleton or odd signed dependency from arbitrary active-cycle response data, force and kill a low-rank binomial-core quotient, control a rank-at-least-two sheet ideal or an aggregate cycle fibre, prove cross-multiplicity coupling, or close the pure-cofactor/deeper exits. This remains **open**: no theorem forces a favourable core/rank outcome, and a proper low-rank quotient consistently imposes only the known holonomy sign. Conserved cofactor signs need not be positive. | [Torsion-sheet aggregate boundary](../claims/arbitrary-order/MATRIX_UNIT_BINOMIAL_CORE_TORSION_SHEET_AND_RANK_ONE_AGGREGATE_QUOTIENT_THEOREM.md#6-consequence-for-the-live-u7-edge), [complete-block lattice boundary](../claims/arbitrary-order/MATRIX_UNIT_COMPLETE_SAME_MULTIDEGREE_FIBRE_LATTICE_REDUCTION_AND_BINOMIAL_PARITY_DICHOTOMY_THEOREM.md#4-consequence-for-the-live-u7-edge), and [phase-holonomy and pure-cofactor boundary](../claims/arbitrary-order/MATRIX_UNIT_PHASE_HOLONOMY_AND_MINIMAL_PURE_COFACTOR_FLOW_REDUCTION_THEOREM.md#8-combined-proof-topology-consequence) |
...
4. **Active word holonomy and pure-shore cancellation.** Every active
   ...
   Laurent gcds.  What remains is to force and kill one of those favourable
   low-rank quotients, control free rank at least two or an aggregate cycle
   fibre, prove a cross-multiplicity bridge, or couple to the deeper topology.
   A least pure cancelling residual likewise has a conserved
