...
flowchart TD
  ...
  GLD71["Torus-star punctured syndrome<br/>PROVED one-word atlas / determinant-safe bridge OPEN"]
  ...
  GLD71 -->|determinant-safe three-word syndrome saturation open| GL
...
| `GLD71` | determinant-safe three-word syndrome saturation | `GL` | Prove that every kernel centre of the full `37 x 9` syndrome matrix is singular after saturating by the three leaf-frame determinants, or independently validate a genuine invertible-centre survivor.  The six root-slice rows and equal-Eisenstein-norm divisors are necessary gates, not an equivalent ten-row compression; a nonunit algebraic survivor is not a graph witness. |
...
| `GLD71` | determinant-safe syndrome obligation | `GL` | For the full `37` parity checks, prove `det A=0` on every kernel after localizing at `det B det C det D`, equivalently certify the determinant saturation.  Retain cross-root coupling: the tested ten-row compression is not equivalent.  A genuine invertible-centre nuisance point would refute only the strong fixed-space exclusion until graph-level integrability is separately proved. |
...
1. **Maximum-root same-pair target attachment.**  `GLS4` closes the
   ...
   **full-swallow** remainder universally starts at nuisance rank four.  This
   ...
   three balanced flattening ranks equal five.  The live target is now a
   radical-membership certificate for the fixed star space, followed by
   residual-coordinate-boundary and triangle atlases; none has yet been
   supplied.
   ...
   its centre is singular and epsilon is zero.  The live universal target is
   therefore determinant safety of the full `37 x 9` three-word syndrome
   kernel.  Six slice rows or a tested ten-row compression are not equivalent
   to the full syndrome; the latter produced and then rejected an apparent
   invertible-centre point.  No fixed-star exclusion has yet been proved.
