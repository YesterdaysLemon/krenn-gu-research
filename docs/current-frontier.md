...
flowchart TD
  ...
  GLS63 -->|restriction-separation / three-plus deficient / attachment open| GL
...
| `GLS56` | rigid coupling / alternate-receiver successor | `GL` | On the all-six-rigid branch, `GLS57` handles the all-rank-one cell, `GLS58` gives the exhaustive rank-profile reduction, `GLS61` excludes the complete all-injective cell, `GLS62` excludes the exactly-one-deficient cell, and `GLS63` localizes the exactly-two-deficient cell to one binary same-kernel residual.  Profiles with three or more deficient maps and the GLS63 restriction-separation residual remain open.  `GLS59` handles the unique-nonrigid source structure: old-probe exchange forces overlapping pure probe stars and closes the natural `GLD3` re-anchor because its star triangle cannot be target-diagonal.  A different legally transported receiver or complete-equation exclusion is still required, with every response, selector, synchronization, nuisance-survival, activity, and anchor gate.  For `r>=4`, neighbour rigidity and arbitrary-root receiver coverage remain separate. |
...
| `GLS63` | remaining restriction-separation successor | `GL` | For exactly two deficient labels, exclude the shared-deck triangle/four-port camouflage or prove it incompatible over the function field with the nonzero binary deficient-pair contraction.  For three or more deficient labels, continue the same mixed-kernel hierarchy without assuming that pairwise mono/binary restrictions synchronize.  Every response, selector, nuisance-survival, activity, anchor, arbitrary-root attachment, and nonzero-anchor gate remains separate. |
