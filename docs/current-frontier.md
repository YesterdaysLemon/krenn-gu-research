...
flowchart TD
  ...
  S2AX -. outer coordinate charts / other (1,1,2) / rank <=4 .-> S2
  S1 -->|boundary| S3
...
| `S2AX` | remaining common-three-space obligation | `S2` | The complete `(1,1,1)` profile and both central `(1,1,2)` coordinate-pair charts are closed.  Genuinely outer coordinate-pair charts, other `(1,1,2)` boundaries, `(1,2,2)`, and all lower or external branches remain open. |
