...
flowchart TD
  ...
  GLS61 -->|deficient ranks / attachment open| GL
...
| `GLS56` | rigid coupling / alternate-receiver successor | `GL` | On the all-six-rigid branch, `GLS57` handles the all-rank-one cell, `GLS58` gives the exhaustive rank-profile reduction, and `GLS61` excludes the complete all-injective cell; every deficient profile remains.  `GLS59` handles the unique-nonrigid source structure: old-probe exchange forces overlapping pure probe stars and closes the natural `GLD3` re-anchor because its star triangle cannot be target-diagonal.  A different legally transported receiver or complete-equation exclusion is still required, with every response, selector, synchronization, nuisance-survival, activity, and anchor gate.  For `r>=4`, neighbour rigidity and arbitrary-root receiver coverage remain separate. |
...
| `GLS61` | remaining rigid source-integrability successor | `GL` | The complete all-injective root-order-three branch is empty.  Couple the deficient mono/binary descents across kernel choices and higher open-set equations, without replacing the common physical decks by independent local tensors.  Every response, selector, nuisance-survival, synchronization, activity, anchor, arbitrary-root attachment, and nonzero-anchor gate remains separate. |
