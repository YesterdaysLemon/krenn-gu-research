...
flowchart TD
  ...
  S2BC -. seven other coloops / rank <=4 / other branches .-> S2
  S1 -->|boundary| S3
...
| `S2BC` | remaining common-three-space obligation | `S2` | The complete `(1,1,1)` and `(1,1,2)` Hilbert--Burch profiles and the distinguished `(1,2,2)` `beta_t` and `alpha_s` coloops are closed.  The other seven `(1,2,2)` coloops, joint rank at most four, other S2T component types, other S2Q strata, and every higher order remain open. |
