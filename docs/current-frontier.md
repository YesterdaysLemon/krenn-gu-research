...
flowchart TD
  ...
  GLD95 -->|E31/g0/H2/Delta exceptional factors, full H4 Q6 outside F88 / pulled-back Fitting / other branches open| GL
  GLD96 -->|E31/g0/Delta exceptional loci / H2 degree-drop handled by GLD99 / p=2 fibre handled by GLD97 / Fitting / other branches open| GL
  GLD97 -->|arbitrary p, pulled-back Fitting, other charts/components/source branches open| GL
...
| `GLD96` | In the normalized H4 offset chart on `V(Q6) intersect D(E31 H2 g0 Delta)`, four exact raw bordered seven-minors force the GLD88 offsets `B=C=0`; GLD95 then excludes the written F88 endpoint on `D(Omega)`.  The polynomial adjugate identity uses no inverse of `R31`, so `R31=0` is included wherever the remaining gates are nonzero.  This is a proved generic localization, not closure of `E31/g0/Delta`, arbitrary H4/Q6 points outside the written offset chart, the Fitting pullback, or the global conjecture; the normalized `H2=Q6=0` degree-drop residual is handled by GLD99. | [H4 Q6 R31-free generic resultant localization](../claims/arbitrary-order/FOUR_ROOT_TORUS_STAR_EQUAL_LEAF_H4_Q6_R31_GENERIC_RESULTANT_EXCLUSION_THEOREM.md), [gate-removal review](audits/FOUR_ROOT_TORUS_STAR_EQUAL_LEAF_H4_Q6_R31_GATE_REMOVAL_REVIEW_2026-08-29.md) |
...
| `GLD95` | H4 Q6 outside the written F88 family, Fitting, and remaining component/source cover | `GLD96` | Use the exact four raw bordered seven-minors and the E31/H2/g0 localization to force the written F88 offsets.  The global adjugate identity removes `R31` as a gate; retain the remaining exceptional factors. |
...
| `GLD96` | E31/g0/Delta exceptional strata, arbitrary H4 Q6 outside F88, Fitting, and remaining component/source cover | `GL` | Analyze the retained exceptional loci, compute the GLD83 pullback with full raw response incidence on `C_F` rank drops, and cover remaining gauges, components, sources, profiles, roots and orders. |
...
| `GLD95` | remaining H4 Q6 outside F88 and global Fitting/component/source-cover obligation | `GLD96` | Close the generic offset residual on `D(E31*H2*g0*Delta)` by an exact four-minor resultant witness; the polynomial bordered-determinant identity includes `R31=0` and retains the other exceptional strata. |
...
| `GLD96` | E31/g0/Delta exceptional strata, arbitrary H4 Q6 outside F88 and global Fitting/component/source-cover obligation | `GL` | Analyze the retained exceptional loci and arbitrary H4 Q6 points outside F88, compute the GLD83 Fitting pullback, and then cover other components/gauges, lower ranks, source branches, profiles, roots, orders and global resolution. |
...
| `GLD99` | arbitrary H4 Q6 outside F88, Delta=0, and global Fitting/component/source-cover obligation | `GL` | Extend beyond the normalized H2 degree-drop offset chart, compute the GLD83 Fitting pullback, and retain all other components, gauges, lower ranks, source branches, profiles, roots, orders and global resolution. |
...
1. **Maximum-root same-pair target attachment.**  `GLS4` closes the
   ...
   **full-swallow** remainder universally starts at nuisance rank four.  This
   ...
   so `R31=0` is included wherever the remaining gates are nonzero.  This is
   a localized generic statement only: `E31=0`, `g0=0`, `Delta=0`, arbitrary
   H4 Q6 points outside F88, the GLD83 Fitting pullback, and the other
   chart/component/source obligations remain open.
   ...
   from GLD96's residual but leaves `E31/g0/Delta`, `Omega=0`,
   arbitrary H4 Q6 points outside F88, `Delta=0`, the GLD83 Fitting pullback,
   other charts/components/source branches, and global resolution open.
