...
flowchart TD
  ...
  GLD104["Equal-leaf H4 Q6 a=0 P8<br/>PROVED nonzero-offset closure on D(H2*Delta)"]
  GLD105["Equal-leaf H4 Q6 a=0 physical incidence<br/>PROVED empty on D(Omega*Delta)"]
  GLD106["Equal-leaf H4 Q6 B=0 C-open<br/>PROVED arbitrary-a offset exclusion on D(Delta)"]
  ...
  GLD96 -->|B=0 cleared residual form and common-kernel specialization| GLD106
  GLD100 -->|B=0 residual gamma/fibre closure is E31-free on D(H2deg*Delta)| GLD106
  GLD99 -->|H2deg=0 arbitrary-a offset closure on D(Delta)| GLD106
  ...
  GLD101 -->|eight-factor P8 composition; selected equations imply the exact norm gate| GLD104
  GLD102 -->|p=0,1 selected-minor subcases at a=0| GLD104
  GLD104 -->|H2deg-open offset closure and written-F88 identification| GLD105
  GLD99 -->|H2deg=0 physical-incidence exclusion at arbitrary a, hence a=0| GLD105
  GLD95 -->|fully qualified written-F88 physical endpoint exclusion| GLD105
  GLD86 -->|incidence equations plus rank(A)<=6 imply full syndrome rank<=6| GLD105
  ...
  GLD100 -->|after GLD106, the E31=0 nonzero-offset wall remains only on D(B_offset*H2deg*Delta); Delta / physical endpoint / wider obligations open| GL
  GLD101 -->|six-selector P6 without D0,Y0 remains open independently of the P8 rank route| GL
  GLD102 -->|outside the a=0 P8 composition, arbitrary-p/arbitrary-a and wider obligations open| GL
  GLD104 -->|P6 / arbitrary a / full E31 / Delta boundary / wider-chart, Fitting and global obligations open; scoped physical endpoint composed in GLD105| GL
  GLD105 -->|Omega=0 / Delta=0 / arbitrary a / full E31 / outside-F88 / Fitting / other charts, components, sources, roots, orders, and global gluing open| GL
  GLD106 -->|remaining D(B_offset) E31 wall / physical endpoint / Delta / Omega / outside-F88 / Fitting / wider and global obligations open| GL
...
| `GLD104` | On the normalized GLD88/F88 H4/Q6 offset chart over `C`, with `a=0` and `D(H2*Delta)`, the eight actual-minor equations `T0,T1,T2,T3,D0,Y0,Y1,X3` force `B=C=0`.  At a nonzero offset the six GLD101 equations give a nonzero monomial vector in the kernel of their exact coefficient matrix, hence the pinned selector determinant and eight-factor norm support vanish.  The exhaustive charts `D(B)` and `V(B) intersect D(C)` are then closed by the arbitrary-p C-open unit and the exact GLD102, `p^2+1`, `R4`, `R8`, and `R110` leaves.  Complete syndrome rank at most six supplies P8 only in the forward direction.  P6, the `B=C=0` endpoint, physical incidence, arbitrary `a`, full E31, `Delta=0`, `H2=0`, wider charts/components/source branches, Fitting/source-integrability/global gluing, and global resolution remain open; the global status remains **UNRESOLVED**. | [a=0 P8 nonzero-offset closure](../claims/arbitrary-order/FOUR_ROOT_TORUS_STAR_EQUAL_LEAF_H4_Q6_A0_P8_NONZERO_OFFSET_CLOSURE_THEOREM.md), [hostile review](audits/FOUR_ROOT_TORUS_STAR_EQUAL_LEAF_H4_Q6_A0_P8_NONZERO_OFFSET_CLOSURE_REVIEW_2026-08-31.md) |
| `GLD105` | Over `C`, on the normalized scale-fixed GLD88/F88 equal-leaf H4 offset chart, `B_incidence intersect V(I_7(A)) intersect H4 intersect V(a,Q6) intersect D(Omega Delta)` is empty.  GLD75/GLD86 turn physical incidence plus `rank(A)<=6` into full syndrome rank at most six.  On `H2deg=2p^2-2p+1!=0`, GLD104 forces the affine offsets to zero and GLD95 excludes the resulting written-F88 physical endpoint with all incidence qualifiers retained; on `H2deg=0`, GLD99 supplies the physical exclusion for arbitrary `a`.  The split is exhaustive and does not confuse `H2deg` with GLD86's collision divisor `p-s=L1/(p+q-1)`.  P6, `Omega=0`, `Delta=0`, arbitrary `a`, the full `E31=0` wall, outside-F88 H4/Q6, the GLD83 Fitting pullback, other charts/components/source branches, roots/orders, global gluing, and global resolution remain open; the global status remains **UNRESOLVED**. | [a=0 physical-incidence exclusion](../claims/arbitrary-order/FOUR_ROOT_TORUS_STAR_EQUAL_LEAF_H4_Q6_A0_PHYSICAL_INCIDENCE_EXCLUSION_THEOREM.md), [hostile review](audits/FOUR_ROOT_TORUS_STAR_EQUAL_LEAF_H4_Q6_A0_PHYSICAL_INCIDENCE_EXCLUSION_REVIEW_2026-08-31.md) |
| `GLD106` | Over `C`, on the normalized scale-fixed GLD88/F88 equal-leaf H4 offset chart with arbitrary `a`, `V(B_offset,Q6) intersect D(C_offset*Delta) intersect {rank M(G)<=6}` is empty.  On `H2deg!=0`, full syndrome rank at most six makes the four actual GLD96 seven-minors vanish; at `B_offset=0`, the cleared residual form and `C_offset!=0` force all four primitive gammas to vanish, while the exact GLD100 necessary eight-factor cover plus every pointwise fibre closure excludes that common zero without using `E31`.  GLD99 supplies the separate `H2deg=0` branch for arbitrary `a`.  The split is exhaustive and uses selected minors only forward.  This removes only the `B_offset=0,C_offset!=0` algebraic chart: the remaining `D(B_offset)` E31 wall, physical endpoint, `Delta=0`, `Omega=0`, outside-F88, Fitting, wider-source, roots/orders, gluing, and global resolution remain open; the global status remains **UNRESOLVED**. | [B=0 C-open offset exclusion](../claims/arbitrary-order/FOUR_ROOT_TORUS_STAR_EQUAL_LEAF_H4_Q6_B0_COPEN_OFFSET_EXCLUSION_COROLLARY.md), [hostile review](audits/FOUR_ROOT_TORUS_STAR_EQUAL_LEAF_H4_Q6_B0_COPEN_OFFSET_EXCLUSION_REVIEW_2026-08-31.md) |
...
| `GLD101` | completed a=0 P8 residual-factor parent composition | `GLD104` | The selected-equation coefficient-matrix bridge feeds the exact eight-factor norm cover; GLD102, the arbitrary-p C-open unit, and the `p^2+1`, R4, R8, and R110 B-open leaves exhaust every nonzero-offset support. |
| `GLD102` | completed p=0,1 input to the a=0 P8 composition | `GLD104` | At `a=0`, the p=0 selected B-open basis contains `a-1`, the p=1 sixth-selector remainder is coprime to the residual quadratic, and both C-open ideals are already covered by the arbitrary-p leaf. |
| `GLD104` | completed a=0 physical-incidence parent composition | `GLD105` | Compose the forward GLD75/GLD86 rank bridge, the GLD104 `H2deg!=0` offset closure, the fully qualified GLD95 F88 endpoint, and the GLD99 `H2deg=0` physical exclusion. |
| `GLD105` | remaining wall, boundary, wider-chart, and global obligations | `GL` | Address arbitrary-`a` physical incidence, the remaining `D(B_offset)` part of the `E31=0` wall after GLD106, `Delta=0`, `Omega=0`, other normalized pivot patches, the GLD83 Fitting pullback, wider charts/components/source branches, roots, orders, and global gluing.  P6 remains open but is not required by the forward rank-to-P8 route. |
| `GLD100` | completed arbitrary-a `B_offset=0,C_offset!=0` composition | `GLD106` | Start after `B_offset=0`: the four GLD96 residual equations force the primitive gamma common zero on `D(C_offset*H2deg*Delta)`, and GLD100's exact pair/fibre core closes it without `E31`; GLD99 supplies `H2deg=0`. |
| `GLD106` | remaining normalized nonzero-offset E31 wall | `GL` | Prove the pointwise `V(E31,Q6) intersect D(B_offset*H2deg*Delta) intersect {rank M(G)<=6}` locus empty, then retain the physical endpoint, `Delta=0`, `Omega=0`, outside-F88, Fitting, wider-source, root/order, lifting, and gluing obligations. |
| `GLD100` | completed normalized E31-open parent synthesis | `GL` | The exact pair-resultant eight-factor cover and strict fibre closures force `B=C=0` on `D(E31*H2*Delta)`; GLD99 supplies `H2=0`, and GLD95 excludes the F88 incidence on `D(Omega)` through `C_8=1`.  GLD106 separately removes the `B_offset=0,C_offset!=0` part of the E31 wall.  Retain its `D(B_offset)` branch, `Delta=0`, physical endpoint and `Omega=0`, arbitrary H4 Q6 outside the normalized F88-offset chart, the GLD83 Fitting pullback, other charts/components/source branches, roots, orders, and global resolution. |
...
1. **Maximum-root same-pair target attachment.**  `GLS4` closes the
   ...
   **full-swallow** remainder universally starts at nuisance rank four.  This
   ...
   GLD104 and GLD105 now finish the exact `a=0` physical composition on this
   normalized chart without upgrading any wider claim.  On `D(H2deg*Delta)`,
   GLD104 exhausts the eight-factor nonzero-offset support and forces the two
   affine offsets to zero from full syndrome rank at most six.  Physical
   incidence plus `rank(A)<=6` supplies that rank condition through the exact
   GLD75/GLD86 bridge, and GLD95 excludes the resulting written-F88 endpoint
   on `D(Omega*Delta)`.  GLD99 independently excludes the complementary
   `H2deg=0` physical branch for arbitrary `a`, so the two cases give the
   GLD105 empty-set statement at `a=0`.  Here `H2deg=2p^2-2p+1` is distinct
   from GLD86's collision divisor `p-s=L1/(p+q-1)`.  P6, arbitrary `a`, the
   full `E31=0` wall, `Delta=0`, `Omega=0`, outside-F88 H4/Q6, the GLD83
   Fitting pullback, other charts/components/source branches, roots/orders,
   global gluing, and global resolution remain open.

   GLD106 sharpens the arbitrary-`a` normalized E31 wall without changing
   that physical or global boundary.  If `B_offset=0` and `C_offset!=0`, full
   syndrome rank at most six makes the four actual GLD96 bordered minors
   vanish.  Their cleared residual form then forces the four primitive gamma
   coefficients to vanish on `D(H2deg*Delta)`, and the exact GLD100
   pair-resultant cover plus all pointwise fibre closures excludes the point;
   this post-`B=0` core does not use `E31`.  GLD99 handles `H2deg=0` for
   arbitrary `a`, so the binary split proves the C-open corollary on
   `D(Delta)`.  The genuinely new normalized E31 computation is therefore
   confined to `D(B_offset*H2deg*Delta)`.  That D(B) locus is still open, as
   are the physical endpoint, `Delta=0`, `Omega=0`, outside-F88, Fitting,
   wider-source, and global obligations.
