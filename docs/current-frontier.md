...
flowchart TD
  ...
  GLD85 -->|rank-eight residual V(I_Pl) and remaining chart/component coverage open| GL
  GLD86 -->|three collision divisors / exact determinant-safe successor| GLD87
  GLD87 -->|H4 named principal-open successor| GLD88
  GLD88 -->|H4 pivot/coefficient boundaries / pulled-back Fitting / other branches open| GL
...
| `GLD88` | H4 exceptional-boundary/Fitting and remaining component/source cover | `GL` | Analyze the parameter, linear-coefficient and six-pivot boundaries inside `H_4 intersect D(Omega)`, compute the `GLD83` pullback there while retaining full raw response incidence on `C_F` rank drops, and then cover the remaining gauges, components, sources, profiles, roots and orders. |
...
| `GLD85` | remaining pulled-back Fitting/component/source-cover obligation | `GL` | Decide whether the rank-eight pullback of `I_Pl` is unit, has additional components, or leaves a residual; then compute the other rank-eight charts and the Gaussian rank-seven and lower-rank branches.  Other survivor components/gauges, triangles, residual-coordinate and isotropic boundaries, smaller survivor families, non-star profiles, other root orders, and global resolution remain open. |
...
| `GLD88` | remaining H4-boundary/Fitting/component/source-cover obligation | `GL` | Analyze the explicit parameter, coefficient and pivot boundaries in `H_4 intersect D(Omega)`, retaining full raw response incidence on `C_F` rank drops; then cover other components/gauges, lower ranks, source branches, profiles, roots, orders and global resolution. |
