...
flowchart TD
  ...
  GLS57["All-rank-one rigid colour pairing<br/>PROVED 2+2+2 pure companions / promoted response polynomial"]
  ...
  GLS56 -->|higher-rank rigid coupling / different receiver / arbitrary-r attachment open| GL
  GLS57 -->|complete nuisance survival / mixed-deck receiver open| GL
...
| `GLS56` | rigid coupling / alternate-receiver successor | `GL` | On the all-six-rigid branch, use complete mixed/deck equations beyond the exact pure-axis, anti-synchronization, and full-rank off-diagonal controls; `GLS57` now refines but does not close its all-rank-one subbranch, while every higher-rank profile remains here.  On the unique-nonrigid branch, the natural `h=0` `GLD3` re-anchor has zero target-diagonal star triangle and fails activity, so exclude that low-activity cell or construct a different legally transported receiver with every response, selector, synchronization, nuisance-survival, activity, and anchor gate.  For `r>=4`, neighbour rigidity and arbitrary-root receiver coverage remain separate. |
...
| `GLS57` | rank-one rigid receiver successor | `GL` | Decide complete-nuisance survival and the remaining mixed-deck cancellations, or force one old-probe four-label window with six diagonal pair responses plus three-colour activity, or legally contract/splice the six-mode first variation to a weighted `P_6` restriction.  The original eight-vertex equation is already physical; the missing step is this six-mode reduction.  Higher joint ranks, the unique-nonrigid branch, every response divisor, and arbitrary-root coverage remain separate. |
...
1. **Maximum-root same-pair target attachment.**  `GLS4` closes the
   ...
   **full-swallow** remainder universally starts at nuisance rank four.  This
   ...
   retained on a common torus open with the two `GLS4` gates.  Pointwise
   response divisors, complete-nuisance survival, legal selector normalization,
   six-row synchronization, `GLD3` activity, higher-rank rigid maps, the
   unique-nonrigid receiver, and arbitrary-root coverage remain open.
