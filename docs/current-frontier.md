...
| `GLS17` | remaining leading-absorption and activity obligation | `GL` | Force leading survival for a sufficient even-target family, or contradict the simultaneous partial-root absorptions by complete mixed GHZ coefficients.  At `r=4`, a surviving four-port first-root class still supplies a legal pure-`M` companion row, but `GLD67` proves that this alone gives no cross-Gram control of the direct port blocks.  At least one swallowed pair base class remains a separate branch.  The theorem neither proves source survival nor integrates `GLS8`. |
