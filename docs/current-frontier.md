...
flowchart TD
  ...
  GLS74 -->|central root-axis / outside endpoint integrability| GL
...
| `GLS73` | coupled off-port source-integrability obligation | `GL` | On Family A `r=1`, change at least one off-port core edge while preserving the GLS72 kernel restrictions, and classify the extra full-deck channels that can cancel the two GLS73 coefficient rows.  Treat Family A `r=2,3`, the GLS74-retained central-root-axis and endpoint Family-B `r=3` charts, every pure/zero-triangle six-deficient profile, and both five-deficient residuals separately.  Resolve the unique-nonrigid/alternate-receiver branch plus every response, selector, nuisance-survival, activity, anchor, arbitrary-root attachment, and nonzero-anchor gate independently. |
| `GLS74` | remaining all-`T_0` Family-B source-integrability obligation | `GL` | Recover a missing Koszul row when one central probe shore is root-axis-only, or use another complete coefficient to exclude the resulting coupled kernel; separately resolve every P/Q-common outside endpoint kernel.  These retained charts keep the typed key and residual count open. |
