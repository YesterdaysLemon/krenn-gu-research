...
flowchart TD
  ...
  GLD80 -->|explicit base divisor / other components / source bridge open| GL
...
| `GLD80` | explicit divisor and source-presentation globalization | `GL` | Eliminate the `s`-saturated projective incidence to exhibit a base polynomial `delta(F)` nonzero at `F_0`, or give a finite exact divisor cover.  Then cover other survivor components/gauges or prove that every relevant source presentation is forced through the excluded effective interface.  Maximum-root/no-fifth-root certification, other roots, non-star atlases, and global coverage remain separate. |
...
| `GLD80` | remaining explicit-divisor/source-interface obligation | `GL` | Compute an explicit survivor-only exceptional polynomial from the saturated elimination ideal, or a finite exact base-divisor cover.  Then cover other survivor components/gauges or establish a source bridge forcing every relevant presentation through this interface.  Maximum-root/no-fifth-root certification, non-star and lower-rank profiles, other root orders, and global resolution remain open. |
...
1. **Maximum-root same-pair target attachment.**  `GLS4` closes the
   ...
   **full-swallow** remainder universally starts at nuisance rank four.  This
   ...
   existential: the base polynomial is not expanded.  Computing that
   divisor, covering other survivor components/gauges, and proving the
   source/interface bridge remain the smallest local-to-global obligations.
