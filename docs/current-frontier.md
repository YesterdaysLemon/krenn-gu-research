...
flowchart TD
  ...
  GLD104 -->|B=C=0 endpoint / P6 / arbitrary a / full E31 / Delta and H2 boundaries / physical, wider-chart, Fitting and global obligations open| GL
...
| `GLD104` | remaining endpoint, wall, and global obligation | `GL` | Address the `B=C=0` endpoint and physical incidence bridge, arbitrary `a` and full `E31=0`, `Delta=0` and `H2=0`, other normalized pivot patches, the GLD83 Fitting pullback, wider charts/components/source branches, roots, orders, and global gluing.  P6 remains open but is not required by the forward rank-to-P8 route. |
