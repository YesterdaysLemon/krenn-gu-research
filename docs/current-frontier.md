...
flowchart TD
  ...
  O3["q=0 r=5 two degenerate or >=3 local defects;<br/>q=0 r>=6, q>=1, or unfactorized OPEN"]
...
| `O3` | In aligned projective `q=0,r=5`, the remaining detector boundary has either two local defects that are both one-sided/zero, or at least three local defects. Every `q=0,r>=6` cell, every `q>=1` cell, and every unfactorized outside graph is also **open**. | [Rank-one-mode exact boundary](../claims/arbitrary-order/PROJECTIVELY_CONSTANT_LIFT_RANK_ONE_MODE_AND_REGULAR_TWO_DEFECT_FIVE_CELL_DETECTOR_THEOREM.md#5-exact-residual-boundary) and [two-open exact boundary](../claims/arbitrary-order/BALANCED_TWO_OPEN_ROOT_GAUGE_DETECTOR_AND_STAR_INVISIBILITY_BOUNDARY.md#6-exact-boundary) |
...
| `O2D` | boundary obligation | `O3` | The rank-one-mode argument leaves two one-sided/zero defects and every three-or-more-defect pattern, and does not transport automatically to larger aligned cells, positive surplus, or the unfactorized branch. |
...
3. **Remaining larger/unfactorized detector.** On the aligned projectively
   constant branch, close the residual `q=0,r=5` patterns with either two
   one-sided/zero local `a/b` defects or at least three local defects.  The
   transverse cell, one arbitrary defect, and regular two-defect strata are
   now detected.  Then treat `q=0,r>=6` or `q>=1`, or prove a legal selector
