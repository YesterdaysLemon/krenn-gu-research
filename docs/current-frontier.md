...
flowchart TD
  ...
  GLS72 -->|transverse coefficient / remaining shared-deck integrability| GL
...
| `GLS72` | transverse coefficient and remaining shared-deck integrability obligation | `GL` | On Family A `r=1`, couple the surviving `alpha=a=b=0` common-edge control to a legal same-source coefficient that sees `e_(5,0)`, or prove an exact integrability bridge from `W_25|_(K_5)` to its transverse value.  Treat Family A `r=2,3`, the all-`T_0` Family B `r=3` quotient, every pure/zero-triangle six-deficient profile, and both five-deficient residuals separately.  Resolve the unique-nonrigid/alternate-receiver branch plus every response, selector, nuisance-survival, activity, anchor, arbitrary-root attachment, and nonzero-anchor gate independently. |
