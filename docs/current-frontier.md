...
flowchart TD
  ...
  GLD78 -->|boundary outside sign plane / source bridge open| GL
...
| `GLD78` | remaining universal response/source-interface obligation | `GL` | Classify and cover the projective rank-one boundary with trivial or standard raw components, including mixed isotypic supports.  Combine an exhaustive cover with the three `GLD78` opens before lifting `GLD74` to a whole-survivor principal open.  Then cover other survivor components or establish a source bridge forcing every relevant presentation through this interface.  Maximum-root/no-fifth-root certification, non-star and lower-rank profiles, other root orders, and global resolution remain open. |
