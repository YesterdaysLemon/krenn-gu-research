...
flowchart TD
  ...
  GLD76 -->|sign-boundary strict transform / global components / source bridge open| GL
...
| `GLD76` | remaining universal response/source-interface obligation | `GL` | Blow up the homogenized incidence along the two named sign directions, decide every strict-transform branch, and cover the rest of the projective boundary before lifting the `GLD74` certificate over a principal survivor open.  Then cover other survivor components or establish a source bridge forcing every relevant presentation through this interface.  Maximum-root/no-fifth-root certification, non-star and lower-rank profiles, other root orders, and global resolution remain open. |
