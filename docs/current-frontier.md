...
flowchart TD
  ...
  GLS62 -->|at least two deficient / synchronization and attachment open| GL
...
| `GLS56` | rigid coupling / alternate-receiver successor | `GL` | On the all-six-rigid branch, `GLS57` handles the all-rank-one cell, `GLS58` gives the exhaustive rank-profile reduction, `GLS61` excludes the complete all-injective cell, and `GLS62` excludes the exactly-one-deficient cell; profiles with at least two deficient maps remain.  `GLS59` handles the unique-nonrigid source structure: old-probe exchange forces overlapping pure probe stars and closes the natural `GLD3` re-anchor because its star triangle cannot be target-diagonal.  A different legally transported receiver or complete-equation exclusion is still required, with every response, selector, synchronization, nuisance-survival, activity, and anchor gate.  For `r>=4`, neighbour rigidity and arbitrary-root receiver coverage remain separate. |
...
| `GLS62` | remaining rigid source-integrability successor | `GL` | The no-deficient and exactly-one-deficient root-order-three cells are empty.  Characterize the common kernel of the one-, two-, and higher-deficient same-source equations and their shared-edge polarizations; independent zero/mono/binary contractions do not synchronize themselves.  Every response, selector, nuisance-survival, activity, anchor, arbitrary-root attachment, and nonzero-anchor gate remains separate. |
