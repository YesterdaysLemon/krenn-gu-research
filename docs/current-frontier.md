...
flowchart BT
  ...
  PSCT -->|higher exterior cuts or third-color coupling are still needed| GL
  PSCL -->|internal cycle with exterior-varying targets remains open| GL
  PSCS -->|full binary source parent and arbitrary fillings remain open| GL
...
| `PSCS` | boundary | `GL` | Every common-star port must have at least two edges; each unordered color pair needs a port of size at least three and a noncomplete binary support component. The direct full-binary-source parent FB remains open after the RCS/BS supply failures. The original two-pair allocation, arbitrary hollow protected fillings, Q4 and arbitrary-source coverage remain open. |
