...
flowchart BT
  ...
  PSCG["Complete protected-edge pair gadgets<br/>PROVED full-source exclusion via cubic matchings"]
...
| `PSCG` | **Proved full construction-family exclusion over C:** pair all protected colored K4 edges across different colors and different components, use one endpoint bijection per pair with two arbitrary nonzero weights, and add no other crossings. The color-state graph partitions into C4s; its auxiliary graph is simple, triangle-free, cubic and properly three-edge-colored. Perfect matchings biject with empty/adjacent-pair physical words, each with a unique original matching. The imported three-perfect-matchings classification forces an extra matching, excluding the entire family from full GHZ. Product -1 in every gadget makes the correspondence a bijection onto all nonzero coefficients. No component bipartition is needed. The proof supplies no paired-depth bound; WP1 and arbitrary protected arrays remain open. | [Cubic matching exclusion](../claims/arbitrary-order/PROTECTED_PAIR_GADGET_CUBIC_MATCHING_EXCLUSION.md), [independent review](audits/PROTECTED_PAIR_GADGET_CUBIC_REVIEW_2026-09-22.md), [review brief](strategy/pair-gadget-cubic-review-2026-09-22.md) |
