...
flowchart BT
  ...
  PSCS["Protected common-star source coupling<br/>PROVED triangles and repeated ports are necessary"]
  ...
  PSCS -->|repeated triangle resources and general fillings remain open| GL
...
| `PSCS` | **Proved all-order source obstructions over C:** common-centre gadgets, with one unequal-colour label per component edge and two nonzero factors per gadget, satisfy q_uv = F_uv - F_u F_v at any edge with no common neighbour. The component targets therefore require a triangle at every edge. They also require local ordered-port repetition: three specified unique ports at an oriented edge force a mixed source of -1 or -2. Thus triangle-free constructions at arbitrary degrees and unique-port constructions with arbitrary triangles are both excluded. The exact general component source is a sum of products of two hafnians over a shared subset; ordinary matching specialization requires a forest. Independent physical replays expose the cycle, triangle and multiple-label boundaries. General common-star fillings, arbitrary protected fillings and Q4 remain open. | [Common-star source obstruction](../claims/arbitrary-order/PROTECTED_SCAFFOLD_COMMON_STAR_COMPONENT_TARGET_OBSTRUCTION.md), [independent review](audits/PROTECTED_SCAFFOLD_COMMON_STAR_REVIEW_2026-09-14.md), [Q4 parent attempt](strategy/scaffold-q4-parent-attempt-2026-09-14.md) |
...
| `G0` | conditional source identity | `PSCS` | Under the common-star physical support hypotheses, the same full source gives q_uv = F_uv - F_u F_v at each component edge outside all triangles. A hypothetical full target supplies the three vanishing coefficients. |
| `PSCS` | boundary | `GL` | Common-star fillings must have both triangles and the specified repeated endpoint ports. Their interacting resources, the original two-pair allocation and arbitrary hollow protected fillings are outside these exclusions; Q4 and arbitrary-source coverage remain open. |
