...
flowchart TD
  ...
  S2AV -. double-repeated intersection / outer coordinate charts / rank <=4 .-> S2
  S1 -->|boundary| S3
...
| `S2AV` | remaining common-three-space obligation | `S2` | The complete `(1,1,1)` profile and every distinct-colour central `(1,1,2)` point except the simultaneous double-repeated outer intersection are closed.  That intersection, same-colour and genuinely outer coordinate charts, `(1,2,2)`, and all lower or external branches remain open. |
