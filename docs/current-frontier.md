...
flowchart TD
  ...
  S2AN -. complementary-plane / other Hilbert--Burch boundaries .-> S2
  S1 -->|boundary| S3
...
| `S2AN` | remaining common-three-space obligation | `S2` | The repeated-coordinate `(1,1,1)` chart survives only when the third factor lies in the complementary coordinate plane.  That boundary, the other `(1,1,1)` charts, the other Hilbert--Burch profiles, and all lower or external branches remain open. |
