...
flowchart BT
  ...
  PSRA -->|one binary decomposition supply BS remains open| PSCS
  PSCS -->|BS supply and arbitrary fillings remain open| GL
...
| `PSCS` | **Proved all-order source obstructions over C:** common-centre gadgets, with one unequal-colour label per component edge and two nonzero factors per gadget, obey an exact shared-neighbour identity retaining both hafnians and four-cycle interference. Singleton and both double-component targets exclude every unique port. The full component target is impossible whenever just one binary state graph is a union of complete bicliques: singleton sums balance the blocks, double rows exclude size one, and a directed binary-resource cycle gives a mixed word with nonzero star-factor product. This excludes the family in which the ports for just one unordered color pair all have size at most two, with the other degrees unrestricted. Earlier complete-tripartite and all-port-at-most-two results remain valid. The stronger tripartite supply RCS is refuted at PSRA; the weaker binary supply BS, arbitrary protected fillings and Q4 remain open. | [Common-star source obstruction](../claims/arbitrary-order/PROTECTED_SCAFFOLD_COMMON_STAR_COMPONENT_TARGET_OBSTRUCTION.md), [two-port review](audits/PROTECTED_SCAFFOLD_COMMON_STAR_TWO_PORT_REVIEW_2026-09-14.md), [binary-resource review](audits/COMMON_STAR_RESOURCE_ALIGNMENT_REVIEW_2026-09-14.md), [parent attempt](strategy/common-star-q4-parent-attempt-2026-09-14.md) |
| `PSRA` | **Proved exact mechanism no-go over Q(sqrt(3)):** S1, both S2 families and every full physical four-minority source do not force the common-star state graph to split into complete tripartite resources. A finite SL(2,F5) realization has 720 protected K4 components, 10080 legal gadgets and 120 connected noncomplete state resources. The exact local identities assemble through unique state-resource membership; an exhaustive crossing-cycle classification and all 8640 coherent component triangles prove full U4. The literal 2880-vertex scalar failure has source 2^(-360), so this is not a CSQ4 or global counterexample. Each binary graph is nevertheless a biclique union, leaving the strictly weaker sufficient supply BS open. | [Resource-alignment no-go](../claims/arbitrary-order/PROTECTED_SCAFFOLD_COMMON_STAR_RESOURCE_ALIGNMENT_NO_GO.md), [independent review](audits/COMMON_STAR_RESOURCE_ALIGNMENT_REVIEW_2026-09-14.md), [parent attempt](strategy/common-star-q4-parent-attempt-2026-09-14.md) |
...
| `PSRA` | open sufficient supply | `PSCS` | Binary-resource exclusion needs only one color-pair graph to be a union of complete bicliques. The exact countercontrol satisfies this weaker conclusion for every pair; whether S1+bothS2+U4 always supply at least one such pair (BS) remains open. This arrow records an obligation, not a proved reduction. |
| `PSCS` | boundary | `GL` | Every common-star port must have at least two edges; each unordered color pair needs a port of size at least three and a noncomplete binary support component. A single complete binary decomposition would contradict the full component target, but the sufficient supply BS remains open. The original two-pair allocation, arbitrary hollow protected fillings, Q4 and arbitrary-source coverage remain open. |
