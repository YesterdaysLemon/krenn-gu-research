...
flowchart TD
  ...
  S2AT -. central coloops / outer coordinate charts / rank <=4 .-> S2
  S1 -->|boundary| S3
...
| `S2AT` | remaining common-three-space obligation | `S2` | The complete `(1,1,1)` profile is closed.  The central-colour coloop orbit and outer coordinate boundaries remain in `(1,1,2)`; `(1,2,2)` and all lower or external branches remain open. |
