...
flowchart TD
  ...
  S2R -. coordinate-boundary annihilators remain .-> S2
  S1 -->|boundary| S3
...
| `S2` | Prove every realized full-sensor target incidence fails normalization, one of the retained projective pair target-column-span identities, or an Euler--hafnian recurrence.  S2P excludes the eight ambient coordinatewise controls; S2Q localizes every `m=3` pair pole to three low-span singleton strata; S2R forces every product annihilator of those physical singleton spans onto the coordinate boundary.  The boundary strata and all higher orders remain open: **open** | [S2P exact boundary](../claims/arbitrary-order/BALANCED_FULL_SENSOR_COMMON_SHORE_BINARY_SYZYGY_PERMANENT_RESIDUAL_OBSTRUCTION_THEOREM.md#7-proof-topology-consequence), [`m=3` pole localization](../claims/arbitrary-order/BALANCED_M3_FULL_SENSOR_SEPARATED_SINGLETON_POLE_LOCALIZATION_THEOREM.md#6-proof-topology-consequence), [torus-annihilator boundary](../claims/arbitrary-order/BALANCED_M3_SINGLETON_SPAN_TORUS_ANNIHILATOR_PERMANENT_RANK_OBSTRUCTION.md#6-proof-topology-consequence) |
...
| `S2R` | boundary-annihilator obligation | `S2` | Product annihilators may still exist on coordinate hyperplanes.  Their zero, binary, and decomposable `P_3` restrictions must be classified against the physical shared-factor map; no higher-order conclusion is inferred. |
