...
flowchart TD
  ...
  S2AS -. ordinary coloops / outer coordinate charts / rank <=4 .-> S2
  S1 -->|boundary| S3
...
| `S2AS` | remaining common-three-space obligation | `S2` | The complete `(1,1,1)` profile is closed.  Four ordinary-coloop orientations and outer coordinate boundaries remain in `(1,1,2)`; `(1,2,2)` and all lower or external branches remain open. |
