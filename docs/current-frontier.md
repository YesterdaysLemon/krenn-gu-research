...
flowchart TD
  ...
  GLD65["First-root product selector<br/>PROVED pure-three camouflage exclusion"]
  GLD66["Product-selector response anchor<br/>PROVED at-most-one-colour / plane synchronization"]
  ...
  GLS16 -->|if all pair base classes survive: common pure-M pair line| GLD16
  GLS17 -->|pair shadows plus four-port first-root shadow survive| GLD16
  ...
  GLS17 -->|first-root product selector / complete foreign labels| GLD65
  GLS18 -->|three pure first-root columns imply full M response| GLD65
  GLD65 -->|response-anchor two-set equations / common dim<=3 space| GLD66
  GLS18 -->|monocolour target coupling / exact first-root shadows| GLD66
  ...
  GLD66 -->|all-full or synchronized-plane profiles / other roots open| GL
...
| `GLD65` | A surviving `GLS17` four-port first-root class supplies a legal pure-`M` selector which evaluates the four root slots at one product vector.  Its complete foreign-label equations express every direct port block as a cross-Gram block of one bilinear form on the common four-dimensional root-incidence space.  If all six direct blocks are diagonal and `C(B)` is pure with three nonzero coefficients, the mixed `2+2` rows force the unique three-matching camouflage support up to six colour permutations.  That support isolates five independent incidence vectors, impossible in dimension four.  Hence the all-six-pair-shadow plus full-three first-root-response branch is empty, including every incidence-rank and nuisance-rank-drop fibre.  This is a **proved product-selector three-colour camouflage exclusion**, not leading-survival, response-nonvanishing, at-most-two-colour, arbitrary-selector, other-root, promoted-source, or permanent forcing. | [Product-selector camouflage exclusion](../claims/arbitrary-order/FIXED_Q_FOUR_ROOT_PRODUCT_SELECTOR_THREE_COLOUR_CAMOUFLAGE_EXCLUSION_THEOREM.md), [hostile review](audits/FIXED_Q_FOUR_ROOT_PRODUCT_SELECTOR_THREE_COLOUR_CAMOUFLAGE_EXCLUSION_REVIEW_2026-08-23.md) |
| `GLD66` | The same product selector's foreign `{q_0,u}` and `{q_1,u}` equations put every root-to-port incidence vector in a common subspace `W<=K^4` of dimension at most three, retaining proportional anchor functionals and every rank-drop fibre.  Combined with the `GLD65` cross-pairing identity, six diagonal direct blocks and pure `C(B)` permit at most one nonzero pure coefficient.  On the all-six-pair-base-survival witness branch, any proper first-root shadow therefore forces a nonzero monocolour `M_U`; every proper first-root nuisance is exactly the coordinate plane missing that shared colour and every other shadow is full.  This is a **proved product-selector two-colour exclusion and first-root plane synchronization theorem**, not survival forcing, exclusion of either remaining shadow profile, arbitrary-selector or arbitrary-root coverage, promoted-source integration, or permanent forcing. | [Two-colour exclusion and plane synchronization](../claims/arbitrary-order/FIXED_Q_FOUR_ROOT_PRODUCT_SELECTOR_TWO_COLOUR_EXCLUSION_AND_FIRST_ROOT_PLANE_SYNCHRONIZATION_THEOREM.md), [hostile review](audits/FIXED_Q_FOUR_ROOT_PRODUCT_SELECTOR_TWO_COLOUR_EXCLUSION_AND_FIRST_ROOT_PLANE_SYNCHRONIZATION_REVIEW_2026-08-23.md) |
...
| `GLS17`, `GLD15` | factored four-port selector and complete foreign-label restriction | `GLD65` | A surviving four-port first-root class has a separating functional which is evaluation at one open-root vector, hence gives a product evaluation on all four roots.  Its zero `Z` coefficient and every complete nuisance label reduce each `Q union {u,v}` matching coefficient to `B_uv` plus one common four-linear root-incidence permanent. |
| `GLD65`, `GLD15` | product-selector response-anchor refinement | `GLD66` | The foreign two-set labels `{q_0,u}` and `{q_1,u}` annihilate every port-incidence vector under two response-anchor functionals.  Their nonzero mutual response confines all port incidences to one common space of dimension at most three, while the `Q union {u,v}` labels retain the common cross-pairing form. |
| `GLS18`, `GLD66` | two-colour exclusion and first-root shadow synchronization | `GL` | With all six pair base shadows surviving, any surviving four-port first-root class makes `M_U` nonzero and pure.  GLD66 forces it to one colour; each first-root nuisance shadow is then either full or exactly the coordinate plane missing that shared colour.  Excluding both the all-full and synchronized-plane profiles remains open. |
...
| `GLS16` | remaining base-circuit and package obligation | `GL` | Force a sufficient family of base pair classes to survive, or exclude every swallowed-base circuit forced by rank zero, pure-`Z`, oblique, or unequal pair lines using complete mixed GHZ coefficients.  The foreign cross-annihilation maps are exact additional equations but are not transport membership.  Four-port synchronization, activity, exceptional fibres, and the distinct GLS8 promoted interface remain. |
| `GLS17` | remaining leading-absorption and activity obligation | `GL` | Force leading survival for a sufficient even-target family, or contradict the simultaneous partial-root absorptions by complete mixed GHZ coefficients.  At `r=4`, on the all-six-pair-survival branch GLD66 reduces the four first-root shadows exactly to either four full shadows or one shared monocolour response with each shadow full or the corresponding coordinate plane.  At least one swallowed pair base class remains a separate branch.  Neither theorem proves source survival or integrates GLS8. |
| `GLS18` | remaining target-coupled failure-locus obligation | `GL` | Exclude the simultaneous geometric radical--Fitting containments for a sufficient target family on the complete witness locus, or exhibit a shared residual point with useful leading shadows and all downstream activity gates.  At `r=4`, GLD66 now supplies exact cross-root coupling on the all-six-pair-survival branch: all four shadows are full, or all proper shadows miss one common physical colour.  Pair failure still forces only the diagonal three-space, not the whole nine-space.  Excluding these profiles, foreign transport, response activity, arbitrary-root source coverage, and GLS8 remain separate. |
...
| `GLD66` | remaining product-selector source obligation | `GL` | On the all-six-pair base-survival branch, exclude the exact all-four-full first-root profile and the nonzero monocolour synchronized-plane profile using complete mixed equations and the simultaneous `GLS18`/`GLS19` Fitting data, or attach one of them to a named receiver with every gate.  One swallowed pair shadow, every non-leading four-port selector, other root orders, the promoted source, anchors, and permanent consequences remain separate. |
...
1. **Maximum-root same-pair target attachment.**  `GLS4` closes the
   ...
   **full-swallow** remainder universally starts at nuisance rank four.  This
   ...
   first-root product-selector branch.  The complete foreign labels turn the
   direct blocks into cross-Gram blocks of one bilinear form on four root
   coordinates.  A diagonal pure-three `C(B)` has exactly the matching-colour
   support, but that support isolates five independent incidence vectors.
   Hence, if all six pair base shadows survive, every four-port first-root
   quotient loses at least one pure colour.

   GLD66 uses the selector's still-unused foreign two-set equations.  Its
   nonzero desired response and two response-anchor functionals confine all
   port-incidence vectors to one common space of dimension at most three.
   Zero edges, third-colour edges, and the remaining two-colour matching
   assignments all contradict the induced pairing-kernel dimensions.  Thus a
   surviving first-root class forces `M_U` to be nonzero monocolour, and every
   proper first-root nuisance shadow is the coordinate plane missing that same
   colour.  The residue is now the all-four-full profile, the synchronized
   monocolour/plane profile, swallowed pair shadows, non-leading selectors,
   other root orders, and the promoted source.
