...
| `U7K` | Every offdiagonal extra matching in an aggregate active-cycle fibre now attaches exactly: a cancelling source or bridge shore contains a conformally minimal primitive cycle, sparse fan, or aggregate port whose terms embed with identical exponent differences into one mixed target fibre; otherwise bridge normalization enters the deeper branch or the complete target equation makes another word active. On a shortest cycle that word is outside the cycle or is the selected successor. The parallel-successor case is sharp even with all pure coefficients one, and can contribute zero new successor-fibre direction; a separate singleton excludes the fixed sharpness support. Purely diagonal excess and universal unit forcing remain open. | [Aggregate extra-matching target attachment](../claims/arbitrary-order/MATRIX_UNIT_AGGREGATE_EXTRA_MATCHING_TARGET_ATTACHMENT_THEOREM.md) |
| `U7` | Force the global target-lattice ideal to be a unit or close a topological exit: turn the attached primitive cycle/fan/aggregate-port relation into an odd dependency or unit, constrain diagonal-only aggregate defects, use an outside or parallel target equation to force genuine non-direct overlap and kill every quotient sheet, control a rank-at-least-two ideal, or close the deeper branch. This remains **open**: `U7K` proves pure-to-mixed attachment for offdiagonal extras but does not force a favourable cancellation, outside word, overlap, rank, unit, killed-sheet outcome, or deeper exclusion; a parallel extra can reuse the selected successor term and aggregate-cycle equations alone can leave `H` algebraically free. | [Attachment boundary](../claims/arbitrary-order/MATRIX_UNIT_AGGREGATE_EXTRA_MATCHING_TARGET_ATTACHMENT_THEOREM.md#8-consequence-for-the-live-u7-edge), [aggregate-cycle boundary](../claims/arbitrary-order/MATRIX_UNIT_AGGREGATE_ACTIVE_CYCLE_DEFECT_FACTORISATION_AND_SPLIT_FIBRE_SHARPNESS_THEOREM.md#6-consequence-for-the-live-u7-edge), and [global cross-multiplicity boundary](../claims/arbitrary-order/MATRIX_UNIT_CROSS_MULTIPLICITY_GLOBAL_TARGET_LATTICE_AND_HOLONOMY_DICHOTOMY_THEOREM.md#7-consequence-for-the-live-u7-edge) |
...
| `U7K` | boundary obligation | `U7` | Parallel bridge reuse can have zero successor-fibre difference even with all pure coefficients one. Diagonal-only excess, conversion of attached relations into units, forced non-direct overlap, and killed quotient sheets remain open. |
...
4. **Active word holonomy and pure-shore cancellation.** Every active
   ...
   non-direct overlap in general, control distinct parallel successor pairs
   and diagonal-only aggregate excess, or close the deeper branch.
