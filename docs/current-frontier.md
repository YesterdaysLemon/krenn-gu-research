...
| `U7D` | A complete exact eight-vertex `r=1` table simultaneously has all three pure target coefficients, strict positive endpoint balance, a moment-balanced representative over `C`, three proper nonempty nonrigidity sets, and an odd three-fibre binomial cycle with `H=-1`. Its exposed mixed coefficient is a transport-isolated one-monomial obstruction in multiplicity `(7,1,0)`. The table's only additional zero mixed fibre is locally coupled to cycle variables, but an exact `Q(t)` deformation satisfies it while retaining `H=-1`. This is sharpness and **not a witness**; stronger mixed coupling remains essential. | [Complete pure-target moment-compatible odd-holonomy sharpness](../claims/arbitrary-order/MATRIX_UNIT_COMPLETE_PURE_TARGET_MOMENT_COMPATIBLE_ODD_HOLONOMY_SHARPNESS_THEOREM.md) and [exposed-fibre transport isolation](../claims/arbitrary-order/MATRIX_UNIT_EXPOSED_MIXED_FIBRE_TRANSPORT_ISOLATION_AND_NEIGHBOUR_SHARPNESS_THEOREM.md) |
| `U7` | Exclude aggregate or binomial Laurent holonomy and the pure cofactor branching/even-cycle exits, or derive a contradiction from additional mixed coefficient identities. This remains **open**: even complete/pure/moment-compatible data do not exclude odd binomial holonomy, and conserved cofactor signs need not be positive. | [Phase-holonomy and pure-cofactor boundary](../claims/arbitrary-order/MATRIX_UNIT_PHASE_HOLONOMY_AND_MINIMAL_PURE_COFACTOR_FLOW_REDUCTION_THEOREM.md#8-combined-proof-topology-consequence) and [complete odd-holonomy sharpness](../claims/arbitrary-order/MATRIX_UNIT_COMPLETE_PURE_TARGET_MOMENT_COMPATIBLE_ODD_HOLONOMY_SHARPNESS_THEOREM.md#6-proof-topology-consequence-and-exact-frontier) |
...
| `U7D` | sharpened boundary obligation | `U7` | Additional mixed target equations beyond the cycle fibres are load-bearing, but the displayed exposed word is outside the transport multidegree and the only additional zero fibre in the exact table leaves the elimination ideal `(H+1)` unchanged. The next step needs same-multidegree equations, a proved cross-multiplicity coupling, or a deeper/pure exit. |
...
4. **Active word holonomy and pure-shore cancellation.** Every active
   ...
   `H=-1`; the elimination ideal in `H` remains `(H+1)`.  The next
   binomial-holonomy step must therefore use a stronger collection of
   same-multidegree identities, prove a new cross-multiplicity bridge, or
   couple to the deeper topology. A least pure cancelling residual likewise
   has a conserved cofactor flow which branches or alternates on even cycles.
