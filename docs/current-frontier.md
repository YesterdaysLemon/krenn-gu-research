...
flowchart TD
  ...
  GLD68 -->|swallowed complementary circuits / non-leading pair supply open| GL
...
| `GLD68` | corrected seven-shadow source boundary | `GL` | The `GLS17` route cannot supply all six pair base shadows: at least three are swallowed, and a maximal surviving family is a star or triangle.  A successor must turn those swallowed circuits into complete mixed-target contradictions or supply legal pair rows from a non-leading or promoted interface. |
...
| `GLD68` | complementary base-shadow parent obligation | `GL` | Use the at-least-three swallowed pair base circuits, arranged as complements of a star/triangle survivor family, to derive a bounded complete mixed-target contradiction; or construct the needed pair rows through a non-leading or promoted source.  The theorem supplies no full-coefficient identity, response activity, arbitrary-root cover, or permanent consequence. |
