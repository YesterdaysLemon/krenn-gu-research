...
flowchart TD
  ...
  GLS70 -->|remaining five-/six-deficient shared-deck integrability| GL
...
| `GLS70` | remaining five-/six-deficient shared-deck integrability obligation | `GL` | Couple Family A's two pure pair faces inside their binary triangle and Family B's exact-three-set leaf to all three strict four-open parents.  Treat the other pure/zero-triangle six-deficient profiles and both five-deficient residuals separately.  Resolve the unique-nonrigid/alternate-receiver branch plus every response, selector, nuisance-survival, activity, anchor, arbitrary-root attachment, and nonzero-anchor gate independently. |
