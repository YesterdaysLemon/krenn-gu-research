...
flowchart TD
  ...
  S2U -. diagonal sparse permanent / rank <=8 remain .-> S2
  S1 -->|boundary| S3
...
| `S2U` | sparse block-permanent boundary obligation | `S2` | The diagonal monomial case still asks whether a nonmonomial invertible joint cross matrix can have its empty block permanent equal GHZ modulo one diagonal root line.  The monomial joint-cross subcase is excluded.  Joint cross rank at most eight and all other previously open strata are not excluded. |
