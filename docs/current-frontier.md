...
flowchart TD
  ...
  S2BD -. residual beta/gamma/alpha coloops / rank <=4 / other branches .-> S2
  S1 -->|boundary| S3
...
| `S2BD` | remaining common-three-space obligation | `S2` | The complete `(1,1,1)` and `(1,1,2)` Hilbert--Burch profiles and the distinguished `(1,2,2)` `beta_t` and `alpha_s` coloops are closed.  Both residual second-root coloops are localized to `w_t=0` but remain open; the three third-root and two complementary first-root coloops, joint rank at most four, other S2T component types, other S2Q strata, and every higher order also remain open. |
