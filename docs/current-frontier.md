...
| `U7` | Force the global target-lattice ideal to be a unit or close a topological exit: combine the primitive pure-cycle relation with mixed response, force the connected pure exchange core into deeper-blocker data, derive a singleton or odd dependency, force useful cross-multiplicity overlap and kill every quotient sheet, or control a rank-at-least-two or aggregate-cycle ideal. This remains **open**: no theorem forces a favourable core, overlap, rank, unit, or deeper outcome, and every proper fully binomial-cycle quotient imposes only the known holonomy sign. | [Pure matching-covered boundary](../claims/arbitrary-order/MATRIX_UNIT_MINIMAL_PURE_COFACTOR_MATCHING_COVERED_CORE_AND_SINGLE_CYCLE_THEOREM.md#6-relation-to-the-matrix-unit-proof-forest), [global cross-multiplicity boundary](../claims/arbitrary-order/MATRIX_UNIT_CROSS_MULTIPLICITY_GLOBAL_TARGET_LATTICE_AND_HOLONOMY_DICHOTOMY_THEOREM.md#7-consequence-for-the-live-u7-edge), and [torsion-sheet aggregate boundary](../claims/arbitrary-order/MATRIX_UNIT_BINOMIAL_CORE_TORSION_SHEET_AND_RANK_ONE_AGGREGATE_QUOTIENT_THEOREM.md#6-consequence-for-the-live-u7-edge) |
...
| `U7H` | boundary obligation | `U7` | Neither the primitive pure-cycle relation nor the connected branching exchange core is yet coupled strongly enough to mixed response or deeper-blocker data to force exclusion. |
| `U7C` | boundary obligation | `U7` | The Laurent holonomy can take the required odd sign and aggregate fibres retain additional summands. The pure-cofactor side is now refined by `U7H`, but further mixed or deeper equations are still needed. |
...
4. **Active word holonomy and pure-shore cancellation.** Every active
   ...
   monomial cofactors; otherwise it has a multi-cycle exchange core of
   cyclomatic rank at least two.  The next pure-side step must couple one of
   those exact structures to mixed response or deeper-blocker data.
