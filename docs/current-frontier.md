...
flowchart TD
  ...
  S2AB -. multi-root joint rank <=6 remains .-> S2
  S1 -->|boundary| S3
...
| `S2AB` | remaining common-three-space obligation | `S2` | At least two root blocks with joint cross rank at most six remain open, along with the multi-boundary, `beta=0`, and collapsed cross-column component types, the other S2Q strata, and every higher order. |
