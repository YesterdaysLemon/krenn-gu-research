...
6. **All-bridge localized cancellations and bipartite least cores.** Every
   ...
   six, and preserves the exact-negative doubletons.  The next step is to
   force such a completion or control its rank-at-least-three target ideal.
   The block polynomial `1+X+Y+Z` is a proper nonunit with no exponent
   dependency, and the complementary fibre may be empty, so neither `A6`
   nor equality `N=beta+1` is an exclusion.
