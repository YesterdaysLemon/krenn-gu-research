...
flowchart TD
  ...
  S2BA -. two beta_t coordinate endpoints / eight other coloops / rank <=4 / other branches .-> S2
  S1 -->|boundary| S3
...
| `S2BA` | remaining common-three-space obligation | `S2` | The complete `(1,1,1)` and `(1,1,2)` Hilbert--Burch profiles are closed.  Two coordinate endpoints of the distinguished `beta_t` coloop, the other eight `(1,2,2)` coloops, joint rank at most four, other S2T component types, other S2Q strata, and every higher order remain open. |
