...
and the contextual `W=P=0` scout remain open; the generic first-minor
factorization and its terminal routes are not used as evidence.  See
...
witness.  Thus only the divisor `w=0` remains from this cover.  That divisor,
the other named chart/projective boundaries, and any locus outside the
retained ordinary sheet remain open.
...
- component twenty-one's complete generic marked `H31` fibre is empty.  Two
  ...
  `H22` ideals are unit.  Parameter-boundary intersections, extension poles,
  and arbitrary
