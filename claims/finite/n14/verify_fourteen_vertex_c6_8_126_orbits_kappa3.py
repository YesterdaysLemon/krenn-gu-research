"""Replay the order-14 C6+C8 certificate excluding 126 kappa>=3 orbits."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--orbit12-augmentation",
        type=Path,
        default=Path(
            "tmp/fourteen_vertex_c6_8_rule_sat_global_merge_"
            "v25_orbit12_targeted_augmentation.json"
        ),
    )
    parser.add_argument(
        "--conditioned-cnf",
        type=Path,
        default=Path(
            "tmp/fourteen_vertex_c6_8_126_orbits_"
            "v25_kappa3_conditioned.cnf"
        ),
    )
    parser.add_argument(
        "--proof",
    ...
    predecessor_output = Path(
        "tmp/fourteen_vertex_c6_8_126_orbits_"
        "predecessor_125_recheck.json"
    )
    run_quiet(
        [
            sys.executable,
            str(REPO_ROOT / "claims" / "finite" / "n14" / "verify_fourteen_vertex_c6_8_125_orbits_kappa3.py"),
            "--output",
    ...
    if (
        ...
        != "C6+C8_kappa3_125_first_factor_orbits_excluded"
        or predecessor_cnf
        != Path(
            "tmp/fourteen_vertex_c6_8_rule_sat_global_merge_"
            "v24_orbit11_targeted.cnf"
        )
        or predecessor.get("global_cnf_sha256")
        != "22e9925fb1a222ad36e35e5e9b449dcc7"
        "99ed684838ed8748c4c26bc6a0b1125"
    ):
        raise AssertionError("125-orbit predecessor reconstruction changed")

    augmentation = read_json(args.orbit12_augmentation)
    global_cnf = Path(str(augmentation["output_cnf"]))
    prerequisite = augmentation.get("three_connectivity_prerequisite")
    if not isinstance(prerequisite, dict):
        raise AssertionError("orbit-12 connectivity prerequisite missing")
    ...
    if (
        ...
        or prerequisite.get("base_extension_clauses") != 1612
        or augmentation.get("certificates_replayed") != 552
        or augmentation.get("new_transport_clauses") != 1632
        or augmentation.get("output_cnf_sha256") != sha256(global_cnf)
        or augmentation.get("output_cnf_variables") != 559
        or augmentation.get("output_cnf_clauses") != 129854
    ):
        raise AssertionError("orbit-12 augmentation changed")

    augmentation_recheck = Path(
        "tmp/fourteen_vertex_c6_8_126_orbits_"
        "orbit12_augmentation_recheck.json"
    )
    run_quiet(
        [
            sys.executable,
            str(REPO_ROOT / "claims" / "finite" / "n14" / "verify_fourteen_vertex_two_even_cycle_minimum_activity_augmentation.py"),
            str(args.orbit12_augmentation),
            "--output",
    ...
    if (
        ...
        or replay.get("certificates_replayed") != 552
        or replay.get("new_transport_clauses") != 1632
        or replay.get("output_cnf_variables") != 559
        or replay.get("output_cnf_clauses") != 129854
        or replay.get("sat") is not True
    ):
        raise AssertionError("orbit-12 augmentation replay changed")
    ...
    if (
        ...
        raise AssertionError("fresh 126-orbit frontier changed")
    ...
    if conditioned_formula.clauses != [
        *global_formula.clauses,
        ...
        raise AssertionError(
            "conditioned CNF is not global CNF plus the 126 selectors"
        )
    ...
    if read_json(drat_output).get("verified") is not True:
        raise AssertionError("126-orbit DRAT replay changed")

    payload = {
        "verified": True,
        "status": "C6+C8_kappa3_126_first_factor_orbits_excluded",
        "scope": (
            "order-14 equality architecture with full factor C6+C8, "
            "skeleton vertex connectivity at least 3, and a pinned "
            "first singleton factor in the listed 126 orbits"
        ),
        "minimal_counterexample_relevance": (
            "the sparse-graph reduction leaves the 4-connected branch "
            "and a degree-three escape case"
        ),
        "predecessor_125_replayed": True,
        "orbit12_certificates_replayed": 552,
        "orbit12_new_transport_clauses": 1632,
