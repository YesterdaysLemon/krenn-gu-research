"""Replay the order-14 C6+C8 certificate excluding 117 first orbits."""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "tmp/fourteen_vertex_c6_8_rule_sat_global_merge_v8_"
            "augmentation.json"
        ),
    )
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "tmp/fourteen_vertex_c6_8_rule_sat_117_orbits_"
            "conditioned.cnf"
        ),
    )
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "tmp/fourteen_vertex_c6_8_117_orbits_kissat.drat"
        ),
    )
    parser.add_argument(
        "--drat-trim",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "tmp/fourteen_vertex_c6_8_117_orbits_final_verified.json"
        ),
    )
    ...
    # The v8 layer is built on the already certified 108-orbit v7 layer.
    predecessor_output = Path(
        "tmp/fourteen_vertex_c6_8_117_orbits_"
        "predecessor_108_recheck.json"
    )
    run_quiet(
        [
            sys.executable,
    ...
    if predecessor.get("verified") is not True:
        raise AssertionError("108-orbit predecessor certificate changed")
    ...
    if (
        ...
        or reconstruction.get("certificates_replayed") != 800
        or reconstruction.get("new_transport_clauses") != 25290
        or reconstruction.get("sat") is not True
    ):
        raise AssertionError("fresh v8 global reconstruction changed")
    ...
    if (
        ...
        raise AssertionError("fresh 117-orbit frontier changed")
    ...
    if conditioned_formula.clauses != [
        *global_formula.clauses,
        ...
        raise AssertionError(
            "conditioned CNF is not global CNF plus the 117 selectors"
        )
    ...
    if drat.get("verified") is not True:
        raise AssertionError("117-orbit DRAT replay changed")

    payload = {
        "verified": True,
        "status": "C6+C8_117_first_factor_orbits_excluded",
        "scope": (
            "order-14 equality architecture with full factor C6+C8 "
            "and a pinned first singleton factor in the listed 117 orbits"
        ),
        "predecessor_108_verified": True,
        ...
        "v8_certificates_replayed": 800,
        "v8_new_transport_clauses": 25290,
