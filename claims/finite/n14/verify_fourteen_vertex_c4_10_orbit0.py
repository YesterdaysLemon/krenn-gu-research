"""Replay the fresh order-14 C4+C10 first-factor orbit-0 certificate."""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "tmp/fourteen_vertex_c4_10_rule_sat_orbit0_"
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "tmp/fourteen_vertex_c4_10_orbit0_final_verified.json"
        ),
    )
    ...
    reconstruction_output = Path(
        "tmp/fourteen_vertex_c4_10_orbit0_"
    ...
    if (
        ...
        or reconstruction.get("certificates_replayed") != 370
        or reconstruction.get("new_transport_clauses") != 11696
    ...
    if (
        orbit_audit["unsat_orbits"] != [0]
        or orbit_audit["sat_orbits"] != list(range(1, 425))
    ):
    ...
    if conditioned_formula.clauses != [
        *global_formula.clauses,
        ...
        raise AssertionError(
            "conditioned CNF is not global CNF plus orbit-0 selector"
        )
    ...
    payload = {
        "verified": True,
        "status": "C4+C10_first_factor_orbit_0_excluded",
        "scope": (
            "order-14 equality architecture with full factor C4+C10 "
            "and pinned first singleton factor orbit 0 only"
        ),
        ...
        "certificates_replayed": 370,
        "new_transport_clauses": 11696,
        "sat_first_factor_orbits": 424,
        "unsat_first_factor_orbits": [0],
