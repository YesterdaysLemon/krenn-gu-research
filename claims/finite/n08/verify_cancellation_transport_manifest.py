"""Audit cancellation-transport patterns inside learned Laurent conflicts."""
...
def main() -> None:
    ...
    payload = {
        "verified": True,
        "scope": (
            "elementary cancellation-transport certificates within exact "
            "Laurent conflict cubes"
        ),
        "source_manifest": str(args.manifest),
        "source_manifest_sha256": sha256(args.manifest),
        "center_degree": center_degree,
        "conflicts_checked": len(conflicts),
        "conflicts_with_transport_certificate": len(certified),
