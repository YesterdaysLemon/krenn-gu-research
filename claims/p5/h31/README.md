...
are four scoped exceptions.  The `embedded-p3/` package contains the
...
package.  Stage 9 used batch `p5-generic-obstructions-stage9`
(mapping_sha256
...
- Root shared utilities consumed by these verifiers:
  ...
  the repository root.
- The internal-`E=0` primary and audit also consume the shared root
  `derive_p5_h31_toric_marked_fibre_elimination.py` and separate primary/audit
  marked-basis constructors.  Those helpers serve other obligations and were
  not absorbed into the divisor package.
...
for the exact internal-`E=0` divisor leaf; the single-gate forest closes only
their rank-one-gate branch.  The
