#!/usr/bin/env python3
"""Independent exact audit of the eleventh component's generic H31 theorem."""
...
def specialized_rows(
    p: int, q: int, r: int, shifts: tuple[sp.Symbol, ...]
    ...
    canonical = (u, add(a_bar, b), add(scale(r, a_bar), b), a_bar)
    beta = tuple(
        tuple(sp.expand(canonical[mode][coordinate] + shifts[mode] * alpha[mode][coordinate]) for coordinate in range(4))
        for mode in range(4)
    )
    return alpha, beta
...
def extension_rows(
    ...
    coefficients = {
        word: subset_permanent(
            tuple(beta_extended[mode] if word[mode] else alpha_extended[mode] for mode in range(4))
        )
        for word in WORDS
    }
    mixed = sp.Matrix(
...
def main() -> None:
    ...
    for point in points:
        ...
        pure = {
            word: sp.factor(
                subset_permanent(tuple(beta[mode] if word[mode] else alpha[mode] for mode in range(4)))
            )
            for word in WORDS
        }
        assert pure[(1, 1, 1, 1)] == -4 * point[0] * (point[2] + 1)
