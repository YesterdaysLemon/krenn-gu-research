#!/usr/bin/env python3
"""No-import audit of component 23's s=1 triple-infinity corner."""
...
PRIMARY = (
    ROOT / "verify_p5_component23_s_one_triple_parameter_infinity_corner_obstruction.py"
)
...
def compactification_geometry_audit():
    ...
    assert tuple(matrix.rank() for matrix in matrices) == (3, 2, 2, 3, 3, 3)
    expected_kernel = [sp.Matrix((0, 1, 0, 0)), sp.Matrix((0, 0, 1, 0))]
    assert matrices[1].nullspace() == expected_kernel
    assert matrices[2].nullspace() == expected_kernel
    return str(closure), tuple(map(int, gradient))
...
def extension_coefficients(deletion, alpha, beta, extension):
    ...
    return {
        word: permanent4(
            tuple(alpha_p[i] if bit == 0 else beta_p[i] for i, bit in enumerate(word))
        )
        for word in WORDS
    }
...
def global_one_marked(deletion, extension, alpha, beta):
    pure = one_marked_map(0, alpha, beta).row_join(sp.zeros(8, 1))
    neighbour = marked_extension(deletion, extension, alpha, beta, 0)
...
def h31_projection_audit():
    ...
    for deletion in range(4):
        ...
        output.append((deletion, tuple(map(str, expected))))
    return output
    ...
    for deletion in (0, 1):
        for name, marking in markings:
            ...
            determinant = sp.factor(
                ...
                .extract((1, 3, 7, 8, 9), range(5))
                .det(method="domain-ge")
            )
            sign = -1 if deletion == 0 else 1
            assert sp.factor(determinant - sign * 32 * actual_a) == 0
            output.append(
                (deletion, name, str(actual_a), str(actual_b), str(determinant))
            )
    return output
...
def h22_audit(chart):
    ...
    generators = ",".join(
        coefficient_row(expression) for model in models for expression in model["mixed"]
    )
    diagonals = tuple(
        coefficient_row(model[key]) for model in models for key in ("A", "B")
    )
    expected = (
        FINITE_MODULE
        if chart == "finite"
    ...
    membership = (1, 0, 1, 0) if chart == "finite" else (1, 1, 1, 1)
    ...
    return chart, membership


def main():
    geometry = compactification_geometry_audit()
    projections = h31_projection_audit()
    branches = branch_audit()
    modules = tuple(h22_audit(chart) for chart in ("finite", "infinity"))
