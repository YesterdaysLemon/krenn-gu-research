...
weighted-`H22` claim package: theorem document + primary verifier +
independent audit, moved together with preserved filenames.  Batch:
`p5-generic-obstructions-stage9` (Stage 9), mapping_sha256
...
The H22 `common-center-kernel-star` `r-zero-divisor` generic theorem,
the unequal-complement / unequal-endpoint generic theorems, and the
embedded-p3 / common-active-binary-triangle generic documents remain
at the repository root with their boundary forests or open recursion.
Migration status changes no theorem claim; the global conjecture
remains **UNRESOLVED**.
