#!/usr/bin/env python3
"""No-import audit of component 25's exceptional A2=0 obstruction."""
...
# Dimensionless exact factor cover for A2=0.
A, B, Lambda = sp.symbols("A B Lambda")
c2_global = (3 * B**2 + B - 1) * A**2 - B * (B + 1) ** 2 * A + B**3
a1_global = -2 * (
...
n_on_a2 = a1_global * Lambda + a0_global
a_minus_one_weight_factor = (3 * B - 1) * Lambda - (B + 1)
assert (
    sp.expand(n_on_a2.subs(A, -1) - 2 * (B - 1) * (B + 1) * a_minus_one_weight_factor)
    == 0
)
assert sp.expand(n_on_a2.subs(B, 1) + 2 * (Lambda - 1) * (A - 1) ** 2 * (A + 1)) == 0

# Exact resultants identify every denominator of the C2 field chart.
r_global = 1 + A * B
k2_numerator = (A + B) ** 2 - A * B * r_global
e2_minus_k2_numerator = A**2 * r_global - k2_numerator
h_numerator = (a1_global - a0_global) * r_global + (a0_global + a1_global) * (A + B)
standing_t_numerator = -(B - 1) * a0_global - (B + 1) * a1_global
assert sp.expand(e2_minus_k2_numerator.subs(A, -1)) == 0
assert (
    sp.factor(
        sp.discriminant(c2_global, A) - B**2 * (B**4 - 8 * B**3 + 2 * B**2 + 8 * B + 1)
    )
    == 0
)
resultant_identities = {
    "Q": (A + B, 4 * B**3 * (B + 1)),
    "R": (r_global, (B + 1) ** 2 * (B**3 - B**2 + 3 * B - 1)),
    "k": (
        k2_numerator,
    ...
    "e^2-k^2": (
        ...
        4 * B**5 * (B - 1) * (B + 1) ** 3 * (2 * B**2 + 3 * B - 1),
    ),
    "A1": (
        a1_global,
        -16 * B**6 * (B - 1) * (B + 1) ** 3 * (B**4 - 8 * B**3 + 4 * B - 1),
    ),
    "A0": (a0_global, 16 * B**6 * (B - 1) ** 3 * (B + 1) ** 5),
    "H": (
        h_numerator,
        -64 * B**10 * (B - 1) * (B + 1) ** 6 * (B**2 - 8 * B + 3),
    ),
    "T": (standing_t_numerator, 128 * B**10 * (B - 1) * (B + 1) ** 4),
}
for polynomial, expected in resultant_identities.values():
    assert sp.factor(sp.resultant(c2_global, polynomial, A) - expected) == 0
...
a0 = l_mul(l_mul(l_sub(a, l_one), l_scalar(b + 1)), a0_core)
...
def residuals(w_value, z6_value):
    alpha_rows, beta_rows = projected(w_value, z6_value, "D01")
...
print(
    json.dumps(
        {
            "status": "PASS",
            "audit_independence": "no project imports; subset-DP permanents; recursive determinants",
            "A2_factor_cover": ["es+1", "js-1", "C2"],
            "easy_factors_empty_or_outside_chart": True,
            "es_plus_one_boundary": "e^2-k^2=0",
            "js_minus_one_N_factor": "-2(lambda-1)(es-1)^2(es+1)",
            "C2_field": "Q(b)[a,k]/(C2,k^2-P+ab)",
            ...
            "C2_leading_divisor_closed_here": False,
