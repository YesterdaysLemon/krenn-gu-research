#!/usr/bin/env python3
"""No-import audit of component 25's C0 companion leading boundary."""
...
def l_mul(left, right):
    # b^2=relation_linear*b+relation_constant on 3b^2-b-1=0.
    return (
...
e = l_neg(
    l_div(
        l_mul(j, j),
        l_mul(l_sub(j, l_one), l_sub(j, l_one)),
    )
)
q = l_add(e, j)
...
# On C0=0, the nonzero companion is lambda=-A1/A2.
...
a2 = l_mul(l_mul(l_add(e, l_one), l_sub(j, l_one)), a2_core)
...
assert slope == (
    base.from_expr(sp.Rational(1, 9)),
    base.from_expr(sp.Rational(2, 3)),
)
