#!/usr/bin/env python3
"""Close component 23's r=0 constant-profile affine weighted-H22 open."""
...
def coefficient_vector(expression, substitutions):
    coefficients = tuple(
        sp.cancel(
            sp.diff(expression.subs({r: 0, **substitutions}, simultaneous=True), var)
        )
        for var in x
    )
...
def pure_pair_certificate():
    ...
    planes = tuple(
        (
            tuple(sp.sympify(entry).subs(r, 0) for entry in alpha[index]),
            tuple(sp.sympify(entry).subs(r, 0) for entry in beta[index]),
        )
...
def module_certificate(
    ...
    for model in models:
        for equation in model["mixed"]:
            vector, scale = coefficient_vector(equation, substitutions)
    ...
    assert all(is_t_unit(scale) for scale in row_scales)
    return label


def endpoint_certificates():
    lambda_zero = mixed.subs({r: 0, lam: 0}, simultaneous=True)
        ...
        json.dumps(
            {
                ...
                "claim_label": "VERIFIED_AFFINE_CONSTANT_PROFILE_OPEN_EMPTY",
