"""Independent no-import audit of the repeated-coordinate HB localization.
...
def derivative_and_grid_audit() -> None:
    ...
    for kernel_vector in (k1, k2):
        image = add(
            *(scale(coefficient, column) for coefficient, column in zip(kernel_vector, columns, strict=True))
        )
    ...
    gamma0 = (F(1), F(0), F(-2, 5))
    gamma1 = (F(0), F(1), F(-3, 5))
    assert sum(a * b for a, b in zip(gamma0, z, strict=True)) == 0
    assert sum(a * b for a, b in zip(gamma1, z, strict=True)) == 0
    assert gamma0[:2] == (F(1), F(0))
    assert gamma1[:2] == (F(0), F(1))
    print("independent derivative/grid audit: PASS (Fraction rank and rows)")
...
def restriction_rows(bases: tuple[list[Vector], list[Vector], list[Vector]]) -> list[Vector]:
    return [
...
def incidence_kernel_audit() -> None:
    ...
    repeated = restriction_rows(([e1, e2], [e1, e2], [e0, e2]))
    expected_repeated = [
        coefficient_vector({(0, 0, 0): 1}),
        coefficient_vector({(0, 0, 1): 1}),
        coefficient_vector({(0, 0, 2): 1}),
        coefficient_vector({(1, 1, 1): 1}),
    ]
    assert kernel_equals(repeated, expected_repeated)
    print("independent incidence kernels: PASS (separate row elimination)")


def boundary_and_orientation_audit() -> None:
    ...
    assert determinant(((F(2), F(3), F(5)), unit(3, 0), unit(3, 1))) == 5
    assert determinant(((F(2), F(3), F(0)), unit(3, 0), unit(3, 1))) == 0

    # L*E00 and L*E11 expose the two off-diagonal entries independently.
    ...
    print("independent boundary/orientation audit: PASS (z_s and L columns)")
...
def square_chart_audit() -> None:
    ...
    # Fully conjugate two-source zero divisors share the square's two base
    # factors; a fully transverse second square cannot result.
    z0 = source(2, 1)
    z1 = source(2, 2)
    v0 = add(scale(2, w), z0)
    v1 = add(scale(-3, w), z1)
    ...
    # Three-source nonzero scaling chart has the same zero product and a
    # dependent scaling row family.
    u3 = add(x, y, z)
    ...
    print("independent square-chart audit: PASS (sparse two-/three-source)")


def main() -> None:
    derivative_and_grid_audit()
    incidence_kernel_audit()
    boundary_and_orientation_audit()
    square_chart_audit()
    print("independent repeated-coordinate Hilbert-Burch localization: PASS")
