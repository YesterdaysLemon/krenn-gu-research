#!/usr/bin/env python3
"""Generate the exact endpoint-incidence Nullstellensatz certificates.

This generator is not needed to replay the proof certificate.  It requires
Singular 4.x, either directly on PATH or through the default WSL
distribution.  The generated JSON is checked by both the primary verifier
and the independent standard-library audit.
"""
...
proc endpoint_ideal(int re,int qe,int sm)
{{
  ...
  P0=0,0,0,1;
  P1=(sm%2),((sm div 2)%2),((sm div 4)%2),0;
...
{{
  ...
  {{
    ...
    {{
      ...
      if (unit_column==0)
      {{
        print("ERROR|nonunit|"+string(re)+"|"+string(qe)
              +"|"+string(mask));
      }}
      ...
      {{
        print("ERROR|bad_lift|"+string(re)+"|"+string(qe)
              +"|"+string(mask));
      }}
...
def parse_output(stdout: str) -> dict[str, object]:
    ...
    expected = {f"{a}{b}-{mask}" for a in range(2) for b in range(2) for mask in range(1, 8)}
