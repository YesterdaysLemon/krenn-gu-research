"""Standalone recursive-permanent audit of the GLD36 wz=2 surface."""
...
ROW_KEYS = (
    (tuple(map(int, "0100")), tuple(map(int, "1000"))),
    (tuple(map(int, "1222")), tuple(map(int, "1222"))),
)
...
def main():
    ...
    assert vector == [1, 1 / U]
    assert sp.factor(detector + 1 / U) == 0
    print("PASS: standalone recursive-permanent audit independently derives the GLD36 contradiction")
