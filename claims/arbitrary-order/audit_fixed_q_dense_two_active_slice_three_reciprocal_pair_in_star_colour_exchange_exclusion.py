"""Standalone matching-topology audit of the GLD55 colour-exchange transfer."""
...
def main():
    ...
    assert sp.cancel(exchanged) != 0
    assert sp.cancel(exchanged - 1) != 0
    ...
    reversed_star = {(right, left) for left, right in out_star}
    position_swap = {0: 1, 1: 0, 2: 2, 3: 3}
    assert {(position_swap[left], position_swap[right]) for left, right in reversed_star} == {
        (0, 1), (2, 1), (3, 1)
    }
    print("PASS: 945 matching topologies independently prove the GLD55 covariance")
