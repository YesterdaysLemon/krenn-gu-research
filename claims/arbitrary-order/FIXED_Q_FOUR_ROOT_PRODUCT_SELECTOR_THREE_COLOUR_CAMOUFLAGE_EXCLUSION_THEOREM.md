...
**PARTIALLY WITHDRAWN (2026-08-24).**  The product-selector cross-Gram
identity in Lemma 2 confuses the root companion `G_D`, which the complete
module selector annihilates, with the full matching coefficient `F_D` after
direct outside edges are restored.  Theorem 5, Corollaries 5.1--5.2, and every
source/exclusion conclusion depending on Lemma 2 are withdrawn and must not
be used.  The exact correction and graph-side counterexample to this
conditional theorem are in
...
Lemma 1's factored-selector implication, the generic matching
stratification, Lemma 3's standalone diagonal-support classification, and
Lemma 4's abstract conditional linear-algebra statement survive only at
their separately stated scopes.  Everything below that asserts the
cross-Gram bridge or its exclusion consequences is retained as a historical
audit trail.

**Former claim (withdrawn).**  A surviving `GLS17` four-port first-root class supplies more
...
No external literature claim is used.  The historical package introduced the
product-form selector lift, a claimed common root-incidence cross-Gram
identity, the complete pure-three diagonal support reduction, and the four-
dimensional camouflage obstruction.  The cross-Gram item is withdrawn; the
other three survive only at their separately stated scopes.
...
### Lemma 2 (withdrawn: product-selector cross-Gram identity)
...
The former proof contracted every root in the full perfect-matching
coefficient with `rho`, evaluated the `Q` slots at `z_Q`, and wrote `F_D` for
the resulting full coefficient on an outside set `D subset B`.

It then claimed that the pure-`M` operator identity gave
...
The fourth line of (12) is false.  Legality sets the distinct root companion
`G_(Q union {u,v})` to zero, while
`F_(Q union {u,v})=mB_uv`.  The calculation below is retained as the valid
conditional matching derivation under the extra, unjustified full-
coefficient zero; it is not a proof that the selector supplies that zero.
...
which would give (11), and normalization would give (10).  This does not
repair the missing hypothesis.  `square`

The 105-matchings expansion still splits as `24+72+9` according to zero, one,
or two outside--outside edges.  Conditional on the extra full-coefficient
zero, (13) is pointwise on every root-incidence and nuisance-rank fibre; no
hafnian, incidence minor, or entry of `J` is inverted.
...
### Theorem 5 (withdrawn: product-selector camouflage exclusion)
...
### Corollary 5.1 (withdrawn: all-seven leading-survival pure-three branch)
...
### Corollary 5.2 (withdrawn: pure leading-column failure)
...
The corrected exact frontier is therefore

```text
GLS17 surviving four-port class gives product selector:       PROVED;
complete foreign labels give common K^4 cross-Gram form:      FALSE/WITHDRAWN;
diagonal pure-three response has matching camouflage support: PROVED;
matching camouflage under an independently supplied K^4 form: EXCLUDED;
all pair shadows + one full-three first-root quotient:         NOT EXCLUDED;
...
The colour-loss residue formerly stated here is withdrawn.  The controlling
boundary is the root-companion/full-coefficient separation theorem linked in
the status: a future argument must supply a genuine full-target equation, a
second independently legal response axis, or another coefficient-pure map.
