"""Standalone recursive-permanent audit of the GLD39 chart completion."""
...
ROW_KEYS = (key("1202", "0212"), key("2212", "2212"))
...
def main():
    ...
    assert {index: value for index, value in enumerate(rows[0]) if value != 0} == {7: -W, 19: W}
    assert {index: value for index, value in enumerate(rows[1]) if value != 0} == {7: -1, 19: 1}
    assert rhs == [0, -1]
    ...
    assert vector == [1, -W]
    assert sp.factor(sum(value * target for value, target in zip(vector, rhs, strict=True)) - W) == 0
    print("PASS: standalone recursive-permanent audit derives the uniform GLD39 contradiction 0=w")
