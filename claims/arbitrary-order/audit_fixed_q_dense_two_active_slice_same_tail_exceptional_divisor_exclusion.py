"""Standalone recursive-permanent audit of the GLD45 divisor closure."""
...
def cross_value(colour, root, port):
    ...
    if colour == 0 and (root, port) == (0, 1):
        return W
    if colour == 1 and (root, port) == (1, 0):
        return W / (W - 1)
    return sp.Integer(0)
...
def main():
    ...
    print(
        "PASS: standalone recursive-permanent audit derives the GLD45 "
        "same-tail exceptional-divisor contradiction"
    )
