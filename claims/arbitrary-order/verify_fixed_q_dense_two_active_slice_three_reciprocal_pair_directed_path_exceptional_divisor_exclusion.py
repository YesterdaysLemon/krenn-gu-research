"""Primary 945-match replay for the GLD51 directed-path closure."""
...
WEIGHTS = (
    W + 1,
    -(2 * U * W - U + 1) / (U * (U - 1)),
    (U**3 + 2 * U * W - 5 * U + 1) / (U * (U - 1)),
    -(U**2 - U * W + U - 1) / (U * (U - 1)),
    (U**3 + 2 * U**2 * W - U * W - 3 * U - W) / (U * (U - 1)),
    (U**2 * W + 2 * U**2 - 4 * U - W) / (U * (U - 1)),
    -1 / U,
    (U - 2) * (U + 1) * (U + 2) / (U - 1),
    -(U**3 - U**2 + U * W - 4 * U + 1) / (U * (U - 1)),
    1,
    (W - 1) / (W * (U - 1)),
    -(W - 1) / (U - 1),
    -(U - 2) * (U + 1) * (U + 2) * (W - 1) / (W * (U - 1)),
    -(U + 1) * (U**2 - U - 1) / (U * (U - 1)),
)
...
def main():
    ...
    print(
        "PASS: 945-match expansion closes the full GLD51 three-pair "
        "directed-path exceptional surface"
    )
