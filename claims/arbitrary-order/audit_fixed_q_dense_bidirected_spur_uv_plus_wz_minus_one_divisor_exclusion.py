"""Standalone recursive-permanent audit of the GLD37 uv+wz-1 divisor."""
...
FIRST_KEYS = (key("0100", "1000"), key("1222", "1222"))
SECOND_KEYS = (key("0100", "0010"), key("2212", "2212"))
...
def main():
    ...
    assert first_vector == [1, (W * Z - 1) / U]
    assert sp.factor(first_detector + (W * Z - 1) / U) == 0
    assert second_vector == [1, W * (W * Z - 1) / U]
    assert sp.factor(second_detector + W * (W * Z - 1) / U) == 0
    print("PASS: standalone recursive-permanent audit independently derives both GLD37 contradictions")
