...
the withdrawn two-file state-lift route under
[`history/state-lift-cycle-fibre/`](history/state-lift-cycle-fibre/). These
additions change filesystem ownership only; each artifact retains its written
scope, lifecycle, and evidence role.
The flat layout preserves the exact, sometimes asymmetric evidence surface of
each claim. Proximity in this directory does not turn a partial result,
correction, modular audit, experiment, or replay script into stronger proof
evidence.

The 42 `ROOT_M7` files form a complete ownership subtree, not a complete
mathematical case cover. In particular, the relative fourth-incidence shell
excludes 908 supports rather than all `binomial(104,4) = 4,598,126` supports.
...
The nonlinear H4 divisor is the sole named retained low-rank candidate.
[`GLD88`](FOUR_ROOT_TORUS_STAR_EQUAL_LEAF_H4_GENERIC_RANK_SIX_COMMON_ROW_KERNEL_EXCLUSION_THEOREM.md)
now excludes a nonempty explicit principal open inside it: two bordered
linear Schur residuals force a rational three-parameter family, and the
...
transcribes the pinned sparse supports and computes the six GLD97 determinants
directly in fixed quotient algebras `K[q]/(Q6)`.  Exact Macaulay membership at
total offset degree four shows that the fixed-fibre ideal generated by `Q6`
and the six minors equals `(Q6,B,C)` at the regular rational samples
`(p,a)=(2,0),(3,1)` and at the
regular `GF(11)` sample `(2,0)`.  The `p=0,1` and `GF(7)` controls are reported
as gate overlaps/failures rather than inverted away.  Raising the diagnostic
cap to degree eight shows why full-row surjectivity is the wrong generic
shortcut: after `B,C` enter the span at degree four, the nonconstant target
still has stable exact deficiencies `8` at `(2,0)` and `4` at `(3,1)` through
degree eight.  None of these sampled identities globalizes in `p`, proves an
exhaustive fibre cover, or changes the live frontier or global
**UNRESOLVED** status.
...
cell closure, not a witness exclusion.

The next-cell
[`transverse four-cell detector theorem`](PROJECTIVELY_CONSTANT_LIFT_TRANSVERSE_FOUR_CELL_TWO_OPEN_DETECTOR_THEOREM.md)
proves that `h -> P_4(h,a,a,b)` is injective when `a_u,b_u` are independent
...
The
[`fixed-Q dense directed-spur uv=1 divisor exclusion`](FIXED_Q_DENSE_DIRECTED_SPUR_UV_ONE_DIVISOR_EXCLUSION_THEOREM.md)
closes a second divisor pointwise.  A sixteen-row relation leaves
`-4u(u+w)(w+2)(u+w+1)(uw+2u+w)`.  Four exact curve relations reduce its
vanishing locus to four rational points and the shared family `u^2+1=0`;
point cores and a twelve-row quotient certificate close all of them.  Thus
`uv=1` is empty for every allowed `w`, leaving two `GLD26` divisors open.

The
[`fixed-Q dense directed-spur uv-u-v-1 divisor exclusion`](FIXED_Q_DENSE_DIRECTED_SPUR_UV_MINUS_U_MINUS_V_MINUS_ONE_DIVISOR_EXCLUSION_THEOREM.md)
...
The
[`bidirected-spur uv=-1, z=1 surface exclusion`](FIXED_Q_DENSE_BIDIRECTED_SPUR_UV_MINUS_ONE_Z_ONE_SURFACE_EXCLUSION_THEOREM.md)
closes a second residual surface pointwise.  The two complete rows
`(1202;0212)` and `(2212;2212)` already give `0=w`; a disjoint two-row check
gives `0=-u^(-1)`.  Thus only `z=-1` and `wz=2` remain on this divisor.

The
[`bidirected-spur uv=-1, z=-1 surface exclusion`](FIXED_Q_DENSE_BIDIRECTED_SPUR_UV_MINUS_ONE_Z_MINUS_ONE_SURFACE_EXCLUSION_THEOREM.md)
closes a third residual surface pointwise.  Two disjoint two-row relations
give `0=-w/u` and `0=-u^(-1)`.  Thus only `wz=2` remains on this divisor.

The
[`bidirected-spur uv=-1, wz=2 surface exclusion`](FIXED_Q_DENSE_BIDIRECTED_SPUR_UV_MINUS_ONE_WZ_TWO_SURFACE_EXCLUSION_THEOREM.md)
closes the final residual surface with the two-row relation `0=-u^(-1)`.
Together with the exact GLD32 case cover and GLD33--GLD35, this pointwise
completes the nonzero `uv=-1` divisor.  Four other GLD31 divisors remain.

The
[`bidirected-spur uv+wz-1 divisor exclusion`](FIXED_Q_DENSE_BIDIRECTED_SPUR_UV_PLUS_WZ_MINUS_ONE_DIVISOR_EXCLUSION_THEOREM.md)
...
The
[`dense bidirected-spur nonzero-chart completion`](FIXED_Q_DENSE_BIDIRECTED_SPUR_NONZERO_CHART_COMPLETION_THEOREM.md)
then re-expands a row pair first used in GLD34 without any specialization.
The two complete equations are
`-w p_(0,2,1)+w p_(1,2,1)=0` and
`-p_(0,2,1)+p_(1,2,1)=-1`; their difference gives `0=w`.  Since `w!=0` is
an original GLD31 chart hypothesis, the entire nonzero bidirected-spur chart
is empty.  This closes the two formerly remaining divisors and subsumes
...
The
[`dense two-active-slice same-tail exceptional-divisor exclusion`](FIXED_Q_DENSE_TWO_ACTIVE_SLICE_SAME_TAIL_EXCEPTIONAL_DIVISOR_EXCLUSION_THEOREM.md)
closes the first of those unions.  On the GLD44 residue `u=-1`, a twelve-row
certificate over `Q(w)` leaves `1` with denominator lcm `2w(w-1)`.  Both
roots are already absent on the active reciprocal-pair locus.  Consequently
the entire same-tail orbit, all `12` labelled support masks, is pointwise
empty.  Reverse, same-head, chain, and disjoint exceptional divisors remain.

The
[`dense two-active-slice disjoint exceptional-divisor exclusion`](FIXED_Q_DENSE_TWO_ACTIVE_SLICE_DISJOINT_EXCEPTIONAL_DIVISOR_EXCLUSION_THEOREM.md)
then closes the disjoint orbit.  On `u=-1`, an eleven-row certificate leaves
`1` with denominator lcm `2w(w-1)`.  The permutation `(0 1)(2 3)` exchanges
the two disjoint directed pairs and therefore covers the second GLD44 branch
`w=-1`.  Hence all `12` disjoint masks are pointwise empty.  Reverse,
same-head, and chain exceptional divisors remain.

The
[`dense two-active-slice reverse exceptional-divisor exclusion`](FIXED_Q_DENSE_TWO_ACTIVE_SLICE_REVERSE_EXCEPTIONAL_DIVISOR_EXCLUSION_THEOREM.md)
closes the reverse orbit.  A seventeen-row certificate closes `u=-1`, pair
exchange closes `w=-1`, and a separate fifteen-row certificate closes
`uw=-1`.  Their denominator roots are all already forbidden by the active
reciprocal parametrization.  Thus all `6` reverse masks are pointwise empty;
...
The
[`dense two-active-slice three-reciprocal-pair generic exclusion`](FIXED_Q_DENSE_TWO_ACTIVE_SLICE_THREE_RECIPROCAL_PAIR_GENERIC_EXCLUSION_THEOREM.md)
advances to the next support size.  The `220` three-pair masks split exactly
into `13` position-relabelling orbits.  One exact `14`--`18`-row
rational-function certificate per orbit excludes its generic complement and
leaves a fully displayed exceptional hypersurface atlas.  This is not a
...
The
[`three-pair fork-path exceptional-divisor exclusion`](FIXED_Q_DENSE_TWO_ACTIVE_SLICE_THREE_RECIPROCAL_PAIR_FORK_PATH_EXCEPTIONAL_DIVISOR_EXCLUSION_THEOREM.md)
closes `O4`.  Two fourteen-row certificates exclude both residual surfaces
away from their intersection, and a thirteen-row curve certificate closes
...
The
[`three-pair in-fork exceptional-divisor exclusion`](FIXED_Q_DENSE_TWO_ACTIVE_SLICE_THREE_RECIPROCAL_PAIR_IN_FORK_EXCEPTIONAL_DIVISOR_EXCLUSION_THEOREM.md)
closes `O11`.  Four exact surface certificates reduce its exceptional union
to three one-parameter intersections; three exact curve certificates have
...
The
[`three-pair O6 exceptional-divisor exclusion`](FIXED_Q_DENSE_TWO_ACTIVE_SLICE_THREE_RECIPROCAL_PAIR_O6_EXCEPTIONAL_DIVISOR_EXCLUSION_THEOREM.md)
closes `O6`.  Four exact surface certificates leave two legal one-parameter
intersections; fifteen-row exact curve certificates have denominator roots
only at inactive amplitudes.  Hence all `24` O6 masks are empty, leaving four
GLD50 orbitwise exceptional unions open.

The
[`three-pair O3 exceptional-divisor exclusion`](FIXED_Q_DENSE_TWO_ACTIVE_SLICE_THREE_RECIPROCAL_PAIR_O3_EXCEPTIONAL_DIVISOR_EXCLUSION_THEOREM.md)
closes `O3`.  Four exact surface certificates leave five intersections;
five exact curve certificates reduce those to two rational points, and
...
The
[`three-pair O9 exceptional-divisor exclusion`](FIXED_Q_DENSE_TWO_ACTIVE_SLICE_THREE_RECIPROCAL_PAIR_O9_EXCEPTIONAL_DIVISOR_EXCLUSION_THEOREM.md)
closes the final three-pair orbit by fourteen exact cores.  Five saturated
denominator covers are unit ideals; the long surface reduces to two coprime
univariate factors.  Hence all `8` O9 masks, and all `220` exactly-three-pair
masks, are pointwise empty in the fixed-Q dense cell.
...
The GLS31 zero-anchor evaluation-pencil identities and maximum-root,
...
The GLS34 tangent-root Fitting boundary, simultaneous constant-anchor
survival dichotomy, and complete three-colour Segre-silence classification
are covered by the
[`maximum-root zero-anchor tangent-root/Segre-silence hostile review`](../../docs/audits/MAXIMAL_ROOT_SURPLUS_TWO_ZERO_ANCHOR_TANGENT_ROOT_FITTING_AND_CONSTANT_ANCHOR_SEGRE_SILENCE_REVIEW_2026-08-22.md).
The GLS35 raw residual-absent root-deck quotient, complete-target
...
The GLS44 characteristic-zero arbitrary-root exclusion of every nonzero
...
The GLD28 `uv=1` divisor exclusion is covered by the
...
The GLD34 `z=1` surface closure is covered by the
[`bidirected-spur uv=-1, z=1 hostile review`](../../docs/audits/FIXED_Q_DENSE_BIDIRECTED_SPUR_UV_MINUS_ONE_Z_ONE_SURFACE_EXCLUSION_REVIEW_2026-08-20.md).
The GLD35 `z=-1` surface closure is covered by the
[`bidirected-spur uv=-1, z=-1 hostile review`](../../docs/audits/FIXED_Q_DENSE_BIDIRECTED_SPUR_UV_MINUS_ONE_Z_MINUS_ONE_SURFACE_EXCLUSION_REVIEW_2026-08-20.md).
The GLD36 `wz=2` surface closure and divisor completion are covered by the
...
The GLD45 same-tail divisor closure is covered by the
[`dense two-active-slice same-tail divisor hostile review`](../../docs/audits/FIXED_Q_DENSE_TWO_ACTIVE_SLICE_SAME_TAIL_EXCEPTIONAL_DIVISOR_EXCLUSION_REVIEW_2026-08-20.md).
The GLD46 disjoint divisor closure is covered by the
...
The GLD59 O6 divisor closure is covered by the
...
The GLD62 O9 divisor closure is covered by the
[`three-pair O9 hostile review`](../../docs/audits/FIXED_Q_DENSE_TWO_ACTIVE_SLICE_THREE_RECIPROCAL_PAIR_O9_EXCEPTIONAL_DIVISOR_EXCLUSION_REVIEW_2026-08-20.md).
...
Later reviewed tranches routed those artifacts to their current owners or
neutral disputed-ownership packages; relocation did not adjudicate scientific
status or a recorded ownership disagreement.

Authoritative migration records:

- [reviewed Stages 27-29 plan](../../docs/architecture/layout-migration-stages27-29-reviewed-plan.md);
- [Stage 27 dry run](../../docs/architecture/high-order-symbolic-forest-stage27-dry-run.md);
